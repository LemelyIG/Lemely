...
export default {
	async fetch(request: Request, env: Env): Promise<Response> {
		...
		// (`_avatar_url_for`); see nginx.conf for the full rationale.
		out.headers.set(
			"Content-Security-Policy",
			"default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https://storage.googleapis.com; media-src 'self' blob:; font-src 'self'; connect-src 'self'; worker-src 'self'; manifest-src 'self'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'; object-src 'none'",
		);
		return out;
	},
