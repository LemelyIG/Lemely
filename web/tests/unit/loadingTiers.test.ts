...
/*
 * DESIGN.md §12's loading tiers, pinned across the three places they have to
 * agree (PR 2 part B).
 *
 * Three checks, one per surface that can silently drift from the other two:
 *
 ...
 *     read from.
 *   - every `RouteFallback` call site in `routes.tsx` (the fourteen top-level
 *     routes) carries `frame="standalone"` — a call site that regresses to the
