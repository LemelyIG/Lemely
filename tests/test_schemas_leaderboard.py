...
# `streak` joined both sets in P5.8 chunk C (D5.14 §2). Widening this set is
# the deliberate acknowledgement the exact-equality assertion exists to force —
# a streak is effort, not attainment, so it passes the forbidden-substring
# check below on its own merits rather than by exemption.
_EXPECTED_ROW_FIELDS = {"userId", "displayName", "xp", "rank", "streak"}
_EXPECTED_VIEWER_FIELDS = {"userId", "displayName", "xp", "rank", "streak"}


class TestNoMarkShapedFields:
    def test_row_dto_field_set_is_exactly_display_identity_xp_rank_and_streak(self) -> None:
        assert set(LeaderboardRowDTO.model_fields) == _EXPECTED_ROW_FIELDS

    def test_viewer_dto_field_set_is_exactly_display_identity_xp_rank_and_streak(self) -> None:
...
class TestStreakIsNullableOnBothDTOs:
    """D5.14 §2: `None` means "no ``streaks`` row", `0` means a broken streak.

    Pinned at the schema level as well as on the wire (``tests/
    test_web_leaderboard.py``) because the tempting "tidy" edit is a type
    change — ``int = 0`` — not a value change, and it would make the two facts
    unrepresentable rather than merely conflated.
    """

    def test_row_streak_admits_none(self) -> None:
        annotation = LeaderboardRowDTO.model_fields["streak"].annotation
        assert type(None) in getattr(annotation, "__args__", ())

    def test_viewer_streak_admits_none(self) -> None:
        annotation = LeaderboardViewerDTO.model_fields["streak"].annotation
        assert type(None) in getattr(annotation, "__args__", ())

    def test_a_zero_streak_is_representable_and_distinct_from_none(self) -> None:
        broken = LeaderboardRowDTO(userId="u1", displayName="Ada", xp=5, rank=1, streak=0)
        unknown = LeaderboardRowDTO(userId="u2", displayName="Bo", xp=5, rank=1, streak=None)
        assert broken.streak == 0
        assert unknown.streak is None
        assert broken.streak != unknown.streak
