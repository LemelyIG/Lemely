...
| 159 | `content-classified-practice-mental-model` | ui | high | C | C3 | done C3 | `npm run check:copy` (no em-dashes) — `PracticeGenerator.tsx`'s header is now a `SectionHead` whose `kicker` states the mental model in one sentence: "Questions come from past papers, your own marked papers, or Lemely's practice bank; pick a source or use all." |
