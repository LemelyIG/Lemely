...
def render_example_toml() -> str:
    ...
    lines.append("")
    lines.append("# US-005b's marking gate (US-040, closed by `08df9302`): read at all three")
    lines.append("# `correct_paper` entry points -- CLI, student upload, teacher grading job.")
    lines.append("# Enabling it CHANGES HOW PAPERS ARE MARKED. This branch ships no accuracy")
    lines.append("# evidence that the verdict path marks better than the legacy path -- that")
    lines.append("# evidence needs a label campaign that has not started. Not a safe default")
    lines.append("# to flip; leave commented out until that evidence exists.")
