...
def render_example_toml() -> str:
    ...
    lines.append("# Object storage for every upload the web app keeps (student scans, teacher")
    lines.append('# papers, mark-scheme PDFs). "local" (the default) writes under')
    lines.append('# `paths.output_dir/storage` — right for dev, Compose and CI. "gcs" talks to')
    lines.append("# Google Cloud Storage via application-default credentials — the Cloud Run")
    lines.append("# runtime service account in staging/production, set by LEMELY_STORAGE__BACKEND")
    lines.append("# in the deploy, not here.")
    lines.append(f'backend = "{s.storage.backend}"')
    lines.append('# Bucket name for "gcs", directory name for "local". The gcs bucket is')
    lines.append("# created by `scripts/gcp-bootstrap.sh`, one per environment — not by this app.")
