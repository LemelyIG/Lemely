...
//
// Without this, accuracy-gate-triage does not fail; it runs on defaults.
// Its guard would fire on the missing gate and log_path, so a red CI run routed
// here would come straight back as missing-args instead of being triaged.
// Double-encoding is handled too (a string that parses to another string).
{
  ...
  while (typeof args === 'string' && depth < 3) {
    ...
    try {
      ...
      log(`accuracy-gate-triage: args arrived as a string that is not JSON (${String((e && e.message) || e)}): ${raw.slice(0, 200)}`)
  ...
  if (typeof args !== 'object' || Array.isArray(args)) {
    ...
    log(`accuracy-gate-triage: args must be an object, got ${shape}. Call as Workflow({ name: 'accuracy-gate-triage', args: { ... } }).`)
