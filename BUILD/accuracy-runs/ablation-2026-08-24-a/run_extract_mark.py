"""Driver for the extract+mark arm of run_label=ablation-2026-08-24-a.

Runs the full golden corpus through the harness's extract+mark pipeline
(extraction then marking, cache_mode='bypass'), capturing per-call Gemini
usage tokens via a structlog capture processor, re-costing at corrected GA
pricing ($0.30/1M in, $2.50/1M out, thoughts counted as output), and writing
one EvalRecord-jsonl file plus a manifest. Stops early (completed=false) if
cumulative re-costed spend reaches 150% of the preflight estimate
($0.18 -> stop at $0.27).
"""
...
WORKDIR = REPO / "BUILD/accuracy-runs/ablation-2026-08-24-a"
RUN_LABEL = "ablation-2026-08-24-a"
ARM = "extract+mark"
PREFLIGHT_USD = 0.18
STOP_USD = PREFLIGHT_USD * 1.5  # 150% of preflight estimate
...
def main() -> None:
    ...
    corpus_listing = sorted(
        p.name for p in golden_dir.iterdir() if p.is_dir() and p.name != "results"
    )
    ...
    run_id = f"{RUN_LABEL}-{ARM}-{run_id_ts}"
    ...
    while attempt < max_attempts:
        ...
        try:
            ...
            anomalies.append(f"{ARM} attempt {attempt}: EXCEPTION {exc!r}")
            result = None
            break
        ...
        if cache_hit_events:
            ...
            anomalies.append(
                f"{ARM} attempt {attempt}: {len(cache_hit_events)} cache HIT(s) "
            ...
            if attempt < max_attempts:
                ...
                anomalies.append(
                    f"{ARM}: cache HIT again on retry -- run invalid, aborting"
                )
                completed = False
                result = None
        break
    ...
    if result is not None:
        ...
        manifest_data["_recosted_usd_corrected_pricing"] = round(recosted_usd, 6)
        ...
        summary = {
            ...
            "recosted_usd": round(recosted_usd, 6),
        ...
        print(
            f"[{ARM}] calls={n_calls} in={sum_in} out(incl thoughts)={sum_out_total} "
            f"recosted_usd={recosted_usd:.4f} mark_accuracy={result.metrics.mark_accuracy:.4f}",
            file=sys.stderr,
        )
    else:
        completed = False
    ...
    top_manifest = {
        ...
        "preflight_estimate_usd": PREFLIGHT_USD,
    ...
    print(
        json.dumps(
            {
                "completed": completed,
                "cumulative_recosted_usd": cumulative_recosted_usd,
                "anomalies": anomalies,
            }
        )
    )
