...
branch: chore/accuracy-run-39-close (worktree HEAD) -> needs PR; feature/accuracy-93-... @ 40a33d1 -> PR #128 OPEN; feature/accuracy-41-... @ 09314c7, feature/accuracy-37-... @ 8f3ba7f pushed, no PRs
last_run_label: run-39-2026-08-26
last_run_headline: ZERO SPEND, no Gemini calls. *** THE PROGRAMME IS UNBLOCKED: the human answered ASKS B1-B17 on 2026-08-26T14:12 (two inbox items, both UNFLIPPED and unactioned except B2/B7). BINDING SEQUENCING: #93 (B7 folded in) lands FIRST, then regenerate+commit the parsed corpus recording the parser SHA, then #88 items 1-2 and 4, then #57's costed preflight. *** #121 LANDED (PR #126 -> 31b7c57, five CI jobs green): test_cli_doctor.py's raw unrestored 'del os.environ' was the ROOT CAUSE of #120 and was still leaking into every module. Review caught that my regression test could be POISONED BY ITS OWN BUG — it checked GEMINI_API_KEY only 'if original is not None', and a partial-restore regression deletes the ambient key BEFORE the test captures it, so the assertion silently skipped and all six tests passed under the mutation. Now asserts against a synthetic sentinel it plants itself; re-running the same mutation FAILS. *** #93 PR #128 OPEN, CI RUNNING, NOT MERGED. B2 waiver recorded on the issue with its basis (harness never invokes the det parser; golden set has zero 0625 p2 cases -> null by construction twice over). B7 folded in: 0625 p3 THEORY_EXTENDED -> THEORY_CORE. Review MUST-FIX caught and fixed: the B7 constant fix silently made test_0625_p3_core_extended_is_not_flagged VACUOUS (mapped == implied -> classify returns at the EARLIER guard, so _changes_parse_path is never reached) and that test was its ONLY coverage; re-pointed at a synthetic profile and PROVEN BY MUTATION.
...
spend_usd: 2.704228
in_the_middle_of: RUN 39 — NOTHING RUNNING, NO GEMINI CALLS, NO SPEND. *** PR #128 (#93) IS OPEN AND NEEDS LANDING: base develop, branch feature/accuracy-93-profiles-paper-type-lookup-order @ 40a33d1. CI was RUNNING at run end — poll 'gh pr checks 128', merge once green, route red to accuracy-gate-triage. Adversarial review ALREADY DONE and its MUST-FIX already fixed in 40a33d1, so it needs CI only. *** THEN, IN THIS EXACT ORDER (the human's binding sequencing of 2026-08-26): (1) regenerate the parsed corpus det-only and COMMIT it, RECORDING THE PARSER SHA — ask B8 says immediately after #93 lands, NOT at 250 now; ~40 min, $0.00; expect ~289 schemes not 250 because #93 fixes 41 that currently ParseError. (2) #88 items 1-2 and 4 (item 1 IS #93, now done): parse the det failure set for STRATUM COVERAGE (est $4.88-6.83, RE-COST after #93), and raise per_run_token_ceiling (lemely.toml:19) deliberately, recording the raise in DECISIONS.md. (3) #57's costed preflight. *** ALSO OPEN AND UNPUSHED-AS-PR: this branch chore/accuracy-run-39-close carries the human's B1-B17 rulings commit (337a60b) plus this state — open its PR. *** THE TWO INBOX ITEMS OF 2026-08-26T14:12 ARE STILL '- [ ]' AND MUST BE FLIPPED WITH NOTES as each ruling is acted on; only B2 and B7 are done so far. B1/B3/B4/B5/B6/B9/B10/B12/B13/B15/B16/B17 are ANSWERED AND UNACTIONED. B17 option 3 (give accuracy_board.py a done path that closes a non-H off-board issue, H-guard by issue number/label not board membership) then CLOSE #114/#120/#121/#122/#124 through it — they are all landed-but-open. *** SECURITY RULING: DO NOT ROTATE the key; local-only exposure accepted. STANDING RULE: never echo the key value into a transcript again, and it must never reach a committed file, a CI log, or any public artefact — this repo is PUBLIC. *** NEW ISSUE #127 (opened, NOT executed): golden fixture 0625_w21_qp_32_theory_nested records theory_extended for a Core paper; correcting it is gate movement and needs a decision — three options on the issue, recommend folding into #95. *** DO NOT RE-DERIVE: #120's cause (env_settings outranks init data; model_dump->model_validate re-triggers the merge; model_copy does not); B7's inertness (39/39 distinct local 0625 p3 schemes resolve theory_core from cover text, ZERO fallback); rows.py flush()/q_row_had_answer (falsified). *** pytest-randomly is NOT installed, so '-p no:randomly' is a no-op — randomised-order isolation is UNVERIFIED, do not claim it. *** FOUR OPEN H ISSUES #49/#51/#52/#55 — B14 says still owed by the human; #49 keeps the split NOT FROZEN. Board Ready column EMPTY.
---
