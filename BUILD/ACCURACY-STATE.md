...
last_run_headline: #58 CONTROL ARM (inbox item 5): the same-input control scored the SAME 31 leaves the reorder arm scored and found 0/31 differing; a duplicate concurrent copy independently scored 0/31 again, so 0/62 combined, Wilson 95% CI [0.0%, 5.8%], 107 calls, cache_hit=False on every one. THE DIRECTIVE'S PREMISE IS FALSIFIED: there is no 0.1565 to subtract. Under p=0.1565 we expect 4.9 differing leaves per 31; we observed 0 across 62, probability 2.6e-05. The 0.1565 was measured END TO END (extract+mark, transcription included); this design feeds ground-truth answers straight in with NO extraction step, so it measures MARKING-ONLY churn — a different quantity. BULLET 4 STAYS UNTICKED, but not for the expected reason: the control removes the 'it is just gemini churn' explanation, yet reorder 1/31 against control 0/62 is Fisher exact two-sided p=1.000, so the design cannot call the violation a real reorder defect either. UNDERPOWERED IN BOTH DIRECTIONS. The settling experiment is re-marking q11b ALONE perturbed vs unperturbed ~10x each (~$0.01, one leaf) — a DIFFERENT design from the one authorised, so it is proposed on #58 and NOT run. COST + MISTAKE: $0.287153 where one run costs ~$0.06, because the control RAN TWICE CONCURRENTLY (pgrep reported the background copy gone and report.json was absent, so it was recorded dead; it was not). Inside the ~$0.28 authorised, but by accident. report.json's spend fields and run.log's usd_cost sum are BOTH wrong (two writers, one ledger); see NOTES.md.
...
spend_usd: 2.363182
in_the_middle_of: RUN 23 — NOTHING IS RUNNING IN THE BACKGROUND. *** PR #90 IS THE ONE OPEN ACTION: base develop, tip bd6cd72, MERGEABLE / CLEAN (the BUILD/ACCURACY-STATE.md conflict is resolved and pushed). CI HAS NOT STARTED FOR THIS TIP — 'no checks reported' as of run end, because the final bookkeeping push reset it; the earlier green (run 32863487591, pre-commit + web PASS, three test jobs pending) was against 240058a/72174ea and does NOT cover bd6cd72. LAND IT with accuracy-pr-land once all five jobs are green on bd6cd72; do NOT merge on the stale run. PR #90 says 'Closes nothing', so landing it does NOT close #58 with bullet 4 unmet — checked, not assumed. *** #58 BULLET 4 STAYS UNTICKED and the human has been told why on the issue: the authorised control arm ran and FALSIFIED its own premise (marking-only churn 0/62, not 0.1565 — that figure is end-to-end and does not transfer), but reorder 1/31 vs control 0/62 is Fisher p=1.000, so the violation is unresolved in BOTH directions. DO NOT re-run the sweep (MISSION 12.9). The settling experiment — re-mark q11b ALONE perturbed vs unperturbed ~10x each, ~$0.01 — is a DIFFERENT design from the one authorised and is PROPOSED ON #58, NOT RUN. It needs the human's word first. *** OPEN QUESTION POSTED TO THE HUMAN (inbox + #58), item 6 of the 17:36 directive: may whitespace golden fixtures be added as ordinary golden cases, joining the accuracy denominator (MISSION 12.2/12.5, and #49 is reopened with the split NOT frozen), or as a metamorphic-only fixture set load_golden_cases does not serve? I recommend the second. NOT implemented pending the answer. *** DONE THIS RUN: item 5 (control arm), item 7 (ledger — DA11: ledgers are per-worktree gitignored state, four exist, header now means the programme-wide SUM 2.363182), item 8 (BLOCKERS RESOLVED blocks for #36/#40/#28, #40 stub de-duplicated, accuracy_board block made idempotent with 3 tests proven to fail on the unfixed code). *** NOT DONE, still the human's: items 1-4 of the 17:36 directive (#88 — profiles.py:50 fix FIRST at zero spend, then the sequenced parse spend, the ratified 9,464 leaf count, the token-ceiling raise), plus the older A4 (#45 PR), A2 (#93), A5 (#37 sweep), and the #59 handwriting corpus. *** SUPERVISOR VERDICT COVERAGE: the 18:01 PASS covers db7c9f4 only, NOT the tip. *** UNPUSHED: chore/accuracy-post-45-bookkeeping holds run-21's E4 remedy (3 commits, local only).
---
