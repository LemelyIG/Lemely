...
branch: chore/accuracy-run-32-bookkeeping (worktree HEAD); feature/accuracy-41-... @ 09314c7, feature/accuracy-37-... @ 8f3ba7f, feature/accuracy-93-... @ 70a6957 all pushed, no PRs
last_run_label: run-32-2026-08-26
last_run_headline: NO SPEND (ledger unmoved at 2.659533). PR #104 and PR #106 MERGED. #105 CLOSED and Done — it hardens two figures that DO NOT EXIST YET, which is the only cheap time: once H7 publishes an agreement number, changing how its denominator is reported is a moved-target problem. (1) MISSION 9 GATE 7 WAS NOT ACTUALLY SATISFIED by #98: agreement_wilson named its exclusion in a DOCSTRING and returned a bare (n, successes, point, lower, upper). A docstring does not travel with the published figure — a reader handed n=28, point=0.86 could not tell whether 2 leaves were excluded or 200, the difference between a sound 10% sample and a silently narrowed one. It now returns a_only/b_only. DA2's 'A's label stands' property is preserved because the counts are aggregates, and the guarding test now asserts the PROPERTY (every field an aggregate scalar) rather than a frozen key list. (2) DA3 AND DA5 WERE PROSE WITH NO CALLER: count_pending() and list_orphan_resolutions() shipped in #98 and NOTHING consulted either, so 'pending tail at zero before the irreversible split freeze' and '#52's sweep before #51's sample' were decisions a future run could walk straight past — the same shape as #49 reaching CLOSED with an unmet box. assert_rulings_settled() now raises unless the tail is zero AND there are no orphans, and agreement_wilson takes rulings_settled as a REQUIRED keyword with no default, so omitting it is a TypeError rather than a silent pass. accuracy-review returned MERGE with ZERO findings; its test-honesty reviewer independently overlaid all 14 new/modified tests onto pre-fix develop in a throwaway worktree and confirmed every one fails there.
...
spend_usd: 2.659533
in_the_middle_of: RUN 32 — NOTHING RUNNING, NO SPEND (ledger sum 2.659533 of $25.00). PR #104 and PR #106 MERGED (8b81c2b, c6e7c28); #105 CLOSED and Done. *** PR #107 IS OPEN AND NEEDS LANDING: base develop, branch chore/accuracy-run-32-bookkeeping, BUILD/-ONLY. CI was still running at run end — poll 'gh pr checks 107' and merge once green. *** NOTE FOR THE NEXT RUN: accuracy-pr-land FAILED on a HARNESS error (a subagent finished without calling StructuredOutput), NOT a gate failure. It had already opened PR #106 before dying; CI was watched and merged BY HAND against the same 7.1 conjunction (clean accuracy-review verdict wf_4bec09c8-1d6 = merge/zero findings, board's own branch name, base develop, all five CI jobs green). If it fails that way again, check 'gh pr list --head <branch>' FIRST — the PR may already exist and opening a second is the real hazard. *** THE PROGRAMME IS NOW BLOCKED ON THE HUMAN ON EVERY REMAINING FRONT. Nothing in the inbox is actionable without a ruling: #41 (the 12.7 A-mark call — 0 of 36 principle strings mention M/A dependency, so bullet 2 is a behavioural no-op; branch 09314c7 held UNMERGED on purpose because the VERSION bump costs nothing until merge and would need a second bump after the real fix; DO NOT CLOSE #41 or the sweep pre-authorisation lapses); #37's two questions (bullet 3's CI target cannot be re-derived from a corpus scoring id_match_rate 1.0 in BOTH arms); #93's sweep waiver (must land before any #57 preflight and before the split freeze — and #88 items 1-2/4 and the PARSED CORPUS regeneration are all sequenced behind it); item 6 whitespace fixtures (I recommend a metamorphic-only set); #58's q11b experiment; the 0625 paper-3 constant; and the THREE #98 RULINGS (stratification axis, DA2 hash-formula deviation, cleartext-salt claim). *** ONE THING IS AUTHORISED BUT I DID NOT START IT: #95's fixture rebuild (execution authorised 2026-08-25T14:37:23, its #27 A/A-floor disposition already decided and posted as superseded-and-not-replaced). I held it because rebuilding golden fixtures through the det parser BEFORE #93 lands would bake #93's known paper_type bug into a corpus MISSION 12.2 calls irreversible — two 0625 paper-3 fixtures are affected. That is the same ordering argument the human accepted for the PARSED CORPUS commit, applied to a case they have not been asked about; it needs their word, and is the first thing to raise. *** #58 BULLET 4 STILL CORRECTLY UNTICKED (Fisher p=1.000). FOUR OPEN H ISSUES: #49, #51, #52, #55.
---
