...
spend_usd: 1.488057
in_the_middle_of: CLOSED THE LAST LOOSE END FROM THE 0625 p2 COUNTERFACTUAL, free and offline. NO SPEND; ledger unmoved at 1.488057. NO new inbox items and NO human answers -- 4th consecutive run with none; every remaining item is gated on a decision or on spend. *** 0625_s24_ms_21's REAL CAUSE, by direct observation not arithmetic: CAIE WITHDREW question 14, so its answer cell holds the literal label 'Question Discounted' instead of a letter -- legitimate scheme content, table extracts fine. find_mcq_answer_col (lemely/io/det/mcq.py:23) requires ALL non-empty values in a candidate column to be in {A,B,C,D}, so ONE non-letter cell disqualifies the answer column; no other column qualifies (marks column is all '1'), it returns None, and parse_mcq_tables (mcq.py:41-42) CONTINUES PAST THE ENTIRE TABLE. Measured: table 0 = 29 rows/answer_col=None/1 discounted cell; table 1 = 13 rows/answer_col=3/0. Parsed 12 -> 40 when that single cell is neutralised = 28 QUESTIONS LOST TO ONE CELL, silently. Worth not rediscovering: parse_mcq_tables:50 ALREADY has a per-row guard that would have skipped that row harmlessly -- the all-or-nothing column detector fires first and costs the other 27. *** BLAST RADIUS 1 of 479, and a FALSE GROUPING I caught in my own analysis: grepping 'discounted' matches 3 schemes, but in 0606_s22_ms_13 and 0606_s24_ms_11 it is ordinary maths marking prose ('negative root discounted') with no withdrawn question. Checked in context rather than reporting the grep count. Do NOT re-report 3. *** BEARING ON #45's ESCALATION: a worked example that option 3 (instrument the parser) pays -- this cause is INVISIBLE to aggregate arithmetic (computed_total vs maximum_mark cannot tell 'question withdrawn' from any other shortfall) yet unambiguous on direct observation, and NO bucket names it. Does NOT choose between options 1/2/3; that stays the human's. *** mcq.py UNTOUCHED, diff empty. Relaxing the detector (obvious shape: strong majority not all, since the per-row guard handles stragglers) changes det parse output and so awarded marks = gate-9 mark-changing = human's call, same reason profiles.py:50 was left alone. Artifact BUILD/accuracy-runs/mechanism-0625-s24-ms-21-2026-08-24/ with script; no answer letters or scheme text committed (12.7). *** THREE HUMAN ASKS STILL OPEN, all pure decisions, nothing technical behind any of them: (1) #58 bullet 4 -- authorise ~USD 0.144 or formally RETIRE it as was done for #40's; PR #90 OPEN, MERGEABLE, green on all 5 jobs at 10b5ec5, reviewed, 2 defects found and fixed. (2) #88 q1/q2/q3 -- q2 now fully costed and measured (fix moves 39 schemes, paid set 229 -> 190, -17.0%); q1/q3 are DA1/H4 (#49) territory, MISSION 3.5 forbids working around them. (3) #45's 3-option design question. *** develop = 1092d8f, local ahead=0; both feature branches pushed, 0 unsigned. ALWAYS REBASE WITH -S (commit.gpgsign unset here). Do not prune local branch chore/accuracy-build-records-2026-08-24. Nothing running in the background.
---
