...
run_pointer: run-2026-08-29-c
worktree: /home/sico/Lemely-worktrees/accuracy
branch: fix/accuracy-110-phantom-leaves (this)
last_run_label: phantom-leaves-110-2026-08-29
last_run_headline: RUN 63 — #110's PHANTOM LEAVES PRUNED; THE GOLDEN CORPUS IS DUPLICATE-FREE AND #95 IS CLEANLY UNBLOCKED. Zero spend. *** I had flagged on #95 that regenerating would bake collapsed leaves into the fixtures. Checking WHICH schemes were affected made the decision unnecessary: FOUR OF FIVE were already clean, and 0580_s23_ms_22's two collisions (ids 1 and 2) were each ONE REAL QUESTION PLUS ONE EMPTY SHELL — the data-table rows #136 mechanism (C) had already stripped of minted marks. *** DA39 declined to fold stray rows into earlier questions and that stands; but DROPPING A NODE WITH NO MARKS AND NO ANSWER POINTS IS NOT FOLDING ANYTHING. It removes something that contributes to no total, cannot be matched against an answer, and cannot be marked, while corrupting DA6 identity. *** A TEST CAUGHT THE RULE BEING TOO WIDE: the first version pruned labelled sub-parts like 1(b), which are structure the paper PRINTED. Narrowed to TOP-LEVEL leaves only. *** MEASURED over 479 schemes: exact 331 -> 331 UNCHANGED, parsed marks 32849 -> 32849 UNCHANGED (the check that matters — pruning a zero-mark leaf cannot move a mark, confirmed not assumed), leaves 17043 -> 16985, leaves lost to collapse 36 -> 25, schemes with dups 21 -> 15. *** SECOND FINDING #110 WAS NOT OPENED ABOUT: 58 phantom leaves removed, only 11 involved in a collision. THE OTHER 47 WERE INFLATING THE LEAF COUNT — and therefore the DENOMINATOR of every per-leaf rate the programme computes. A defect that adds noise to a denominator is far harder to notice than one that collides two ids, because nothing ever disagrees. *** GOLDEN: 0580_s23_ms_22 dup_ids 2 -> 0, leaves 37 -> 35, still 70/70. ALL FIVE of #95's schemes are duplicate-free. *** 15 schemes with 25 collapsed leaves REMAIN UNREPAIRED — rows with real content under a colliding id; what they ARE is not readable off the page. DA41. Cumulative DA39+DA41: leaves lost 57 -> 36 -> 25, schemes 34 -> 21 -> 15.
