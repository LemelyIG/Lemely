...
branch: chore/accuracy-run-27-bookkeeping (worktree HEAD); feature/accuracy-37-delete-the-positional-fallback @ 6cecf50 and feature/accuracy-93-... @ 70a6957 are pushed, no PRs
last_run_label: sweep37-2026-08-25
last_run_headline: #37 GATE-9 SWEEP RAN AND ABORTED ITSELF — NOT REPORTABLE, reason is COVERAGE not significance (MISSION 12.9 success). The in-process stop-and-ask tripped at $0.282324 against its $0.25 threshold after 2 of ~5 distinct papers (39 leaves). THE BRAKE WORKED — #58 overran 1.94x with nothing watching; this checked after every case. MY PREFLIGHT WAS WRONG BY ~4-5x: I costed $0.0625/pass from ablation-2026-08-24-a's recosted arm figure, but the real rate is ~$0.047/CASE, so 6 cases cost $0.282 where I projected $0.125 for eleven; I read a recosted aggregate as a per-pass rate without checking coverage. Full coverage would now cost ~$0.52 — inside the $20 ceiling but 4x what the human was shown, so A5 condition 2 fires and it is NOT re-run. WHAT IT ESTABLISHED: id_positional_fallback fired ZERO times across all 39 leaves; id_match_rate 1.0 in BOTH arms; mark correct 33 old / 32 new; McNemar b=1 c=0. THE SUBSTANTIVE FINDING: bullet 3's CI target CANNOT be re-derived from this corpus — an id_match_rate of 1.0 in both arms says nothing about genuine agreement under extraction drift, and re-deriving 0.99 from a corpus that scores 1.0 either way would be PICKING a number, not measuring one. The single mark difference (0606 4b partial, 1->0 vs truth 1) is PROVABLY not the change: fallback_fires=0 means both arms got byte-identical answers, so it can only be marking nondeterminism — the shared-extraction design isolating the change well enough that the remainder is visibly noise.
...
spend_usd: 2.659533
in_the_middle_of: RUN 28 — NOTHING RUNNING. SPENT $0.282324 this run on the #37 sweep (authorised, preflight posted first); ledger sum 2.659533 of the $25.00 ceiling. *** #37 IS THE LIVE WORK AND IS NOW BLOCKED ON TWO QUESTIONS POSTED TO THE ISSUE. Branch feature/accuracy-37-delete-the-positional-fallback @ 8f3ba7f, PUSHED, NO PR. Bullet 1 DONE (fallback deleted, tests inverted and proven fail-before/pass-after). Bullet 2 PARTIAL ('exact' now true by construction; 'fuzzy' still never emitted — needs raw-vs-canonical provenance out of extract_answers, touching 3 web call sites). Bullet 3 BLOCKED, see below. *** THE SWEEP RAN AND ABORTED ITSELF: in-process stop-and-ask tripped at $0.282324 vs $0.25 after 2 of ~5 distinct papers (39 leaves). NOT REPORTABLE — coverage, not significance (MISSION 12.9 success). DO NOT re-run without an answer: A5 condition 2 fires because MY PREFLIGHT WAS WRONG BY ~4-5x (I read ablation-2026-08-24-a's recosted aggregate as a per-pass rate; real rate ~$0.047/CASE, so full coverage is ~$0.52, not $0.125). *** THE SUBSTANTIVE FINDING, which reframes the issue: id_positional_fallback fired ZERO times across all 39 leaves and id_match_rate is 1.0 in BOTH arms. So BULLET 3'S CI TARGET CANNOT BE RE-DERIVED FROM THIS CORPUS at any spend — a corpus that scores 1.0 either way says nothing about genuine agreement under extraction drift, and re-deriving 0.99 from it would be PICKING a number, not measuring one. Asked the human to choose: leave 0.99 untouched with a recorded limitation (my lean), or open a follow-up for a corpus that can actually exercise id drift. That is a target-setting call I must not make alone. *** The single mark difference (0606 4b partial, 1->0 vs truth 1) is PROVABLY not the change — fallback_fires=0 means both arms got byte-identical answers, so it is marking nondeterminism. The shared-extraction design (extract ONCE, both normalisers over the same raw) is what proves it, and it is worth reusing for any future paired A/B on a gemini path. *** ALSO OBSERVED, not chased: a gemini_validation_failure on 0606 — the model emitted a JSON string that ran away into thousands of literal newline escapes and truncated at column 66172. Real robustness signal, out of #37's scope. *** NOTE FOR THE NEXT RUN: DNS FAILED for part of run 27 (github.com and generativelanguage.googleapis.com both unresolvable, raw IP reachable) and then recovered. If it fails again, the local-only work is real: write scripts, run pure-logic checks offline, commit, and record what could not be pushed. *** #93 STILL WAITS ONLY ON THE HUMAN'S WAIVER: branch feature/accuracy-93-... @ 70a6957 pushed, no PR, conditions 1 and 3 satisfied, sweep null by construction twice over. Must land BEFORE any #57 preflight and BEFORE the split freeze. *** ALSO STILL THE HUMAN'S: the 0625 paper-3 constant (separate issue?); item 6 whitespace fixtures (I recommend a metamorphic-only set); #58's q11b experiment (~$0.01); PARSED CORPUS regeneration (artifacts do not exist — 33 files not 250, no parse.log; must wait for #93); 17:36 items 2-4 (#88, sequenced behind #93); #59 decisions 1-2. *** #58 BULLET 4 STILL CORRECTLY UNTICKED (Fisher p=1.000; DO NOT re-run, MISSION 12.9).
---
