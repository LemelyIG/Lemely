...
branch: chore/accuracy-run-31-bookkeeping (worktree HEAD); feature/accuracy-41-... @ 09314c7, feature/accuracy-37-... @ 8f3ba7f, feature/accuracy-93-... @ 70a6957 all pushed, no PRs
last_run_label: run-31-2026-08-26
last_run_headline: NO SPEND (ledger unmoved at 2.659533). PR #103 MERGED. #41 (M1.6) IMPLEMENTED BUT DELIBERATELY NOT LANDED — A13's premise does not hold and the fix needs a MISSION 12.7 ruling I must not make. MEASURED: the injected-principles branch ends 'if, and only if, these principles are silent on M/A dependency ... treat the A mark as dependent on its preceding M mark', and 0 OF 36 parsed principle strings across the golden corpus mention M/A dependency at all (no match for 'M mark', 'A mark', 'accuracy mark', 'method mark', 'depend'). So the silent condition holds for EVERY mark point of EVERY paper: the blanket rule stays operative for 100% of the corpus and, on the A-mark axis, the diff does exactly what VERSION 4 did while spending a bump that invalidates the whole cached corpus. NOT AN IMPLEMENTATION BUG — CAIE Generic Marking Principles govern awarding fairly and consistently; mark-code semantics are simply not written there, so no better parsing surfaces them. THE INFORMATION IS IN THE CORPUS, in per-answer-point math_mark_type: 57 M / 41 A / 39 B / 24 untyped over 161 answer points in 7 of 11 fixtures, and loose_schemas.py:83 already defines A as 'Accuracy mark (dependent on preceding M)'. CORRECTED MY OWN ERROR BEFORE REPORTING: a first pass counted 'mark_points' and found zero typed points; the field is 'answer_points'. THREE CASES, and the ruling covers only two — principles-parsed-and-they-speak-to-M/A NEVER happens (0/36); principles-not-parsed happens for 5 fixtures; principles-parsed-but-SILENT is 100% of GMP-carrying papers and A13 does not address it. Any default there moves 41 A-marks, so it is examiner judgment. Recommended option 3 (drive from math_mark_type, the paper's own printed mark codes) but did NOT adopt it. NO SWEEP RUN and no preflight posted: spending real money to measure a guaranteed null result is not a measurement.
...
spend_usd: 2.659533
in_the_middle_of: RUN 31 — NOTHING RUNNING, NO SPEND (ledger sum 2.659533 of $25.00). PR #103 MERGED (770c5a7). *** #41 IS THE LIVE WORK AND IS BLOCKED ON A HUMAN RULING POSTED TO THE ISSUE. Branch feature/accuracy-41-inject-the-caie-generic-marking @ 09314c7, ONE COMMIT, PUSHED, NO PR — deliberately not landed. Bullet 1 (inject the paper's GMP into the marker prompt) is real and works. Bullet 2 is a BEHAVIOURAL NO-OP: 0 of 36 parsed principle strings mention M/A dependency, so the prompt's 'if and only if these principles are silent' escape clause fires for 100% of papers and the blanket no-M-means-no-A rule stays operative. THE QUESTION ASKED: which rule governs the silent case? Options posted on #41 — (1) strict M-then-A, stated honestly rather than dressed as principle-driven; (2) no unconditional default, justify from principles plus dep/ft/ecf annotations; (3) DRIVE FROM math_mark_type, the paper's own printed mark codes, which is where dependency actually is (57 M / 41 A over 161 answer points, 7 of 11 fixtures) — I RECOMMEND 3 but did not adopt it, because any default moves 41 A-marks and that is MISSION 12.7 examiner judgment. *** HOLD THE BRANCH UNMERGED until that ruling: the VERSION 4->5 bump invalidates the entire cached golden corpus on merge and has cost nothing while unmerged, so landing a no-op now would spend the bump and require a second one after the fix. The 7 batching constraint still holds (only correction_ai.py's VERSION moved; answer_extraction.py and mark_scheme_parsing.py untouched) and neither #37 nor #93 touches any prompt file. *** DO NOT CLOSE #41 BEFORE ITS SWEEP RUNS — the gate-9 pre-authorisation is scoped to the issue's OPEN lifetime (#28 general rule) and lapses if it closes unswept. No sweep was run and no preflight posted, on purpose: spending real money to measure a guaranteed null result is not a measurement. *** 8 SEQUENCING STILL BINDING: #41 is mark-RAISING and #38/#39 are mark-LOWERING; they must not land together. Both are unimplemented, so #41 alone is clean. *** PR #104 IS OPEN AND NEEDS LANDING: base develop, branch chore/accuracy-run-31-bookkeeping, BUILD/-ONLY. CI was still running at run end — poll 'gh pr checks 104' and merge once green. *** STILL BLOCKED ON THE HUMAN, unchanged: #37's two questions; #93's sweep waiver (must land before any #57 preflight and before the split freeze); item 6 whitespace fixtures; #58's q11b experiment; the 0625 paper-3 constant; PARSED CORPUS regeneration; and the THREE #98 RULINGS (stratification axis, DA2 hash-formula deviation, cleartext-salt claim). *** FOUR OPEN H ISSUES: #49, #51, #52, #55.
---
