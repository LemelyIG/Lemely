...
run_pointer: run-2026-08-29-h
worktree: /home/sico/Lemely-worktrees/accuracy
branch: measure/accuracy-38-clean-defaulted (this)
last_run_label: defaulted-38-2026-08-29
last_run_headline: RUN 68 — C2's DEFERRAL CAME DUE: THE CLEAN DEFAULTED-MARK RATE, ZERO SPEND. *** C2 deferred #38 bullets 2-3 behind #136's fix and required the trigger decided against a CLEAN rate. #136 landed, so the deferral is discharged. Both arms over the SAME 397 papers; the before arm imports a package copy with rows.py/marks.py from the commit before #136. *** CONTAMINATED -> CLEAN: papers with any defaulted point 177 (44.58%) -> 136 (34.26%); papers where EVERY point defaulted 25 -> 3; points defaulted 4928 (22.05%) -> 2000 (8.94%). THE BEFORE ARM REPRODUCES THE PUBLISHED FIGURES (44.58 vs 44.4, 22.05 vs 21.6 — the published ones came from a 60-PDF sample, these are all 397), which is what makes the after arm COMPARABLE rather than merely different. *** C2'S PREMISE WAS RIGHT AND THE CONTAMINATION WAS MOST OF THE SIGNAL: mechanism (B) accounted for 2928 of 4928 defaulted points = 59%. The sharpest movement is the number nobody quoted — papers where EVERY point defaulted, 25 -> 3. Those were papers whose entire marks column had merged into the answer text: the flag was telling the truth, the cause was the parser. *** THE TRIGGER, which is what C2 asked for: a BARE defaulted-count is STILL WRONG at 34.26% — most defaults remain correct BY LUCK (1 is right for every B1/M1/A1/C1) and routing a third of the corpus to a fallback DA35 measured failing ~50% is not an improvement. WHAT THE CLEAN RATE MAKES AFFORDABLE FOR THE FIRST TIME: a trigger on 'EVERY point in the paper is defaulted' — now 3 of 397 (0.76%), down from 25. PROPOSED, NOT ARMED: it feeds the same fallback as #110's and #39's detectors, so all three are ONE cost-and-coverage decision and it is the HUMAN's. BULLETS 2 AND 3 STAY UNTICKED — this supplies the clean rate C2 required BEFORE the decision, not the decision. *** BY-PRODUCT OF THE CLAUSE WALK: checking clause 2's 'published' requirement found TWO SUPERSEDED figures in ACCURACY-REPORT.md — the ratchet row still published 29.03% as the committed constant (DA33 replaced it with 0.4838 the day before), and the det-defect section still published the contaminated 44.4%/21.6%. BOTH CORRECTED. A REPORT CITING A CONSTANT ITS OWN DECISION RECORD HAS RETIRED IS NOT A DOCUMENTATION LAG. DA46.
...
in_the_middle_of: RUN 67 — branch chore/accuracy-s13-clause1-and-5 carries DA45, the journal entry and the MISSION figure corrections. Open its PR to develop and merge on green. Cut the next branch from origin/develop AFTER it merges. *** SECTION 13 STATUS AFTER THE CLAUSE WALK: clause 1 NOT MET (#38, #39, #41, #57, #58, #59, #88, #95, #110, #127, #136, #161 open — #112 CLOSED this run); clause 2 PARTIAL (published; ratchet NOT armed, and DA33 measured that restating it does not change that); clause 3 MET (the NOT_APPLICABLE verdict and its reason are published; the 2x2 waits for #57 and its spend authorisation LAPSED when #28 closed); clause 4 NOT MET (~0 of ~300 labelled leaves; the two-seat protocol requirement IS met, DA40); clause 5 NOW ACCURATE — det parse coverage 331 of 479 measured and the MISSION corrected in four places; Gemini marking accuracy still UNMEASURED in its own terms; clause 6 NOT ASSESSABLE (ratchet unarmed); clause 7 MET (PR #159 open, body refreshed); clause 8 MET (all four H issues now current — #49 and #55 updated this run). *** SO CLAUSES 3, 7, 8 ARE MET, 5 IS HALF MET, AND 1/2/4/6 NEED THE HUMAN. Clause 4 is the structural one: ~300 HUMAN-labelled leaves. *** FIVE THINGS NEED THE HUMAN: #41's $0.30 sweep (code merged, bullet 4 unmet until it runs); #166's fix (BOTH halves or it moves the rate without closing the gap; and whether 'diagnosed' satisfies C25 condition 1 is YOUR call); #52's ruling; H7's overlap share (DEADLINE — before #47 completes); and THE THREE UNARMED DETECTORS (#38 44.4% of papers, #110 15 schemes, #39 4 questions) which route to the same failing fallback and want ONE decision. Plus #95's three options (DA42). *** PR #159 (develop -> main) IS OPEN — DO NOT MERGE, MISSION 12.3 is human-only. *** TRAP IN DA44 for whoever edits lemely/io/correction_ai.py: the broad except Exception around ai.mark_question turns a PROGRAMMING error into ai_marking_failed and awards 0 to every question — no crash, no traceback. *** LESSONS: WALK THE CLAUSES — 'cannot be completed' is a claim about a whole and a whole is not a work item; MEASURE BEFORE IMPLEMENTING A GATE; re-run a feasibility probe before acting on its verdict; when the cheapest option is the one you want, that is the reason not to take it; do not close an issue on a generous reading of your own work; when a fix removes a ONE-DIRECTIONAL error the metric it was inflating GETS WORSE first; USE QUOTED HEREDOCS for anything containing backticks — shell expansion silently ate four terms from an issue comment this run; re-sum all four worktree ledgers every run (6.370451, headroom $1.629549).
---
