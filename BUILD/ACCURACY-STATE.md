...
spend_usd: 1.488057
in_the_middle_of: #58 (M1.8) INSTRUMENT LANDED ON BRANCH, NOT MERGED, and its evidence bullet is NOT met. lemely/accuracy/metamorphic.py + tests/test_metamorphic.py: 18 tests, all green; scripts/check.sh --fast tests/test_metamorphic.py PASSED every gate incl. import-linter; pre-commit run --all-files green (NOTE: mypy/import-linter hooks are language:system and FAIL unless .venv/bin is on PATH -- that is an env quirk, not a code failure). Module lives in lemely/accuracy, NOT lemely/eval, because the contract "Evaluation analyses must stay pure" forbids lemely.eval from importing lemely.io and the runner must call correct_paper. *** WHAT IS PROVEN AND WHAT IS NOT: the tests use an INJECTED stub marker, so they verify the INSTRUMENT, not the three properties. The only real-marker run possible for free was the MCQ golden case via mcq_only=True (deterministic, zero cost): it returned 24 skipped / 0 held / 0 violated -- every property is INAPPLICABLE to MCQ leaves (no answer_points; answers already whitespace-normal). That is a real end-to-end exercise of the production marker and a deliberate design win (a naive build would have reported 24/24 HELD and looked green), but it is ZERO EVIDENCE about marker stability. NO PROPERTY HAS BEEN TESTED AGAINST REAL MARKING. *** ACCEPTANCE BULLET 4 IS A SPEND GATE, NOT DONE: "run against the golden set with cache_mode=bypass" needs the 10 theory cases through Gemini, which is live spend. NO inbox item authorises #58. Per the 2026-08-24T15:27 rebuke, an unauthorised sweep is a hard stop -- costed preflight + human authorisation FIRST, and this run did not spend and did not re-derive the gate away. Bullets 1,2,3,5 are implemented; 4 is outstanding. *** NO PR OPENED, deliberately: origin/develop is still 8 commits behind local develop (verified by real git fetch this run; origin 4b3ac66, local 97a5eac, ahead=8 behind=0), so ANY new PR would carry those 8 #88/#37/#45 commits in its diff. A HUMAN must push/reconcile develop before #58, #45 or anything else can open a clean PR. *** STILL BLOCKED ON THE HUMAN, unchanged this run: #45 design question (3 options, escalated 2026-08-24); #88 3 questions now unanswered for a 9th run. Board: #58 In progress.
---
