...
branch: chore/accuracy-run-30-bookkeeping (worktree HEAD); feature/accuracy-37-... @ 8f3ba7f and feature/accuracy-93-... @ 70a6957 pushed, no PRs
last_run_label: run-30-2026-08-26
last_run_headline: NO SPEND (ledger unmoved at 2.659533). PR #101 and PR #102 MERGED. #102 implements #59 DECISIONS 1 and 2, the last unhandled inbox directive: the three Paper 42 files are flattened to pixels at 150 DPI RGB A4 before extraction can ever see them — 23,727/24,754/22,961 text chars in, 0/0/0 out — with the DPI pinned by test to synth.py's own page_size=(1240,1754)/resolution=150.0 so the two arms of #59 cannot drift apart. #59 RESTATED as a HANDWRITING comparison with SCAN REALISM recorded as explicitly UNMEASURED, and #59 stays OPEN (PR says Refs not Closes; the measurement still needs the GoldenCase two-render data-model change). RENDERED PAGES NOT COMMITTED — verbatim CAIE content, MISSION 12.7, only script plus digest manifest landed. THREE THINGS MEASURED NOT ASSUMED, one of them my own error: the render was NOT byte-deterministic (Pillow embeds wall-clock time and a /Title; pinned as synth.py:150-162 does), MY FIRST TEST FOR IT WAS A TEST THAT LIES (render-twice-compare-bytes passes with the pinning removed because a 2-page synthetic renders inside one clock second; replaced by asserting the pinned /CreationDate,/ModDate,/Title are present, verified to fail without them), and pages are 1241x1754 not 1240x1754 (A4 rounds up one pixel) — recorded as a difference rather than called an exact match. Also flagged: my text-char counts (23,727 for s25) disagree with the 2026-08-25 audit's 34,090; counting-method difference, not a different file, input SHA256s recorded.
...
spend_usd: 2.659533
in_the_middle_of: RUN 30 — NOTHING RUNNING, NO SPEND (ledger sum 2.659533 of $25.00). PR #101 and PR #102 MERGED to develop (0834903, eeaea55), all CI green. *** THE INBOX IS NOW FULLY DRAINED except the 17:36 item, whose remaining parts are all blocked: items 1-2 and 4 (#88) are sequenced strictly behind #93, and item 6 (whitespace fixtures) is a QUESTION I ASKED AND THE HUMAN HAS NOT ANSWERED — may they be ordinary golden cases joining the accuracy denominator, or a metamorphic-only set load_golden_cases does not serve? I recommend the second and will implement on their word. *** PR #103 IS OPEN AND NEEDS LANDING: base develop, branch chore/accuracy-run-30-bookkeeping @ HEAD, BUILD/-ONLY. CI was still running at run end — poll 'gh pr checks 103' and merge once green. *** #59 IS IN PROGRESS AND CORRECTLY STILL OPEN: decisions 1 and 2 landed, the MEASUREMENT DID NOT. Its real blocker is the GoldenCase two-render data-model change (harness.py:66 scan_path, harness.py:115 hard-coded 'scan.pdf') — 'same paper, two renders' is not expressible today. Also 0625_w25_qp_42 is extractable but NOT markable (no w25 mark scheme exists upstream; probed, not assumed). Rendered fixtures live OUTSIDE the repo at /home/sico/lemely-fixtures/handwritten-59 and are reproducible from BUILD/accuracy-runs/handwritten-59/raster-manifest.json. *** STILL BLOCKED ON THE HUMAN, unchanged: #37's two questions (bullet 3's CI target cannot be re-derived from a corpus scoring id_match_rate 1.0 in BOTH arms — that would be PICKING a number); #93's sweep waiver (branch pushed, no PR, null by construction twice over, MUST land before any #57 preflight and before the split freeze); #58's q11b experiment (~$0.01, a DIFFERENT design, proposed not run); the 0625 paper-3 constant; PARSED CORPUS regeneration (artifacts do not exist, must wait for #93); and the THREE #98 RULINGS from run 29 (the #51 stratification axis, the DA2 hash-formula deviation, the cleartext-salt claim). *** #58 BULLET 4 STILL CORRECTLY UNTICKED (Fisher p=1.000; DO NOT re-run, MISSION 12.9). *** FOUR OPEN H ISSUES: #49, #51, #52, #55.
---
