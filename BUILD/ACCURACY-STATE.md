...
run_pointer: run-2026-08-29-l
worktree: /home/sico/Lemely-worktrees/accuracy
branch: fix/accuracy-legacy-cover-maxmark (this)
last_run_label: corpus-expanded-2026-08-29
last_run_headline: RUN 72 — CORPUS EXPANDED 2019-2025 -> 2010-2025 AND DET COVERAGE FELL 69.1% -> 34.8%. That is the most useful thing that happened. ZERO GEMINI SPEND (PaperScraper download only): 1332 docs, 0 failed; canonical mark schemes 479 -> 1130, question papers 479 -> 1134, only 5 qp lack a matching ms. *** NOTHING REGRESSED, THE MEASUREMENT GOT HONEST. Every parser fix this session was built and validated on 2019-2025 papers — a population with 2 parse errors. The expanded one has 354. '331 of 479' was measured on THE EASY DECADE, and DA45's correction of the MISSION to that figure is itself as-of-a-population, not absolute. By era: 2019-2025 331/477 (69.4%) REPRODUCES the earlier figure exactly, which is what makes the populations comparable; 2010-2018 62/299 (20.7%). *** ONE DEFECT WAS 94% OF THE FAILURES AND IS FIXED: 411 of 438, every affected session 2010-2016 — the pre-2017 cover page says 'maximum raw mark 70' not 'Maximum Mark: 70', so _MAX_MARK_RE raised BEFORE LOOKING AT A SINGLE QUESTION. 45 errored schemes sampled before touching it, 41 matched. 'raw' is optional but SPECIFIC; a permissive gap would let cover prose supply the number, and a test pins that. Errors 438 -> 354, parsed 692 -> 776. *** AND IT BOUGHT ZERO COVERAGE: exact stayed at 393; all 84 newly-parsing schemes landed in not_exact. In production they escalate anyway, so the fix moved the DIAGNOSTIC picture and not the routing. '84 more schemes parse' would have been true and misleading. The 2010-2018 exact RATE even fell 28.8% -> 20.7% because the denominator grew and the numerator did not — same shape as DA39. *** A HYPOTHESIS OF MINE, FALSIFIED IN ONE COMMAND: the remaining errors say 'No tables found — may be a scanned PDF' and I had the report half-written that older schemes are scans and out of reach. THEY ARE NOT. The errored 2010-16 papers carry MORE extractable text per page than the papers that parse (872 chars vs 1029, 1.30 tables vs 0.95). THE ERROR MESSAGE IS THE PARSER'S OWN GUESS AND I WAS ABOUT TO PROMOTE A GUESS INTO A FINDING BECAUSE IT WAS PRINTED IN A TRACEBACK. *** WHAT ACTUALLY BLOCKS THEM: the pre-2017 layout has no ruled row separators, so pdfplumber returns a WHOLE PAGE of questions as ONE 2-ROW TABLE with boundaries as newlines INSIDE cells. qualifies_as_mark_scheme_table rejects them CORRECTLY. Supporting that layout is A SECOND PARSING STRATEGY, NOT A TWEAK, and wants its own issue. *** CONSEQUENCE: THE DET PATH'S REACH IS BOUNDED BY CAIE'S 2016/17 LAYOUT CHANGE, NOT BY PARSER QUALITY. Expanding the corpus backwards does NOT expand det coverage. For 2010-2016 the Gemini path is the ONLY option and DA35 measured it failing ~50%. EVERY CORPUS-WIDE FIGURE PUBLISHED BEFORE TODAY IS AS-OF THE 479-SCHEME POPULATION and must carry that label. DA50.
...
in_the_middle_of: RUN 70 — branch fix/accuracy-ci-comment-stale carries DA48, the journal entry AND the ci.yml correction. The push was first REJECTED for missing 'workflow' scope; the human granted it and the fix is applied. NOT worked around while blocked — routing the corrected text elsewhere would have left the false claim where readers meet it, and a scope check is a permission boundary. Merge on green. *** WHAT DA48 FOUND: ci.yml said above the review-rate gate step that it 'has not been executed in CI and must not be claimed as CI-verified'. FALSE. Run 33235411251 carries the gate's own output; the step runs on every test job. Billing was restored, the step began running, the comment was never updated — so it told every reader, including me twice, that section 13 clause 2's enforcement half was not happening. THE GATE RUNS BUT DOES NOT BLOCK (exits 0 while review_rate_ratchet_armed=false). Clause 2 stays PARTIAL because observation is not enforcement — but THE GATE IS WIRED AND NOBODY NEEDS TO BUILD IT. *** THIRD STALE CLAIM OF THIS SHAPE TODAY: MISSION's 289 of 479 (DA45), the report's 29.03% (DA46), this comment (DA48). Each accurate when written, each rotted in place. DEFENCE NOW USED: a claim about the state of the system should NAME THE EVIDENCE THAT WOULD FALSIFY IT — the corrected comment names a run id. *** SECTION 13 AFTER THE FULL WALK: clause 1 NOT MET (11 non-H issues open; #112 closed this session); clause 2 PARTIAL (published; gate RUNS in CI but does not block; arming needs the rate to fall, DA33); clause 3 MET; clause 4 NOT MET (~0 of ~300 human labels; the two-seat protocol requirement IS met, DA40); clause 5 HALF (det parse coverage 331/479 measured AND the MISSION corrected in four places; Gemini marking accuracy unmeasured in its own terms); clause 6 NOT ASSESSABLE; clause 7 MET (#159 open, body refreshed); clause 8 MET (all four H issues current). *** SIX THINGS NEED THE HUMAN: (1) #41's $0.30 sweep, ledger-bounded, n=1 first-call check — code is merged, bullet 4 unmet until it runs; (2) #166's fix, BOTH halves, and whether 'diagnosed' satisfies C25 condition 1; (3) #52's ruling — cases are pulled and posted; (4) H7's overlap share — DEADLINE, before #47 completes; (5) THE THREE UNARMED DETECTORS — #38's all-defaulted variant (3 papers), #110 (15 schemes), #39 (4 questions) — all route to the same fallback DA35 measured failing ~50%, so ONE decision; (6) #95's three options (DA42) and #57's three routes (DA47). ON (6): do NOT let an agent narrow #95 to the excerpted questions or drop DA1's parse-path axis — both work by CHANGING THE MEASUREMENT TO FIT THE DATA. *** PR #159 (develop -> main) IS OPEN — DO NOT MERGE, MISSION 12.3 is human-only. *** LESSONS: WALK THE CLAUSES — 'cannot be completed' is a claim about a whole and a whole is not a work item, and judging section 13 in aggregate twice suppressed real work; a claim about the STATE OF THE SYSTEM should name the evidence that would falsify it; MEASURE BEFORE IMPLEMENTING A GATE; do not close an issue on a generous reading of your own work; when the cheapest option is the one you want, that is the reason not to take it; a permission boundary is not an obstacle to route around; USE QUOTED HEREDOCS for anything with backticks; re-sum all four worktree ledgers every run (6.370451, headroom $1.629549).
---
