...
spend_usd: 1.488057
in_the_middle_of: #88 (M2.1b, NEW, In progress) — the mark-scheme parsing pass #57 was blocked on. Created, boarded under epic #43, plan comment posted; #57 and #45 both commented to point at it; BLOCKERS section E updated and committed as 3b2b8a4 on develop (UNPUSHED, along with cd2a2c4). STRUCTURING CHOICE THE HUMAN LEFT ME: DECIDED — own issue, because #45 needs the identical det-failure set and a second run would pay the Gemini theory cost twice. ZERO-COST DET PARSE IN FLIGHT: '.venv/bin/lemely parse-mark-schemes /home/sico/PaperScraper/papers --output-root /tmp/acc57-full/out', log /tmp/acc57-full/parse.log. Det-only, NO NETWORK, NO SPEND, idempotent (skips existing JSON) — if it died just re-run the same line. Analyse with '.venv/bin/python /tmp/acc57-full/count.py /tmp/acc57-full/out det' and '.venv/bin/python /tmp/acc57-full/failures.py /home/sico/PaperScraper/papers /tmp/acc57-full/out /tmp/acc57-full/parse.log'. NOTE /tmp IS NOT DURABLE — if the box rebooted, re-run; it is free. *** FINDINGS ALREADY SOLID AT ~250/479, they will not reverse: (1) The section-E framing 'need ~300 leaves, have 71' IS WRONG. Partial yield is ALREADY 12358 leaves. Volume was never the constraint. (2) THE REAL CONSTRAINT IS STRATUM COVERAGE. 0625 p1 MCQ alone gives 1600 leaves but ALL are 0625 x det x band1 = 1 of DA1's 18 strata. At ~250 schemes all NINE det strata (3 syll x 3 band) are populated, min cell 0606-band1=46. (3) THE NINE GEMINI-PATH STRATA ARE EMPTY BY CONSTRUCTION and cannot be filled for free: ChainedMarkSchemeParser makes parse path det-if-det-succeeded / Gemini-if-det-failed, so the ONLY way to populate them is to pay for the det FAILURE set. That failure set is therefore the preflight denominator — measured, not estimated. (4) NEW PROBLEM NOT IN ANY ISSUE: 2894 of 12358 partial leaves (23%) carry marks=0/unknown, so they fit NONE of DA1's 1/2/3+ bands and are unassignable to a stratum. Under drop-only they would have to be excluded. This needs a decision on #57 and is a lead for #45. *** NEXT: finish the census, post it on #88, then produce the COSTED PREFLIGHT as an issue comment. DO NOT SPEND until that preflight is posted; --cache-mode bypass NEVER refresh (E2); $20.00-of-$25.00 stop-and-ask. Spend still 1.488057, unchanged this run.
---
