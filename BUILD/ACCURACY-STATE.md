...
branch: chore/accuracy-run-34-b4-causes (worktree HEAD); feature/accuracy-41-... @ 09314c7, feature/accuracy-37-... @ 8f3ba7f, feature/accuracy-93-... @ 70a6957 all pushed, no PRs
last_run_label: run-34-2026-08-26
last_run_headline: NO SPEND, NOTHING RE-DERIVED. PR #108 MERGED. B4 sharpened by READING EXISTING EVIDENCE rather than re-running: #45's failure-reason census had already classified all 229 det failures, and all four papers blocking #95 are in that set. 0580_s23_ms_22 = marks_cell_notation_not_parsed (+3, FULLY explained by 3 defaulted empty cells); 0606_s23_ms_12 = mark_aggregation_overcount (+36); 0625_s20_ms_31 and 0625_w21_ms_32 = genuine_mark_total_mismatch (-4 and -7, with 34 and 23 empty cells defaulted). So route 2 ('fix the parser first') is THREE DIFFERENT BUGS, not one, and only one of the four has a mechanically closed explanation — a work-stream, not a preliminary. CAREFUL ABOUT THE LABEL, stated so it is not over-read: genuine_mark_total_mismatch is the census's RESIDUAL bucket ('ruling out an overcount, column detection and cell-notation parsing'), NOT a positive finding that the paper is inconsistent — #45 records that the theory path has no shortfall heuristic so large theory deficits fall through to it. Those two causes are UNRESOLVED, not diagnosed; the named unconfirmed mechanism to check first is empty-cell defaulting interacting with rows.py flush(), which only sums a leaf's AnswerPoints when q_row_had_answer was set. ALSO PULLED OUT FOR B2's PRIORITY: the same census puts paper_profile_misconfiguration at 40, which IS #93 — so landing #93 alone converts 17.5% of the entire det failure set from failing to parsing and moves those 40 schemes from #88's paid-Gemini set to the free det set.
...
spend_usd: 2.659533
in_the_middle_of: RUN 34 — NOTHING RUNNING, NO SPEND (ledger 2.2803523500000105 this worktree; DA11 header sum 2.659533 of $25.00). PR #108 MERGED (e5825ff). *** PR #109 IS OPEN AND NEEDS LANDING: base develop, branch chore/accuracy-run-34-b4-causes, BUILD/-ONLY. CI was still running at run end — poll 'gh pr checks 109' and merge once green. *** NOTHING WAS RE-DERIVED THIS RUN: the four papers blocking #95 were looked up in #45's EXISTING classification of the 229 det failures rather than re-diagnosed. Posted to #95 and folded into ACCURACY-ASKS.md B4. *** DO NOT RE-RUN THE #95 PROBE OR THE #45 CENSUS — both are on disk: BUILD/accuracy-runs/golden-reparse-95-2026-08-26/manifest.json and BUILD/accuracy-runs/census-2026-08-24-b/{manifest.json,classified-failures.txt}. *** THE PROGRAMME REMAINS FULLY HUMAN-BLOCKED on BUILD/ACCURACY-ASKS.md B1-B14; notification sent last run. If only one is answered, B1 (#41's A-mark silent case) holds a finished branch. B2 (#93's waiver) is now measurably the highest-leverage: #93 is the paper_profile_misconfiguration bucket, 40 of the 229 det failures (17.5%), so landing it moves 40 schemes from #88's paid set to the free set BEFORE that preflight is re-costed. *** #41's branch is held UNMERGED on purpose (the VERSION bump costs nothing until merge; a no-op merge would need a second bump after the real fix) and #41 MUST NOT BE CLOSED or its sweep pre-authorisation lapses. *** FOUR OPEN H ISSUES: #49, #51, #52, #55 — #52's three seed rulings and #51's labeller-B identity are owed by the human; #49 keeps the split NOT FROZEN.
---
