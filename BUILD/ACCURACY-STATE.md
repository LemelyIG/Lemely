...
spend_usd: 0.4026
in_the_middle_of: #26 (M0.2) at 3b39646 (merge of develop) on feature/accuracy-26-determinism-substrate-plus-the-three. UNPUSHED, NO PR YET, NOT RE-REVIEWED since I reworked it. Single commit 44b4789 carries all of #26. REGRESSION I CAUSED AND FIXED THIS RUN: sweep of a9e6052 had pytest FAIL — I had hand-edited lemely.toml.example without updating lemely/runtime/example_toml.py, which GENERATES it; test_settings_example_drift pins the two together. Fixed in the generator (source of truth) + regenerated, folded into the same single commit. Lesson: a module-scoped green run does not cover generated-file drift tests. KEY DECISION needing human awareness: per_run_token_ceiling stays RUN-scoped; I did NOT add a per-sweep counter reset (that would delete the budget guard, §14). The issue's '200000 trips on sweep two' came from the EXAMPLE file, not the operative lemely.toml (2000000, ~17 sweeps). Branch now carries #66's 2700s pr-land cap. NEXT: accuracy-review {issue:26,base:origin/develop,head:HEAD}; then accuracy-pr-land {issue:26,branch:feature/accuracy-26-determinism-substrate-plus-the-three}. Still-red web gates (impeccable-detect, playwright-e2e, ui-thresholds) unchanged, web-only, not in CI, BLOCKERS.md.
