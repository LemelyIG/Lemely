...
spend_usd: 0.4668
in_the_middle_of: THE M0 QUEUE HAS DRAINED — all four branches landed 2026-08-23 under the human's CI waiver (commit 1289d8b): #77 PR #78, #30 PR #79, #33 PR #80, #29 PR #81. develop = 465fe34. THE BILLING BLOCK WAS NEVER FIXED, it was routed around: every merge rests on LOCAL gates plus the supervisor's out-of-session sweep, and NOT ONE CI STEP HAS EVER EXECUTED against any of them (every run concludes in 1-3s with steps=0, no runner provisioned; last checked run 32632548137). DROP THE WAIVER the moment a run provisions a runner, and treat the first real CI run on develop as genuinely new information — it is the first outside check these four will have had. CORRECTION TO THE PREVIOUS HEADER, verified not assumed: it claimed the human's two tooling commits would reach develop by NO route. THAT WAS WRONG — both c08460f (diff-scoped gates + persisted gate logs in scripts/check.sh) and 1289d8b (the CI waiver in .claude/workflows/accuracy-pr-land.js) ARE on develop, confirmed by content at 465fe34. They arrived because accuracy-pr-land's OPEN phase pushes the LOCAL branch ref, not whatever you pushed yourself: it merged origin's tip into the local feature/accuracy-29 ref (producing ec6515a, which carried the human's commits) and the PR squashed THAT. KNOW THIS FOR NEXT TIME — the tree that lands is the LOCAL branch ref, so gate the local ref, not just the sha you pushed. Checked here: the a893401 (gated) vs 465fe34 (landed) delta is exactly three files — accuracy-pr-land.js, scripts/check.sh, BUILD/ACCURACY-STATE.md — with NO production Python differing, and the supervisor's PASS sweep over 9a53d97 did cover both tooling commits, so the landed tree was in fact gated. Verify the same way if it recurs: 'git diff --stat <pushed> <merge-commit>'. THE ONE REAL DEBT LEFT: the 29.03% review_rate in this header, in BUILD/review-rate-baseline.json and in lemely/runtime/config.py review_rate_last_merged=0.2903 was measured BEFORE #29 changed which records enter the denominators. scripts/check_review_rate_gate.py still PASSES and still prints 0.2903 because it re-reads the PRE-#29 golden run and compares against a COMMITTED CONSTANT — it structurally cannot detect the change. That is #33's own open follow-up (the script has ZERO test coverage) and MUST be resolved before the ratchet is armed at M1. Recomputing is a MEASUREMENT item belonging AFTER #27: it needs human SPEND authorisation and #27's A/A churn floor first. Re-deriving offline from the saved golden run returns the SAME 0.2903 out of the OLD denominator — a fake recompute, the narrowed-denominator failure mode itself (Evidence log E1). DO NOT DO IT. A DA7 NUMBERING COLLISION was resolved at merge time: #29 and #30 both authored a 'DA7'; #30 landed first and keeps it, #29's became DA8 with its CHANGELOG.md / DELIVERY.md / docs/ACCURACY-STRATEGIES.md references updated; the DA7 citation in lemely/eval/analyses.py points at #30's entry and is correct as-is. NEXT: #27 (M0.3 A/A churn floor) is unblocked and MUST use '--cache-mode bypass', NEVER 'refresh' (Evidence log E2). It SPENDS REAL MONEY — tell the human before spending. Board currently reports NOTHING Ready under epic #23; a human may need to move #27 to Ready. ENV: jq NOT installed (use gh --jq); pre-commit needs .venv/bin on PATH; verify signing with 'git cat-file commit <sha> | grep -c gpgsig', NEVER %G?.
---
