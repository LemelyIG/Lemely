...
spend_usd: 1.488057
in_the_middle_of: NOTHING IN FLIGHT, tree clean, develop synced at 4b3ac66. Board: nothing in flight, nothing blocked, no open H issues, inbox 0 unchecked. *** LANDED THIS SESSION (5): #44 corpus restoration (5f7cde7), #40 coherence gate (d203916), #46 two-pass blind labeller (ba92be3), #36 the confidence unit (5cc58cb), #28 oracle-ablation MECHANISM (4b3ac66). All on genuinely green CI; allow_merge_without_ci was NEVER used, including several times a watch timeout made it tempting. M0 Done=10 (only epic #24 left), M1 Done=3, M2 Done=2. *** #57 (M0.7b) IS THE ONLY OUTSTANDING DIRECTIVE and is UNSTARTED. Authorised via option A: a mark-scheme parsing pass to reach the ~300-leaf target. ORDERING IS BINDING: (1) run the DETERMINISTIC MCQ parser FIRST over the restored corpus at ZERO COST and report how far it gets; (2) only then a costed preflight, POSTED AS AN ISSUE COMMENT, before any Gemini theory-scheme spend; (3) $20.00 of $25.00 stop-and-ask governs. DO NOT freeze #57 over the 71 golden leaves — considered and rejected by the human because drop-only makes it irreversible and it would discard most of what #44 fetched. STRUCTURING CHOICE THE HUMAN LEFT TO ME, recommendation recorded but NOT yet executed: make the parsing pass ITS OWN ISSUE rather than burying it inside #57, because #45 (M2.2 failure-reason census) needs the same parsed schemes and would otherwise duplicate the spend. I did not create that issue unilaterally — decide and do it at the start of the next run. *** READ THIS BEFORE THE NEXT SWEEP — the mistake that cost $0.0625 this session: I launched #28's ablation against a preflight that had ALREADY told me the oracle arm was dead code, and got a one-armed run with no 2x2. The preflight worked; I failed to act on what it said. RULE NOW BINDING: before launching ANY accuracy-measure run, confirm every arm the mode needs can actually PRODUCE RECORDS on this corpus — and if the preflight says a component is missing, STOP and build it, do not spend. #28's mechanism now exists (measure_accuracy(arm=...) plus a --arm CLI flag), so a real 2x2 is finally possible. *** IF THE #28 ABLATION IS RE-RUN (it is authorised, no per-item cap, $20 stop-and-ask): expect and accept a BOUNDED NO-DETECTION result. 31 distinct leaves is ~14% of MCNEMAR_IMPROVEMENT_N_FLOOR=219, and any gemini-path delta under 11.6pp sits inside the published A/A churn floor (det floor exactly 0.0). MISSION 12.9: 'NOT REPORTABLE, with reason' is a SUCCESS and must never be answered by re-running at higher n. The extraction share is a LOWER BOUND ONLY (synth.py cannot render superscripts or crossings-out) — that caveat is now in ablation_2x2's docstring. Use --cache-mode bypass, NEVER refresh (refresh skips the read but still WRITES, overwriting the shared cache). *** TWO LATENT GAPS RECORDED, NOT FIXED, both out of scope where found: (1) NONE of the 13 archived manifests under BUILD/accuracy-runs/ load through RunManifest — 11 carry an extra _actual_cache_mode_requested key that StrictModel(extra='forbid') rejects, 2 are sweep summaries lacking run_id. Verified 0/13 both BEFORE and AFTER #28's change, so nothing is newly broken, but archived evidence unparseable by the model that defines it is a real integrity problem worth its own issue. (2) eval/scans/<paper_id> has NO PRODUCER, so #46's pass-1 blindness holds against the MARK SCHEME but NOT against neighbouring questions on the same page; M2.4 labelling must not start assuming per-question crops exist. Both are the same shape as #57's parsing gap: #44 restored PDFs and M2 keeps needing derivation steps nobody scoped. *** #36 CARRIES ONE STRICT XFAIL: tests/eval/test_analyses.py::test_m0_baseline_band_distribution_reported_honestly. Bullet 6 is UNMET because all 71 rows of the saved A/A corpus have extraction_conf=null (that corpus predates extraction-confidence capture), so the propagation path is never exercised there. strict=True means it FAILS LOUDLY once a baseline re-run with per-answer extraction confidence closes the gap — then remove the xfail and assert the real distribution. #36 also newly lets a correct MCQ be flagged for review, which can RAISE review_rate; unmeasured, and covered by the human's gate-9 scoping (routing not marks) to fold into the M1 milestone-close sweep. *** SPEND 1.488057 raw of the 25.00 ceiling. ENV/PROCESS traps unchanged and still true: pre-commit is not on PATH and a piped 'grep -iv passed' with an unconditional echo WILL mask a real failure — I committed a lint error that way this session; use 'set -e' with an explicit PASS/FAIL branch. accuracy_board.py block writes to BLOCKERS.md as a side effect. git cherry-pick has no -q flag. git rebase silently drops signatures — check 'git cat-file commit HEAD | grep -c gpgsig'. accuracy-pr-land needs a DURABLE verdict in its args. CI matrix jobs run SERIALLY at ~10-18 min each, so a short watch window times out legitimately and TIMEOUT IS UNKNOWN, never a pass. Write BUILD/ docs on DEVELOP, never on a feature branch that may not merge. BUILD/ACCURACY-STATE.md and BUILD/BLOCKERS.md conflict on EVERY PR — resolve with 'git checkout develop -- <file>', develop is the superset.
---
