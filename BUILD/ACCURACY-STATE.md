...
branch: feature/accuracy-37-delete-the-positional-fallback @ 8eca7c2 — PR #147 OPEN, base develop, awaiting CI
...
spend_usd: 3.138147
in_the_middle_of: RUN 53 — PR #147 (#37, M1.2) OPEN and awaiting CI. Poll in background id bzggtpd2h, log /tmp/claude-1001/-home-sico-Lemely-worktrees-accuracy/dfc5ba97-22db-4b2f-87d0-c8567da1e6c9/tasks/bzggtpd2h.output — it RE-READS the PR head each cycle, waits for check-runs total_count>=5 with no queued/in_progress, prints 'CI COMPLETE'. 'TIMEOUT' from it means UNKNOWN, not red. Merge with 'gh pr merge 147 --squash' once green. *** ZERO SPEND this run, ledger 3.138147. *** WHY #37 WAS PICKED UP: the run-47 inbox note claiming 'B3 done in runs 42-44' is FALSE. B3's instruction was 'Land the branch'; the branch had NO PR and the positional fallback was STILL LIVE on develop (answer_extraction.py:47,73). Verified against develop directly rather than trusting the note — DO NOT trust 'done' notes without checking the artefact. *** WHAT #147 DOES: deletes the second pass of normalize_extracted_answers (the positional fallback that handed any unmatched answer the next unclaimed manifest id, so a guess was stamped id_match='exact' and one missing answer re-aligned a whole paper). Unmatched answers now keep their original id and surface UNMATCHED. Bullets 1+2 ticked; BULLET 3 NOT TICKED — it asks for the CI target to be RE-DERIVED and it was not; per B3 0.99 is INHERITED, NOT MEASURED, recorded as resolved-by-ruling. Limitation now recorded AT SOURCE at lemely/runtime/config.py beside id_match_rate_target. Same-commit rule (spec 7) HONOURED not relaxed: squash-merge lands removal+record as ONE commit, and the sweep measured id_match_rate 1.0 in BOTH arms so no red-CI window exists. *** STATED PLAINLY ON THE ISSUE: no accuracy-review workflow ran against this diff this session; the authorisation is B3 itself plus green mechanical gates plus a line-by-line read, NOT a review verdict. *** KNOWN GAP, no follow-up issue per B3: 'fuzzy' is still never emitted (canonical-form matching records '1 b'->'1(b)' as 'exact'); separating them needs raw-vs-canonical provenance out of extract_answers and its three web call sites. *** ALREADY MERGED EARLIER THIS SESSION: #145 (d9c5c45, #136 diagnosis) and #146 (037f8e1, DA21 + run-52 close). *** STILL HUMAN-GATED, none to be re-derived: B15/#112+#110 sweep waiver (also gates #136's fix half), #88 item 2 (DO NOT RESUME), B1/#41 (branch @1d41a97, NO PR by design), #58's two questions, B14/H #49/#51/#52/#55 (split NOT FROZEN). #59 not Ready; #95 blocked on #136. *** UNCHECKED AND WORTH A LOOK NEXT: #38 and #39 both have human rulings recorded (A6/A7, A8-A11) but appear UNIMPLEMENTED, and #127 is unlabelled/untriaged — check them against develop the same way #37 was caught.
---
