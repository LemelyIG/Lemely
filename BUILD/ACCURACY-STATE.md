...
run_pointer: run-2026-08-29-a
worktree: /home/sico/Lemely-worktrees/accuracy
branch: fix/accuracy-110-112-alt-routes (this)
last_run_label: alt-routes-112-110-2026-08-29
last_run_headline: RUN 61 — #110 AND #112 FIXED JOINTLY (B15), ZERO SPEND. *** #110 PREVALENCE MEASURED: 34 of 477 parsed schemes (7.13%) carry duplicate leaf ids, 57 leaves collapse of 17064 — AND 14 OF THE 333 SCHEMES THAT RECONCILE EXACTLY ARE AMONG THEM, so mark_total_mismatch_escalating is BLIND to this by construction. TWO MECHANISMS: (1) CAIE reprints the question number at each continuation page and the parser opened a fresh question (0606_w23_ms_13 opens '10' FOUR times) — FIXED; (2) a stray non-question table whose first column holds small integers (0580_w21_ms_22 emits 2,3,5 between Q20 and Q20(b)) — DELIBERATELY NOT AUTO-REPAIRED, because folding it back would be INVENTING question identity rather than reading it. Reported loudly instead. 57 -> 36 leaves lost, 34 -> 21 schemes. *** THE DUPLICATE-ID DETECTOR IS UNARMED BY DEFAULT AND THAT IS A DECISION: arming routes the paper to the Gemini fallback, which DA35 measured failing on ~50% of det-failures and 100% of 0606 — it trades a silently-wrong paper for a probably-absent one. Same footing as escalate_on_defaulted_marks. HUMAN CALL. *** #112: all three sub-defects fixed by ONE rule — A LINE CONSISTING SOLELY OF A MARKER (OR/EITHER/ALTERNATIVE/ALTERNATIVELY). The line-alone requirement IS the safety argument: CAIE writes 'accept either form' as an INLINE or, and 0625_s22_ms_33 carries '4000/10 OR 4000/9.8' inside ONE point. ALSO FIXED, not in the issue: EITHER was OPENING THE ALTERNATIVE in the continuation branch, so an EITHER...OR pair scored NEITHER route as primary. *** THREE READINGS MEASURED NOT ARGUED: sticky (marker to end of leaf) 313 exact / 245 marks removed — reproduces #112's OWN upper bound of 246 almost exactly; before-half-merged-as-text 304 exact; NON-STICKY (shipped) 331 exact / 64 removed. *** RECONCILIATION IS A CONFOUNDED CRITERION HERE AND MUST NOT BE USED TO ACCEPT OR REJECT: exact fell 333 -> 331, and that is EXPECTED — removing a one-directional overcount EXPOSES an undercount elsewhere. Optimising 'exact' would have selected the variant that fixed the defect LEAST. The 6 schemes that left exact were INSPECTED, not scored: all six are genuine alternatives. *** PRE-STATED PREDICTION MISSED: #112 predicted 77-246 marks removed, measured 64, BELOW the band. Explanation (the cell split keeps the marker-bearing point's primary half) is INFERRED FROM THE DESIGN, NOT SEPARATELY MEASURED. *** A BUG OF MINE CAUGHT BY THE MEASUREMENT: the split's before-half ignored mark_is_alternative, so on a BRACKETED row (#136 mechanism D) the primary half of an already-alternative route was counted — pushing 0606_s23_ms_12 from an exact 80 to 82. TWO ALTERNATIVE-ROUTE MECHANISMS CAN FIRE ON THE SAME ROW AND NEITHER MAY RE-ADMIT WHAT THE OTHER EXCLUDED. Fixed, pinned, back to 80/80. DA39.
...
in_the_middle_of: RUN 61 — branch fix/accuracy-110-112-alt-routes carries DA39, the journal entry and BUILD/accuracy-runs/{alt-routes-112-2026-08-29,dup-ids-110-2026-08-29}/. Open its PR to develop and merge on green. CUT THE NEXT BRANCH FROM origin/develop AFTER THIS MERGES — every accuracy PR appends to DECISIONS/JOURNAL and edits the state header, so a branch cut before its predecessor squashes ALWAYS conflicts (cost 3 rebuilt PRs: #168->#170->#171, #172->#173). Cherry-pick, never rebase (rebase strips signatures); and cherry-pick --continue does NOT re-sign — check 1. *** #95 IS THE NEXT AGENT WORK and is now fully unblocked: #136 fixed the four blocking schemes AND #110 has removed the page-break duplicate ids. Read the comment posted on #95 first. STILL TRUE: regenerating bakes in the REMAINING 36 collapsed leaves (mechanism 2, 21 schemes) — decide whether that is acceptable BEFORE regenerating, because DA6 keys leaves on (paper_id, question_id). *** OTHER LIVE AGENT WORK: #47 for TWO SEATS (C24), #39 (see the filtered-sum comment of 2026-08-28 — its 'filtered = 0 of 575' does not hold on the wider corpus and bullet 3's tautology claim is population-dependent), #41 under B1, #127 folded into #95 by C5. *** HUMAN ASKS: #52 (cases are pulled and posted — CASES.md; the H8 log stays empty until you rule) and whether to ARM escalate_on_duplicate_leaf_ids (a cost/coverage call, see DA39). *** C25 lets #55 fire ONCE without further interview when #166 is resolved AND a costed #28 preflight passes AND #47's two-seat protocol is delivered. #166 is DIAGNOSED, NOT FIXED — 'fixed or formally re-scoped' is the human's judgment, do not assume it is met. #49 is NOT a gate (human's explicit call) but is NOT closed. *** A FIX FOR #166 IS NOT AUTHORISED: validating one needs spend. It has TWO parts — make Gemini set is_alternative on alternative routes, AND stop it emitting empty {} objects inside parts. Addressing only one will move the failure rate without closing the gap. *** PR #159 (develop -> main) IS OPEN — DO NOT MERGE, MISSION 12.3 is human-only; its section-13 scorecard predates C11. *** LESSONS: when a fix removes a ONE-DIRECTIONAL error, the metric that error was inflating GETS WORSE before the corpus gets better — optimising it ships the wrong variant; read the fields the code already fills before spending to rediscover them; an ABORT IS NOT A PILOT; a premise I WROTE is not evidence; opposite-sign defects HIDE EACH OTHER; re-sum all four worktree ledgers every run (6.370451, headroom $1.629549).
---
