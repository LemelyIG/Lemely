...
branch: chore/accuracy-run-24-bookkeeping (worktree HEAD); feature/accuracy-93-profiles-paper-type-lookup-order is pushed at e82199f and is the live work
...
spend_usd: 2.369728
in_the_middle_of: RUN 25 — NOTHING RUNNING, NO SPEND THIS RUN (ledger sum 2.369728; zero gemini calls were made). *** #93 IS READY TO LAND AND WAITS ONLY ON A WAIVER. Branch feature/accuracy-93-profiles-paper-type-lookup-order @ 70a6957, pushed, NO PR OPEN. The gate-9 sweep was PRE-AUTHORISED on #93 at 2026-08-25T11:35:43Z (I missed that comment last run and asked a question it had already answered; withdrawn on the issue). Conditions 1 and 3 are now SATISFIED: the costed preflight is posted and its answer is DO NOT SPEND, and the adversarial review is done. WHAT IS OUTSTANDING IS THE HUMAN'S WAIVER of the harness sweep on instrument-blindness grounds — the same waiver #38 got under A6 ruling (a). On that word: open the PR and land. *** WHY NO SWEEP: it is null by construction, twice over — the harness never invokes the det parser (harness.py:103-109 deserializes pre-parsed mark_scheme.json; lemely.io.det imported NOWHERE under lemely/accuracy or lemely/eval; the det question-paper extractor is reachable only from the question-bank CLI at cli.py:919), AND the golden set has ZERO 0625 paper-2 cases. Spending would buy a guaranteed-zero delta. *** SUBSTITUTE EVIDENCE IS COMMITTED at BUILD/accuracy-runs/sweep93-2026-08-25/ — a census of 489 schemes at $0.00: 41 schemes 0625 p2 theory_core -> mcq (the fix), 40 schemes 0625 p3 theory_extended -> theory_core (a SECOND real bug, fixed by the ORDER change, causally inert since scheme_format is POINT_BASED both sides), no other subject or paper changed, and the theory-flips-to-MCQ regression hazard did not occur once. *** OPEN QUESTION ALSO ON #93: shall I open a separate issue to correct the 0625 paper-3 CONSTANT? It is wrong (cover says Core, map says Extended) and deliberately left wrong-but-annotated, because correcting it is an unmeasured second change riding on this one. *** WHEN #93 LANDS: #88's paid-parse preflight MUST BE RE-COSTED (denominator had ~40 schemes on the wrong side), and #93 must land BEFORE any #57 preflight is acted on and BEFORE the split is frozen — both ordering constraints still stand. *** #58 BULLET 4 STILL UNTICKED and still correct to leave so: the control arm falsified its premise (marking-only churn 0/62, not 0.1565 — that figure is end-to-end), but reorder 1/31 vs control 0/62 is Fisher p=1.000, unresolved in BOTH directions. DO NOT re-run (MISSION 12.9). The settling experiment (re-mark q11b alone ~10x each arm, ~$0.01) is a DIFFERENT design, proposed on #58, NOT run, needs the human's word. *** ALSO STILL WAITING: item 6 of the 17:36 directive (whitespace fixtures — ordinary golden cases joining the accuracy denominator, or a metamorphic-only set? I recommend the second), 17:36 items 2-4 (#88 spend, the ratified 9,464 leaf count, the lemely.toml token-ceiling raise), A5 (#37 sweep), the #59 handwriting corpus. *** THIS BRANCH (chore/accuracy-run-24-bookkeeping, off develop d2f3662) carries only state/bookkeeping and must be landed or folded into the next PR. *** UNPUSHED: chore/accuracy-post-45-bookkeeping holds run-21's E4 remedy (3 commits, local only).
---
