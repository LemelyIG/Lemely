...
run_pointer: run-2026-08-29-e
worktree: /home/sico/Lemely-worktrees/accuracy
branch: none in flight — cut the next from origin/develop
last_run_label: undersum-39-2026-08-29
last_run_headline: RUN 65 — #39 BULLETS 1-3 MEASURED BEFORE IMPLEMENTING, AND TWO OF THE THREE COLLAPSE. Zero spend, 479 schemes, 13690 leaf questions with answer points. *** BULLET 1 IS MET IN FORM AND UNENFORCED IN FACT: Question.validate_mark_point_sum already states the FILTERED sum (excluding is_alternative and is_optional) — but it NEVER RUNS ON DET OUTPUT. It is a model_validator(mode='after'), rows.py assigns marks/answer_points AFTER construction, and pydantic revalidate_instances defaults to 'never' so wrapping in a MarkScheme does not re-check either. VERIFIED BOTH DIRECTIONS: model_validate on a breaching payload RAISES; construct-then-assign DOES NOT. 4 questions across 479 schemes reach output in breach unflagged (e.g. 0606_s19_ms_13 11i, tariff 1 vs primary sum 3). *** BULLET 2 WOULD FIRE ZERO TIMES: 13668 exact, 0 under, 4 over, 135 with no points. Not luck — rows.py DERIVES q.marks from the filtered primary sum, so under-summing is IMPOSSIBLE BY CONSTRUCTION on the det path. Implementing it would ship a gate that cannot fire. This is the MIRROR of escalate_on_defaulted_marks, which turned out to fire on 44.4% of papers; both failure modes cost a run and the measurement costs minutes. *** BULLETS 2 AND 3 ARE THE SAME BULLET: the under-sum direction only means anything where the tariff is NOT derived from the points — the GEMINI path, which is exactly the missing paper-level aggregate bullet 3 names. FOLD 2 INTO 3. From DA35: Gemini's observed failures are the OVER direction, already caught because that path DOES go through model_validate; the under direction there is UNMEASURED because output failing validation never lands. *** SHIPPED: a reporter in reconcile.check, UNARMED by default (escalate_on_primary_sum_breach), consistent with escalate_on_duplicate_leaf_ids. NOT SHIPPED: the under-sum check (cannot fire) and the paper-level Gemini aggregate (bullet 3's own scope, needs spend). DA43.
...
in_the_middle_of: NOTHING IN FLIGHT. The accuracy PR queue is EMPTY; every PR opened this session is merged. Cut the next branch from origin/develop — every accuracy PR appends to DECISIONS/JOURNAL and edits this header, so a branch cut before its predecessor squash-merges ALWAYS conflicts (that cost 3 rebuilt PRs: #168->#170->#171, #172->#173). Cherry-pick, never rebase (rebase strips signatures); cherry-pick --continue does NOT re-sign — check 'git cat-file commit HEAD | grep -c gpgsig'. *** MERGED THIS SESSION: #167 (C20/C21), #169 (#136), #171 (#161), #173 (C22-C25 + #166 diagnosis), #174 (#110+#112), #175 (C24), #176 (#110 phantom leaves), #177 (#95 re-probe), #178 (#39), #179 (figure correction). DA31-DA43. Spend $0.376981, ALL of it C22's authorised diagnosis. *** DET PARSER, cumulative and measured corpus-wide over 479 schemes: reconciling exactly 289 -> 331; still not exact 190 -> 148 (it was 146 after #136 alone and ROSE BY 2 when #112 removed double-counted alternative routes from papers reconciling BY CANCELLATION — that rise is the fix WORKING, see DA39); overcount 845 -> 349 marks; leaves lost to id collapse 57 -> 25; schemes with duplicate ids 34 -> 15; phantom contentless leaves 58 -> 0. *** PR #159 (develop -> main) IS OPEN AND ITS BODY WAS REFRESHED 2026-08-29 — it had gone materially stale (still cited #151/ask C8 as gating the queue, which C11 CLOSED, and called fixed defects merely 'diagnosed'). DO NOT MERGE IT: MISSION 12.3 is human-only. *** FOUR HUMAN ASKS: (1) #52 — cases pulled and posted, H8 log stays empty until ruled; (2) H7's overlap share (DA40) — HAS A DEADLINE, must change BEFORE #47 completes or the choice becomes visible to the sample it selects; (3) the THREE UNARMED DETECTORS — escalate_on_defaulted_marks (#38, 44.4% of papers), escalate_on_duplicate_leaf_ids (#110, 15 schemes), escalate_on_primary_sum_breach (#39, 4 questions) — all route to the same fallback DA35 measured failing ~50%, so decide them TOGETHER; (4) #95's three options (DA42) — do NOT narrow it to the excerpted questions without a ruling, that rule invents correspondence inside the measurement corpus. *** REMAINING AGENT WORK: #41 (code half is zero-spend, but acceptance REQUIRES a gate-9 before/after sweep = SPEND, so post a costed preflight first; and note the BATCHING CONSTRAINT — every M1 marker-prompt change must land in ONE commit with ONE VERSION bump or the delta is unattributable); #39 bullet 3 (paper-level Gemini aggregate — needs spend, and bullet 2 should be FOLDED INTO IT because the under-sum direction cannot fire on the det path); a cheap issue for the five WRONG is_excerpt flags (DA42, deliberately not opened). #127 is folded into #95 by C5 and blocked with it. *** #166 IS DIAGNOSED, NOT FIXED, and a fix is NOT authorised. TWO parts — set is_alternative on Gemini's alternative routes, AND stop the empty {} objects inside parts; one without the other moves the failure rate without closing the gap. C25 condition 1 says 'fixed or formally re-scoped', which is the HUMAN's judgment to apply. *** LESSONS: MEASURE BEFORE IMPLEMENTING A GATE (#39 bullet 2 cannot fire; #38's could not stop firing; neither was visible from the code); re-run a feasibility probe before acting on its verdict (half of #95's had aged out and half had not); when the cheapest option is the one you want, that is the reason not to take it; widening a scope to unblock yourself is not an agent's call; when a fix removes a ONE-DIRECTIONAL error the metric it was inflating GETS WORSE first, so do not optimise it; read the fields the code already fills before spending to rediscover them; an ABORT IS NOT A PILOT; opposite-sign defects HIDE EACH OTHER; re-sum all four worktree ledgers every run (6.370451, headroom $1.629549).
---
