...
branch: feature/accuracy-88-item6-whitespace-fixtures @ 2c6286b -> PR #134 OPEN, CI re-running after two real fixes
last_run_label: run-45-2026-08-26
last_run_headline: ZERO SPEND; ledger unmoved at 3.124540. *** PR #133 MERGED to develop 17:30:38Z on five green checks (DA13/DA13a, the aborted-sweep evidence, run-42/43/44 bookkeeping). *** PR #134 (B5 / #88 item 6) WENT RED IN CI ON TWO REAL DEFECTS IN MY OWN DIFF — both fixed, neither was gate noise, and I did NOT loosen either gate. (1) answers.json had no trailing newline: my local 'pre-commit run --all-files' passed because --all-files means files GIT KNOWS ABOUT and the fixture was still UNTRACKED when I ran it. STAGE FIRST, THEN RUN THE HOOKS — this is the 2nd time this class of miss landed a red pre-commit (2026-08-23 was 12 evidence JSONs). (2) tests/test_golden_corpus.py's _EXPECTED_MAXIMUM_MARK is a PER-DIRECTORY registry and the new case was absent, failing 2 invariants on all 3 Python versions; fixed by REGISTERING the case (value 70 inherited — the variant's mark_scheme.json is BYTE-IDENTICAL to the _correct sibling's, verified), never by loosening the assertion. Missed because I ran only the modules I predicted were affected and my grep for corpus constants looked for '== 71'-shaped literals, which a dict registry does not match. *** #134 then merged with develop (NOT rebased) and the DECISIONS.md conflict resolved by KEEPING BOTH SIDES in numeric order DA12 -> DA13 -> DA13a -> DA14. *** #88 item 2 STILL STOPPED awaiting the human's $13.31-vs-$7.26 decision.
...
spend_usd: 3.124540
in_the_middle_of: RUN 45 — NOTHING RUNNING. ZERO SPEND, no Gemini call; ledger unmoved, programme-wide 3.124540. *** PR #133 IS MERGED (develop, 17:30:38Z, five green checks, verified with gh pr view). *** ONE PR OPEN: **#134** (feature/accuracy-88-item6-whitespace-fixtures @ 2c6286b, base develop), B5 / #88 item 6. Its CI was RED and is now re-running after two REAL fixes — poll 'gh pr checks 134' and merge when green. It is MERGEABLE against current develop (merged, not rebased; DECISIONS.md resolved by KEEPING BOTH sides in order DA12 -> DA13 -> DA13a -> DA14, nothing dropped). *** THE TWO CI FAILURES WERE MY DEFECTS, NOT GATE NOISE, AND NEITHER GATE WAS LOOSENED: (1) tests/golden/0580_s23_qp_22_theory_whitespace/answers.json had NO TRAILING NEWLINE. Local 'pre-commit run --all-files' passed because --all-files means files GIT KNOWS ABOUT — the fixture was still UNTRACKED when I ran it. THE RULE: git add -A FIRST, THEN pre-commit, THEN commit. This is the SECOND time (2026-08-23 was 12 evidence JSONs). Path.write_text(json.dumps(...)) writes no trailing newline. (2) tests/test_golden_corpus.py's _EXPECTED_MAXIMUM_MARK is a PER-CASE-DIRECTORY registry; the new case was missing, failing test_every_case_directory_has_a_recorded_baseline and test_is_excerpt_matches_the_issue_table on all three Python versions. FIXED BY REGISTERING THE CASE at 70 — inherited, because the variant's mark_scheme.json is BYTE-IDENTICAL to the _correct sibling's (verified, not assumed). _EXPECTED_IS_EXCERPT needed NO change: it is keyed by paper_id, which the variant shares. *** WHY I MISSED IT, so the next run does better: I ran only the modules I PREDICTED were affected (test_analyses, test_metamorphic, test_accuracy_harness, the new file) and never ran tests/test_golden_corpus.py; my grep for corpus constants searched for '== 71'-shaped literals and a dict-keyed registry does not match that shape. WHEN TOUCHING tests/golden/, RUN tests/test_golden_corpus.py. *** #88 ITEM 2 IS STILL STOPPED AND MUST STAY STOPPED until the human answers issuecomment-5428086623: preflight falsified at 1.83x, measured $0.07005/scheme, 190 = $13.31 not $7.26, 7.25M tokens not 3.60M. DO NOT RESUME, DO NOT RE-DERIVE THE GATE. 6 parsed schemes kept at outputs/sweep88-2026-08-26/out (gitignored). The 5M ceiling is UNDERSIZED (needs 7.25M, trips ~scheme 131/190) and LEFT at 5M deliberately — re-raising is part of the human's decision. *** MEASURED, do not re-derive: golden corpus is now 12 dirs / 78 ROWS / 31 DISTINCT LEAVES; whitespace property 7 held (was 0 held / 71 skipped). synth.py's render-fidelity guard REFUSES tabs and newlines (no vendored glyph), so a rendered fixture STRUCTURALLY CANNOT test those forms. *** STILL OWED BY ME: B1 (#41), B4 (#95 route 2), B6 (#58, ~$0.01 authorised), B12 (#105), B13 (#59), B15 (#112), B16 (#114). B14/H issues stay the human's: #49/#51/#52/#55. #49 keeps the split NOT FROZEN. Board Ready EMPTY.
---
