...
run_pointer: run-2026-08-29-d
worktree: /home/sico/Lemely-worktrees/accuracy
branch: chore/accuracy-95-excerpt-blocker (this)
last_run_label: golden-reparse-95-2026-08-29
last_run_headline: RUN 64 — #95 RE-PROBED, NOTHING REGENERATED, ZERO SPEND. *** BLOCKER 1 IS CLEARED: the 2026-08-26 probe found 4 of 5 source schemes failing to det-parse (regenerating would have destroyed 10 of 11 fixtures); all five now parse to their printed maximum EXACTLY and carry NO duplicate ids, after #136/#110/#112. *** BLOCKER 2 STANDS, UNTOUCHED BY ANY OF THAT PARSER WORK, and is now quantified per fixture: every fixture is an EXCERPT, and regenerating the SCHEME alone replaces it with the FULL PAPER while answers.json and scan.pdf still describe the excerpt. Blow-up runs 4.2x to 43x — 0625_w21_qp_32_theory_nested carries ONE QUESTION AND MAX 5 against a 43-leaf, 80-mark paper; 0625_s20_qp_31 carries 7 questions / max 19 against 41 leaves / 80. *** NEW FINDING: is_excerpt IS FALSE ON 5 OF 11 FIXTURES, ALL OF WHICH ARE EXCERPTS (0625_s20_qp_31 x3, 0625_m20_qp_12_mcq, 0625_w21_qp_32_nested). This is DISTINCT from A8's ground for retiring #39 bullet 4 — A8 said the flag is a HARNESS ATTRIBUTE not a scheme property, and THAT STANDS; what is new is that the flag is also FACTUALLY WRONG. Invisible because nothing reads it. *** VERDICT: #95 REMAINS NOT EXECUTABLE AS WRITTEN. Its scope covers the SCHEME, which is one of three coupled artefacts per fixture. Regenerating answers.json and scan.pdf in step is a far larger and more irreversible operation than the issue describes and is NOT what A6 authorised — WIDENING SCOPE TO MAKE BLOCKED WORK POSSIBLE IS NOT AN AGENT'S CALL. *** THREE OPTIONS, NO RECOMMENDATION: (1) widen #95 to regenerate schemes+answers+scans together; (2) narrow #95 to regenerating the scheme RESTRICTED TO THE EXCERPTED QUESTIONS — cheapest-looking, and the one I was tempted by, which is why I did not push it: it needs a rule for WHICH PARSED QUESTIONS CORRESPOND TO THIS EXCERPT, and an agent choosing that rule INVENTS CORRESPONDENCE INSIDE THE MEASUREMENT CORPUS; (3) abandon the rebuild and accept scheme-parsing stays unmeasurable in the golden harness. *** SEPARATELY ACTIONABLE AND CHEAP: correcting is_excerpt on the five mislabelled fixtures is zero-spend and changes no measurement. DELIBERATELY NOT DONE — editing fixture metadata under a rebuild issue is how scope creep enters an irreversible corpus; it wants its own issue. DA42.
...
in_the_middle_of: RUN 64 — branch chore/accuracy-95-excerpt-blocker carries DA42, the journal entry and BUILD/accuracy-runs/golden-reparse-95-2026-08-29/. Open its PR to develop and merge on green. CUT THE NEXT BRANCH FROM origin/develop AFTER IT MERGES. *** MERGED THIS SESSION: #167, #169 (#136), #171 (#161), #173 (C22-C25 + #166), #174 (#110+#112), #175 (C24), #176 (#110 phantom leaves). DA31-DA42. *** #95 IS BLOCKED AGAIN AND THE BLOCKER IS NOT A PARSER PROBLEM — see DA42. Do NOT narrow it to the excerpted questions without a human ruling: that rule invents correspondence inside the measurement corpus. *** HUMAN ASKS, none blocking other agent work: (1) #52 — cases pulled and posted, H8 log stays empty until ruled; (2) H7's overlap share — DA40, and it has a DEADLINE: it must change BEFORE #47 completes or the choice becomes visible to the sample; (3) whether to ARM escalate_on_duplicate_leaf_ids — 15 schemes, 25 collapsed leaves, and arming routes them to a fallback DA35 measured failing ~50%; (4) #95's three options in DA42. *** REMAINING AGENT WORK: #39 (read the filtered-sum comment of 2026-08-28 first — its 'filtered = 0 of 575' does NOT hold on the wider corpus and bullet 3's tautology claim is population-dependent), #41 under B1, and a NEW cheap issue worth opening for the is_excerpt correction. #127 is folded into #95 by C5 and therefore blocked with it. *** #166 IS DIAGNOSED, NOT FIXED, and a fix is NOT authorised (validating one needs spend). It has TWO parts — make Gemini set is_alternative on alternative routes, AND stop it emitting empty {} objects inside parts; fixing one moves the failure rate without closing the gap. C25 condition 1 says 'fixed or formally re-scoped', which is the HUMAN's judgment. *** PR #159 (develop -> main) IS OPEN — DO NOT MERGE, MISSION 12.3 is human-only; its section-13 scorecard predates C11. *** LESSONS: re-run a feasibility probe before acting on its verdict — half of #95's had aged out and half had not; when the cheapest option is the one you want, that is the reason not to take it; widening a scope to unblock yourself is not an agent's call; when a fix removes a ONE-DIRECTIONAL error the metric it was inflating GETS WORSE first; read the fields the code already fills before spending to rediscover them; an ABORT IS NOT A PILOT; re-sum all four worktree ledgers every run (6.370451, headroom $1.629549).
---
