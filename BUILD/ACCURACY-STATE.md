...
branch: none
last_run_label: retired-2026-08-29
last_run_headline: PROGRAMME RETIRED under human ruling C26 (DA51). NOT DONE — retired. Section 13's Definition of Done is NOT met and retirement does not make it met: clause 1 NOT MET (8 non-H sub-issues open at close, plus 6 off-epic defect issues), clause 2 PARTIAL (gate runs in CI on every test job, review_rate_ratchet_armed=False at config.py:190 so it observes and does not block), clause 3 MET (NOT_APPLICABLE published with its reason), clause 4 NOT MET (**ZERO of ~300 labels exist — eval/labels/ holds only .gitkeep**), clause 5 BOTH HALVES SHORT, clause 6 NOT MET (32.58% mean vs a 10% budget), clause 7 MET (#159 open, LEFT OPEN per section 12.3 at the human's direct instruction), clause 8 MET. *** M0 CAME CLOSEST AND STILL DID NOT MEET ITS OWN ACCEPTANCE. All 12 sub-issues of #24 are closed and that is NOT the same thing: box 3 needs the baseline split into extraction-attributable/marking-attributable/masked and THAT SPLIT DOES NOT EXIST — #28 is closed as COMPLETED with all four acceptance boxes UNTICKED, the oracle+mark arm produced zero records, so ablation_2x2() returns b=c=n_pairs=0. THE ONE QUESTION THE PROGRAMME WAS BUILT TO ANSWER — extractor's fault or marker's? — HAS NO MEASUREMENT BEHIND IT. I first wrote 'M0 completed in full' by counting closed sub-issues; that is the generous self-assessment section 14 warns about, caught reviewing my own retirement text. *** The headline outcome is unmet in ALL THREE parts: no attribution measurement, the review rate did not fall (32.58% vs the 10% budget), and the wrong-mark catch rate FELL rather than doubled — flag_recall 14.29% n=71 on the honest post-D18 run run-ef443fc2931e, against the 27.3% (3/11) the >=6/11 target was set from. AND NOTE THE SHAPE: when mark_accuracy was restated post-D18 it moved UP and was promptly relabelled historical-superseded, while flag_recall and flag_precision_high moved DOWN in the SAME run and kept their better-looking legacy numbers in three files. Only the flattering restatement got published — selective disclosure, same family as D18. *** 24 issues closed: **1 completed (#136), 23 not planned**. All 65 board items Done, where Done now means 'no longer being worked'. Spend 6.370451 of 8.00 across four worktree ledgers, re-read at close. *** TWO STALE FIGURES FOUND IN THE PUBLISHED REPORT WHILE RETIRING IT: (1) it still said 'det parse coverage MET, now 331 of 479' which DA50 falsified THAT MORNING (393 of 1130 = 34.8%) — PR #189 landed DA50 into DECISIONS/JOURNAL/state and never touched the file that publishes the figures; (2) the spend table read 5.993470, taken before the last ledger write, sitting directly beneath a written rule against exactly that. FOURTH and FIFTH instances of DA45/DA46/DA48. Both corrected. THE RULE: a restatement is finished when every artifact that published the superseded figure carries it, not when the decision record does. AND: none of the five was caught by a gate, a test or CI — the controls bind code and do not bind prose. *** NOTHING UNMERGED CARRIED UNLANDED WORK — checked, not assumed. Final account: BUILD/ACCURACY-FINAL-REPORT.md.
...
in_the_middle_of: NOTHING. The programme is retired (C26/DA51) and there is no next work unit. If you are a supervisor or orchestrator reading this to resume: STOP — read the retirement banner at the top of BUILD/ACCURACY-MISSION.md, do not query the board, do not pick an issue. An empty queue is the intended end state, not a fault to route around. PR #159 (develop -> main, green, 105 commits) is OPEN and stays open: merging to main is human-only under section 12.3 and the human confirmed that at retirement. Issues #10-#22 are untouched product work, NOT part of this mission.
---
