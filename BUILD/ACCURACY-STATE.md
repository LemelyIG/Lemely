...
spend_usd: 1.488057
in_the_middle_of: RUN 9 WITH NO HUMAN INPUT (10th consecutive). NOTHING PENDING A POLL; nothing running in the background. Tree clean, tip 377cb5f+1. *** SUPERVISOR SWEEP NOW COVERS 377cb5f -- the gap flagged last run is CLOSED; last run's two commits (20908e1 fixes, 377cb5f record) are full-suite green. *** THIS RUN: verified the gate-9 classification of #37 (M1.2) instead of inheriting it, and it CHANGED. The 2026-08-24 gate-9 directive provisionally listed #37 among the items NOT needing a per-item sweep, with 'unless one of them turns out to move marks -- say so and ask'. IT MOVES MARKS. Proof at source, not judgement: normalize_extracted_answers (lemely/io/answer_extraction.py:69-77) REWRITES question_id -- the field deciding which mark-scheme leaf an answer is marked against -- and the rewritten answers are what GeminiAnswerExtractor.__call__ returns (answer_extraction.py:187-195), flowing straight into marking. Deleting the fallback therefore changes awarded marks whenever it fires. #37 belongs with #38/#39/#41. NOTE THIS IS THE EXPENSIVE DIRECTION: it BLOCKS #37 rather than releasing it, so it is not the cheaper branch being assumed. *** SECOND FINDING, bears on #37 acceptance bullet 3 and is non-obvious from either file alone: id_match on EvalRecord is SYNTHESISED BY THE HARNESS, not read from the extractor -- harness.py:402 hardcodes id_match='exact', id_match_rate is matched_extraction_ids/total_extraction_questions at harness.py:855, and grep for id_positional_fallback across lemely/ returns exactly ONE hit (the warning log itself). Nothing wires the fallback outcome into id_match. So the fallback rewrites a guess into a valid manifest id BEFORE the harness matches, and the harness scores it matched -- the metric literally counts a positional guess as a successful id match, confirming bullet 3's 'post-fallback coverage' premise and making the same-commit rule load-bearing (removal drops id_match_rate against the 0.99 target at config.py:143, enforced in CI at cli.py:1098). In #37's favour: IdMatch Literal exact/fuzzy/unmatched ALREADY EXISTS at lemely/eval/records.py:23; only the provenance wiring is missing. *** COULD NOT DETERMINE FREE: how often the fallback fires. Needs live extraction; the committed 915 exact / 8 unmatched counts are harness-side and POST-fallback, blind to it by construction. *** answer_extraction.py UNTOUCHED, zero diff. *** THE ASKS ARE NOW FIVE, all pure decisions: (1) #58 bullet 4 -- authorise ~USD 0.144 or formally RETIRE as done for #40's; the ONLY thing between PR #90 (green, reviewed, MERGEABLE) and merge. (2) #88 q2 -- profiles.py:50 gate-9 mark-changing, call chain verified; blocks #88->#57->#47. (3) #88 q1/q3 -- DA1/H4 (#49), MISSION 3.5 forbids working around. (4) #45's 3-option design question; its branch is now accuracy-review CLEAN (merge, zero findings) but MISSION 7.1 condition 1 fails so NO PR -- #45 stays OPEN. (5) NEW: #37 needs a gate-9 sweep (unauthorised spend) or an explicit waiver to land without one. Also awaiting a call: mcq.py detector relax and profiles.py:50, both mark-changing, both ZERO-DIFF. *** BLOCKERS.md section G still exists only on the #45 branch -- tail-conflict hazard UNCHANGED. *** NO SPEND; ledger unmoved at 1.488057.
---
