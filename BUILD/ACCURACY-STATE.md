...
run_pointer: run-2026-08-29-k
worktree: /home/sico/Lemely-worktrees/accuracy
branch: measure/accuracy-clause5-gemini (this)
last_run_label: path-accuracy-2026-08-29
last_run_headline: RUN 71 — CLAUSE 5's GEMINI HALF IS MEASURABLE FROM DATA ALREADY ON DISK, AND THE HEADLINE IS A BLEND. Zero spend. *** I had written the Gemini half off as needing a marking run. IT DID NOT. aa-floor-2026-08-23-a's per-repeat records carry parse_path, so the split was one script away since 2026-08-23. *** METHOD VALIDATED BEFORE USE: the leaf collapse reproduces the published wilson_mark_accuracy_per_repeat successes EXACTLY ON ALL TEN REPEATS, and the script ASSERTS it and refuses to run otherwise. Only then split by path. parse_path tracks the marking path exactly as C11 describes — det is the MCQ paper alone, gemini is all four theory papers. *** RESULT: det 8/8 = 100%, honest Wilson [67.6%, 100%] +/-16.2pp on 8 MCQ leaves from ONE paper. gemini 15.5/23 = 69.6%, honest Wilson [49.1%, 84.4%] +/-17.6pp. *** THE FINDING: the ~77.4% headline is a MIXTURE of a 100% path and a 70% path, AND THE WEIGHT IS BACKWARDS. Per C11/DA23 MCQ schemes carry ZERO answer_points; the Gemini path carries 10,314 of 10,314. THE PATH RESPONSIBLE FOR EVERY ANSWER POINT MEASURES 69.6%, and the headline is pulled upward by a path carrying none of them. The weighting is an artefact of the fixture set (8 MCQ leaves vs 23 theory), not of the production corpus. THAT IS SECTION 14's ANTI-GOAL IN A FORM NOBODY HAD NAMED — a headline improving while the path doing the work goes unreported. *** TWO NUMBERS COMPUTED AND THEN REFUSED: det's 100% has a Wilson LOWER BOUND of 67.6% at n=8, so it is NOT evidence the det path is perfect; and POOLING the 10 repeats would report +/-6.0pp instead of +/-17.6pp, because they re-mark the SAME 31 leaves under an identical fingerprint and are NOT independent. Both naive figures are in the artifact TO BE REFUSED, NOT USED. *** CLAUSE 5 IS NOW HALF-MET HONESTLY RATHER THAN HALF-MET BY OMISSION: det coverage MET, Gemini accuracy MEASURED BUT NOT TO SCOPE (23 leaves against a clause asking for 10,314 answer points). Closing it needs #47, not more analysis of this corpus. DA49. *** MY ERROR THIS RUN, NOT BURIED: I merged #186 while its CI was still IN PROGRESS — the waiter had watched the PREVIOUS run and exited when a later push started a new one, and I read its exit as green. Changes were docs plus a workflow comment and develop was re-checked, BUT THE RULE IS GREEN-BEFORE-MERGE AND I BROKE IT. The waiter pattern must pin the RUN ID it is watching, not the PR.
