...
last_run_label: none (no measurement run this run)
last_run_headline: NO SWEEP THIS RUN. PR #97 LANDED (develop c34e35c), restoring FOUR human directives that were missing from develop entirely — 14:36:24 (the #93/#41 gate-9 PRE-AUTHORISATION, previously reachable only as a GitHub issue comment), 14:37:23, 14:41:54 and 15:00:17 — plus completion notes for A4, #88 q3 and #59 decision 3. All four verified present on develop after the merge. #37 (M1.2) bullet 1 IMPLEMENTED at zero spend: the positional fallback is deleted from normalize_extracted_answers, so an unmatched extracted id now KEEPS its own id instead of being handed the next unclaimed manifest id. Two harms closed: id_match_rate measured post-fallback COVERAGE (a guessed answer was stamped id_match='exact', so the metric could not fall below its 0.99 target however far extraction drifted), and one missing answer shifted every later unmatched answer by a slot, silently re-aligning a whole paper. The pre-existing test that asserted the fallback WORKS was INVERTED, not deleted; both new tests verified failing before the change and passing after.
...
spend_usd: 2.375268
in_the_middle_of: RUN 27 — NOTHING RUNNING, NO SPEND (ledger sum 2.375268, zero gemini calls this run). *** PR #97 LANDED; develop = c34e35c. The four previously-missing directives are verified present on develop. chore/accuracy-post-45-bookkeeping is fully merged and no longer needs carrying. *** #37 (M1.2) IS THE LIVE WORK. Branch feature/accuracy-37-delete-the-positional-fallback @ 6cecf50, pushed, NO PR. Bullet 1 DONE at zero spend (fallback deleted; tests inverted and proven to fail before / pass after). Bullet 2 PARTIALLY done — 'exact' is now true by construction rather than manufactured, but 'fuzzy' is STILL NEVER EMITTED because canonical matching makes '1 b' -> '1(b)' a fuzzy match recorded as exact; separating them needs raw-vs-canonical provenance out of extract_answers, which touches THREE web call sites (student.py:977, teacher.py:474, teacher.py:921). Bullet 3 NOT done — it re-derives the id_match_rate CI target and needs the sweep's number. *** COSTED PREFLIGHT IS POSTED TO #37 (A5 condition 1 satisfied) — $0.0625 per extract+mark pass MEASURED from ablation-2026-08-24-a's own arm over the same 11 golden cases, $0.125 central for two arms, $0.25 stop-and-ask (sized 2x because the #58 run came in at 1.94x its estimate). Ceiling $20.00, ledger 2.375268 of 25.00. NO SPEND YET. *** TWO THINGS I ASKED ON #37 AND WOULD LIKE ANSWERED BEFORE RUNNING IT: (a) A5 condition 3 says count id_positional_fallback WARNING lines in the SAME run, but THIS CHANGE DELETES THAT WARNING — the after-arm emits 0 by construction, so the count must come from the BEFORE arm running pre-change code. Confirm that satisfies the condition; I will not report a meaningless 0. (b) Widen #37 to include the exact/fuzzy plumbing, or take it as its own issue? I lean to its own issue. *** SWEEP DESIGN I INTEND TO USE unless told otherwise: run extraction ONCE and feed the identical raw extraction through old-normalise and new-normalise, then mark both. It isolates the code change (the #58 control proved a single-repeat A/B over a gemini path cannot separate change from churn), halves extraction spend, and makes the arms genuinely PAIRED so McNemar applies. *** STATED BEFORE THE READ, so it cannot be tuned after: id_match_rate WILL FALL, possibly a lot, and that is the metric starting to work rather than a regression. Bullet 3's new target must come from what genuine id agreement actually is — do NOT pick a target that happens to keep CI green; if the honest value is below anything shippable that is a FINDING to report. *** #93 STILL WAITS ONLY ON THE HUMAN'S WAIVER: branch feature/accuracy-93-... @ 70a6957, pushed, no PR, conditions 1 and 3 satisfied, sweep null by construction twice over (harness never invokes the det parser; golden set has zero 0625 p2 cases). Evidence at BUILD/accuracy-runs/sweep93-2026-08-25/ — 489 schemes, 41 p2 theory_core->mcq, 40 p3 theory_extended->theory_core, nothing else changed. #93 must land BEFORE any #57 preflight and BEFORE the split freeze. *** ALSO STILL THE HUMAN'S: the 0625 paper-3 CONSTANT (wrong, left wrong-but-annotated — separate issue?); item 6 whitespace fixtures (I recommend a metamorphic-only set); #58's q11b settling experiment (~$0.01, different design, proposed not run); the PARSED CORPUS regeneration (artifacts do not exist — 33 files not 250, no parse.log; must wait for #93 or it bakes in the bug); 17:36 items 2-4 (#88, sequenced behind #93); #59 decisions 1-2. *** #58 BULLET 4 STILL CORRECTLY UNTICKED (Fisher p=1.000, unresolved both directions; DO NOT re-run, MISSION 12.9).
---
