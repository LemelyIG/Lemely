...
spend_usd: 1.488057
in_the_middle_of: #45 (M2.2) round 2 IN FLIGHT on branch feature/accuracy-45-failure-reason-census-over-the-failing (3 commits, tip 360dfed). Round 1 gates ALL GREEN but review verdict BLOCKED with 3 MUST-FIX, all three independently re-verified by the orchestrator before acting. Work state (not a handle): census artifact exists at BUILD/accuracy-runs/census-2026-08-24-b/ (229 rows, denominator intact); remediation brief posted as issue comment 5393234337; NO PR. If this run died mid-workflow the workflow is UN-RUN not pending (MISSION 7) — re-run accuracy-issue-execute {issue:45}. *** THE 3 DEFECTS: (1) classify_profile_misconfiguration fires on any paper_type_by_number vs cover-text mismatch, but 0625 p3's THEORY_EXTENDED-vs-THEORY_CORE delta is CAUSALLY INERT — every PaperType consumer (det/metadata.py:114, det/parser.py:110, det/question_papers.py:280, core/loose_schemas.py:974) branches only on MCQ-vs-not, so 34/229 rows (15%) are attributed to a non-cause and D7 is UNDERSTATED at 128/229 when the true share is >=159/229 (69.4%). Fix = gate the label on a counterfactual (keep only when the corrected type changes the parse path AND changes the mismatch; 0625 p2->MCQ passes, p3 does not). (2) manifest.json + BLOCKERS.md section G escalate 0625 p3 to the human as a SECOND profiles.py bug instance and count it as a cause — a FALSE causal claim routed straight into #88's pending profiles.py/strata decisions. Fix = record it as a separate metadata defect explicitly ruled out as a cause. (3) genuine_mark_total_mismatch is a residual dumping ground with no positive evidence; 19 of its 27 rows have computed_total > maximum_mark, grossly (0606_s21_ms_21 108v80, 0606_s23_ms_12 116v80, 0580_s23_ms_43 139v130) — falsified on its face, traced to build_questions summing alternative/EITHER-OR branches additively, i.e. a PARSER defect M3 needs. Fix = split out mark_aggregation_overcount; residual must state what was ruled out; else UNCLASSIFIED. *** STILL DO NOT TOUCH lemely/io/det/profiles.py (question 2 on #88, the human's). ZERO SPEND, ledger 1.488057. *** #88 UNCHANGED: In progress, blocked, its 3 questions (2026-08-24T04:33Z) unanswered for a 4th run. *** origin/develop is 8 commits BEHIND local develop and origin/main lacks the spec file — needs a human to reconcile pushed refs before/alongside the PR.
---
