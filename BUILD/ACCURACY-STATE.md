...
branch: chore/accuracy-run-35-det-diagnosis (worktree HEAD); feature/accuracy-41-... @ 09314c7, feature/accuracy-37-... @ 8f3ba7f, feature/accuracy-93-... @ 70a6957 all pushed, no PRs
last_run_label: run-35-2026-08-26
last_run_headline: NO SPEND, READ-ONLY DIAGNOSIS. PR #109 MERGED. I TESTED MY OWN RUN-34 HYPOTHESIS AND IT IS FALSIFIED — recorded as a negative result so nobody re-tests it. The rows.py flush()/q_row_had_answer mechanism is NOT the cause of #95's deficits: parsing with escalate_on_mark_mismatch=False (only the escalation disabled, nothing written) shows 0625_s20_ms_31 has 41 leaves and ZERO gap leaves, 0625_w21_ms_32 has 43 leaves and ZERO gap leaves — every leaf's q.marks exactly matches its non-alternative answer points, so no marks are lost in propagation. WHAT THE DEFICITS ACTUALLY ARE: both papers have clean top-level ids (1-12) and plausible tariffs (1-4 marks), so the missing 4 and 7 marks are CONTENT NEVER CAPTURED — a table-selection/question-extraction question, not a marks bug. NEW DEFECT FOUND AND OPENED AS #110 (opened, NOT executed): the det parser emits DUPLICATE TOP-LEVEL QUESTION IDS — 0606_s23_ms_12 emits '9' twice, and 0580_s23_ms_22's sequence RESTARTS at '1','2','8'. Directly observed, not inferred. This breaks (paper_id, question_id) leaf identity — the exact property #105 hardened downstream — and is INDEPENDENT of mark-total reconciliation, so a paper can reconcile cleanly and still emit duplicate ids. ALSO NOTED AS SUSPICION NOT PROOF: 0606 carries single leaves worth 16 and 18 marks on an 80-mark Additional Maths paper (16+18=34 against a +36 overcount), consistent with total/summary rows ingested as leaves; confirming means reading those two leaves' text.
...
spend_usd: 2.659533
in_the_middle_of: RUN 35 — NOTHING RUNNING, NO SPEND (ledger 2.2803523500000105 this worktree; DA11 header sum 2.659533 of $25.00). PR #109 MERGED (4a41062). *** PR #111 IS OPEN AND NEEDS LANDING: base develop, branch chore/accuracy-run-35-det-diagnosis, BUILD/-ONLY. CI was still running at run end — poll 'gh pr checks 111' and merge once green. *** DO NOT RE-TEST THE flush()/q_row_had_answer HYPOTHESIS — run 35 tested it and FALSIFIED it (zero gap leaves on both deficit papers). Evidence at BUILD/accuracy-runs/det-mismatch-diagnosis-2026-08-26/manifest.json. Also do NOT re-run the #95 probe or #45's census; both are on disk. *** #110 OPENED, NOT EXECUTED: duplicate top-level question ids in the det parser, which break (paper_id, question_id) leaf identity independently of mark totals. Fixing it changes parse output so it is MARK-CHANGING and needs its own review and before/after sweep — it must not be moved to Ready without a human decision. *** THE PROGRAMME REMAINS FULLY HUMAN-BLOCKED on BUILD/ACCURACY-ASKS.md B1-B14; notification sent run 33. B2 (#93's waiver) is measurably the highest-leverage — #93 IS the paper_profile_misconfiguration bucket, 40 of the 229 det failures (17.5%), moving those schemes from #88's paid set to the free set before that preflight is re-costed. B1 (#41's A-mark silent case) holds the only finished branch. *** #41's branch is held UNMERGED on purpose and #41 MUST NOT BE CLOSED or its sweep pre-authorisation lapses. *** FOUR OPEN H ISSUES: #49, #51, #52, #55 — #52's three seed rulings and #51's labeller-B identity are owed by the human; #49 keeps the split NOT FROZEN.
---
