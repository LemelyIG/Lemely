...
branch: chore/accuracy-run-29-bookkeeping (worktree HEAD); feature/accuracy-37-... @ 8f3ba7f and feature/accuracy-93-... @ 70a6957 pushed, no PRs
last_run_label: run-29-2026-08-25
last_run_headline: NO SPEND (ledger unmoved at 2.659533). TWO PRs MERGED. #99: the leaf denominator is 9,464, NOT 12,358 — census_leaves.py filtered leaves on getattr(q,'sub_questions',None) and Question has no such field (child list is 'parts', loose_schemas.py:1008), so the filter was a NO-OP and 'LEAF questions' was every question at every depth; re-run of the fixed script over the same surviving 250 schemes reproduces the human's ratified figures exactly (12,358 flat = 2,894 parents + 9,464 leaves, 0 unbanded true leaves, 9 of 18 strata, bands unchanged). Error ran toward OVERSTATEMENT: the headline inflated the leaf corpus by 30.6%, so any n-floor or power figure against 12,358 is inflated. Recorded as DA12. #100 (#98): the #51/#52 machinery — hash-chained eval/rulings.jsonl writer, agreement+Wilson, pre-committed sample manifest — ZERO SPEND, no label touched, rulings.jsonl 0 bytes, membership null, #51/#52 still OPEN. Two adversarial review rounds caught FOUR real defects in my own machinery before merge, all the same family: leaves keyed on question_id alone (the D18 narrowed-denominator shape — same leaf across N papers collapsed to one, discarding every disagreement but one); the leaf representative was the EARLIEST record, so a labeller's correction was ignored; a resolution naming an unparked pending_id cleared it anyway, so count_pending() could read ZERO with real judgment questions open (the counter DA3 puts in front of the irreversible split freeze); and a negative fraction sliced from the end, turning a 10% sample into a 90% one. All fixed with fail-before regression tests.
...
spend_usd: 2.659533
in_the_middle_of: RUN 29 — NOTHING RUNNING, NO SPEND (ledger sum 2.659533 of $25.00). PR #99 and PR #100 both MERGED to develop (c0c9540, 3bb0cca), all CI green; #98 CLOSED and board Done. THIS RUN'S OWN BOOKKEEPING IS UNCOMMITTED-THEN-PUSHED ON chore/accuracy-run-29-bookkeeping — open its PR and land it. *** THREE RULINGS ASKED OF THE HUMAN ON #98, none of which I resolved by picking the convenient branch, ALL recorded in the inbox note on the 14:41:54 item: (a) the #51 stratification axis genuinely conflicts between DA2 (full 3-axis DA1 stratum) and the 14:41:54 item (mark band only) — I implemented mark-band-only with the axis as a one-line parameter; (b) I DEVIATED from DA2's literal sha256(relabel_salt || question_id) to hash the full (paper_id, question_id) leaf identity, because under the literal formula every paper's '1a' shares a rank and enters the sample AS A BLOCK — nothing has been sampled under either formula so no membership changes, but it is the human's decision that I altered; (c) the salt is cleartext and I corrected the module's false claim that it makes membership 'unpredictable in advance'. *** STILL BLOCKED ON THE HUMAN, unchanged from run 28: #37's two questions (bullet 3's CI target cannot be re-derived from a corpus scoring id_match_rate 1.0 in BOTH arms — that would be PICKING a number, not measuring one); #93's sweep waiver (branch pushed, no PR, sweep null by construction twice over, MUST land before any #57 preflight and before the split freeze); item 6 whitespace fixtures (I recommend a metamorphic-only set that load_golden_cases does not serve, rather than enlarging the accuracy denominator); #58's q11b settling experiment (~$0.01, a DIFFERENT design from the one authorised, proposed not run); the 0625 paper-3 constant; the PARSED CORPUS regeneration (artifacts do not exist — 33 files not 250, no parse.log — and it must wait for #93). *** 17:36 items 1-2 and 4 (#88) remain sequenced strictly behind #93. Item 3 is DONE (DA12). #59 decisions 1-2 unstarted. *** #58 BULLET 4 STILL CORRECTLY UNTICKED (Fisher p=1.000; DO NOT re-run, MISSION 12.9). *** FOUR OPEN H ISSUES: #49, #51, #52, #55 — the #98 machinery deliberately did NOT touch their work; only the tooling around it exists.
---
