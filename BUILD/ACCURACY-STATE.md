...
branch: chore/accuracy-c20-sweep (this)
last_run_label: c20-budget-bounded-sweep-2026-08-27
last_run_headline: RUN 58 — C20 AND C21 EXECUTED. *** C21: per_run_token_ceiling is now None; the committed $8.00 total_usd_ceiling is the SOLE guard (DA32). The 5M gitignored override is REMOVED, as C12 required for the dollar ceiling. It had been sized twice from a model DA26 records as falsified. *** C20: the sweep was built so the ESTIMATE WAS ALLOWED TO BE WRONG — the live ledger is re-read before every scheme and the run stops when spent+reserve would cross $3.00. THE CAP HELD: $2.8470 of $3.00, stopped before scheme 24 of 37. *** BUT THE SWEEP IS NOT REPORTABLE: 24 attempted, 12 parsed, 12 FAILED (50%). FOUR STRATA GOT ZERO SUCCESSES (0606/p1, 0606/p2, 0625/p4, 0625/p5), so the DA1 stratum coverage the spend was justified by was NOT delivered. Cost per SUCCESS $0.2372 = 3.39x the $0.07005 projection, extrapolating to $45.08 for 190. *** THE REAL FINDING, bigger than the cost question: the GEMINI PARSE FALLBACK — C11's designated path for every scheme det cannot handle — FAILS ON HALF OF THEM, SYSTEMATICALLY BY SIZE. 0580 9/11, 0625 3/9, 0606 0/4. Failures average 10.0 pages / 11637 chars vs 7.3 / 8608. NOT truncation (65536 limit, largest success 26571). NOT fully deterministic (the probe failed then succeeded unchanged). *** WHY IT WAS INVISIBLE: the 2026-08-26 run aborted at 6 of 190 ON COST and all 6 happened to succeed. AN ABORT IS NOT A PILOT — never read a rate off the prefix of a run cut short for an unrelated cause. *** NOT DIAGNOSED: reproducing one 0606 failure costs ~$0.15 of the $2.01 headroom, which is a SPEND DECISION. Opened as #166 (owner:human). The 12 parsed schemes are KEPT but MUST NOT be presented as stratum coverage. DA31, DA32.
...
in_the_middle_of: RUN 58 — branch chore/accuracy-c20-sweep carries DA31+DA32, the C15 rewrite in ACCURACY-ASKS, the journal entry and the sweep artifacts under BUILD/accuracy-runs/preflight-88-c20-2026-08-27/. Open its PR to develop and merge on green. *** #88 IS NO LONGER A STOP-AND-ASK ON COST — C20 answered C15 — but it is NOT resumable either: #166 must be answered first, because 50% of the remaining 190 will fail and 0606 will fail entirely. *** THE ONLY HUMAN ASKS LEFT: #166 (spend ~$0.15 to diagnose the parse failures? or retry the 12 failures? or proceed knowing the Gemini path closes ~half the gap) and C9 (#49 sign-off, #51 ONBOARDING only since C14 named labeller B, #52's three seed rulings, #55). *** AGENT WORK, all live and none gated on a human: #136 fix half (DA21 mechanisms A and B named at source, arithmetic closes exactly; C2 sequences #38's re-measurement behind it); then #95 -> #127. #112+#110 JOINTLY under C1's zero-spend deterministic corpus before/after. #39 under C3's same waiver (cost-changing: the REROUTE COUNT is the number to watch). #41 under B1. #161 under C13 (upper bound from the EXISTING 10-repeat A/A data, no spend). *** #57 bullet 1 is agent work and is NOT blocked on #49 — but its DA1 Gemini-path strata are still empty and four are now KNOWN-UNFILLABLE without #166. *** PR #159 (develop -> main) IS OPEN — DO NOT MERGE, MISSION 12.3 is human-only; its section-13 scorecard predates C11 and must be refreshed before anyone acts on it. *** #58 stays OPEN: bullet 1 unmet BY DESIGN (Fisher p=0.4737, MISSION 12.9 forbids re-running). *** LESSONS: an ABORT IS NOT A PILOT (a run stopped early for reason X leaves survivors that are not a random sample of what it never reached); make the BUDGET binding rather than the estimate, and the estimate is then allowed to be wrong; the ceiling guard reads only the LOCAL worktree ledger, so re-sum all four every run (5.993470).
---
