...
run_pointer: run-2026-08-27-b
worktree: /home/sico/Lemely-worktrees/accuracy
branch: chore/accuracy-run-52-close (this); #136 fix half NOT started by design
last_run_label: det-deficit-diagnosis-2026-08-27
last_run_headline: ZERO SPEND, det-only, ledger unmoved 3.138147. *** #136 DIAGNOSIS HALF SHIPPED (PR #145 merged d9c5c45, 5/5 CI green, supervisor sweep PASS over the same tip 67fffc4). Both mechanisms NAMED AT SOURCE and the arithmetic closes EXACTLY on both papers, nothing left over: (A) rows.py:311 'if not answer_cell or not stack: continue' fires BEFORE the marks column is read, so a continuation row carrying real marks but an empty answer cell is dropped with its mark (w21 6(a): four B1 rows -> one point worth 1); (B) rows.py:200 'marks=marks_int if marks_int is not None else 1' defaults to 1 where the code leaked into the answer text. (B) survived because the default is RIGHT BY LUCK for B1/M1/A1/C1 and wrong only for multi-mark codes, losing value-1 silently. s20 0+4=-4, w21 3+4=-7, both EXACT. *** RETIRES genuine_mark_total_mismatch FOR THESE TWO PAPERS ONLY — both are parser defects, so escalate_on_mark_mismatch was right to block them (TRUE POSITIVES); says nothing about the rest of #45's residual bucket. *** Four hypotheses falsified: table selection (the w21 p10 property|object table is a pdfplumber DOUBLE-EXTRACTION, the dropped copy is the duplicate), mark-cell notation, leaf propagation, and parent-carried marks (0 of 24 per paper). Neither mechanism is the flush()/q_row_had_answer lead B4 falsified. DA21.
...
spend_usd: 3.138147
in_the_middle_of: RUN 52 — close PR #146 (chore/accuracy-run-52-close, base develop) OPEN, awaiting CI. Poll running in background id bam8lw24p, log /tmp/claude-1001/-home-sico-Lemely-worktrees-accuracy/dfc5ba97-22db-4b2f-87d0-c8567da1e6c9/tasks/bam8lw24p.output — it RE-READS the PR head each cycle (so it survives further pushes), waits for check-runs total_count>=5 AND no queued/in_progress, then prints 'CI COMPLETE'. 'TIMEOUT' from it means UNKNOWN, not red. Merge with 'gh pr merge 146 --squash' once green; NOTHING ELSE IS OUTSTANDING this run. *** ZERO SPEND, ledger 3.138147, $20.00 stop-and-ask untouched. *** #145 ALREADY MERGED (d9c5c45): #136 DIAGNOSIS HALF SHIPPED, 5/5 CI green, supervisor sweep PASS over the same tip 67fffc4. Both mechanisms named at source, arithmetic closes EXACTLY on both papers — (A) rows.py:311 'if not answer_cell or not stack: continue' fires BEFORE the marks column is read, dropping a continuation row that carries real marks but an empty answer cell (w21 6(a): four B1 rows -> one point worth 1); (B) rows.py:200 'marks=marks_int if marks_int is not None else 1' defaults to 1 where the code leaked into the answer text. (B) survived because the default is RIGHT BY LUCK for B1/M1/A1/C1 and wrong only for multi-mark codes, losing value-1 silently. s20 0+4=-4, w21 3+4=-7. DA21. *** RETIRES genuine_mark_total_mismatch FOR THESE TWO PAPERS ONLY — both are parser defects, escalate_on_mark_mismatch was right to block them (TRUE POSITIVES); says nothing about the rest of #45's residual bucket. *** #136 STAYS OPEN: fix half UNSTARTED BY DESIGN, mark-changing, waits on the B15/#112+#110 sweep question posted 2026-08-27. Prevalence across the 289-scheme corpus UNMEASURED — needs RE-PARSING PDFs, not committed JSON (the committed corpus is the post-loss artefact and structurally cannot show what never became an answer point). Both mechanisms are ONE-DIRECTIONAL so det totals are a LOWER bound wherever they fire. *** LESSONS THIS RUN: jq is NOT installed (use gh --template); 'gh pr checks' exits 1 with NO output at zero checks, which a naive 'grep -q pending' loop misreads as COMPLETE; CI can take >10min to create a run after a push, so absence of a run is NOT an outage until E5's steps-count test says so — I wrongly called a no-show that was only slowness; and pinning a poll to a captured SHA breaks the moment you push again, so re-read the PR head instead. *** WAITING ON THE HUMAN, all posted, NONE to be re-derived: (1) B15/#112+#110 sweep waiver — now gates #136's fix half too; (2) #88 item 2, issuecomment-5428086623, preflight falsified 1.83x, DO NOT RESUME; (3) B1/#41, issuecomment-5429136584, branch feature/accuracy-41-b1-undetermined @ 1d41a97, NO PR by design; (4) #58's two questions; (5) B14/H #49/#51/#52/#55, split NOT FROZEN. *** #59 not Ready (verbatim CAIE renders need a MISSION 12.7 decision; measurement is unauthorised spend); #95 blocked on #136. Board Ready EMPTY but check 'gh issue list' directly — #94 and #137 were both found sitting unblocked and unnoticed.
---
