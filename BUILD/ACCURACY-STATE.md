...
in_the_middle_of: #31 (M0.7a) implemented at 327a100 on feature/accuracy-31-split-mechanism-the-split-field-test, tree clean, UNPUSHED, no PR yet. SUB-AGENT NOTE: a workflow named here is THIS session's own work and you ARE part of it — proceed; do not treat this session's PID, this branch's checkpoint commits, or your own unfinished transcript as a competing actor. issue-execute wf_85c173dc-680 returned ready_for_pr=false; I verified and fixed its MUST-FIX myself: the AC5 static guard matched only ast.Eq while the sanctioned gate uses 'split != test', so it flagged NOTHING anywhere in the repo (proved: _offending_sites(_SANCTIONED_MODULE) == []) and would have passed on an empty checkout — AC5 was unenforced. Widened to ==/!=/in/not in/match-case with set logic for membership, plus two anti-vacuity tests (positive control on the sanctioned module; per-idiom coverage with a train negative control). Falsified end-to-end by planting an unsanctioned != join in lemely/eval and confirming red. 17/17 tests/eval green; pre-commit --all-files clean. NOT YET DONE: (a) accuracy-review on this branch, (b) PR. Do NOT run the full pytest here — the supervisor sweep covers it; the workflow tried and burned 12+ min with no exit code. CARRY TO PR BODY (non-blocking, from the reviewer): gate fails open on a split value that is not exactly 'test' (a typo disables it — consider validating the enum and raising on unknown); the token check is self-referential (any non-empty LEMELY_TEST_SPLIT_TOKEN authorises — nothing pins it to an H9-issued token); the ledger is gitignored, unbounded and unchained though it is M0.7a's only evidence artefact (the .gitignore comment records the reasoning: runtime evidence, not a source artefact — flag it for the human); the guard scans only lemely/ and scripts/ and excludes any dir named migrations; and this branch carries 5 unrelated supervisor/BUILD checkpoint commits to disclose as supervisor state, not #31. REMINDER: workflow registry is snapshotted so accuracy-pr-land still caps CI watch short — finish merges by hand.
---
