...
run_pointer: run-2026-08-29-b
worktree: /home/sico/Lemely-worktrees/accuracy
branch: chore/accuracy-c24-two-seat (this)
last_run_label: two-seat-47-2026-08-29
last_run_headline: RUN 62 — C24 EXECUTED: #47 IS ALREADY TWO-SEAT CAPABLE IN CODE, AND H7's 10% SAMPLE CANNOT ESTABLISH THE CEILING IT EXISTS FOR. Zero spend. *** VERIFIED AT SOURCE, not assumed: labeller_id is SERVER-BOUND and never taken from the client (records.py:130-160); storage is per (paper_id, pass) so two labellers interleave in one hash chain and stay attributable; manifests are per (paper_id, labeller_id) so seats cannot overwrite each other; CROSS-LABELLER BLINDNESS IS ALREADY TESTED (test_pass2_context_for_a_different_labeller_does_not_see_labeller_a); per-mark-point agreement exists from #105/B12. C24 IS NOT A REWRITE. *** THE FINDING: #51 fixes H7's sample at 10% of labelled leaves = 30 of #47's 300, and states H7's purpose as supplying 'a ceiling on how good the pipeline can honestly be said to be'. At n=30 the 95% Wilson half-width is 9.7-13.9pp against 4.18pp for the accuracy it bounds. BUT THE WIDTH IS NOT THE FINDING: UNLESS B AGREES WITH A ON ALL 30 OF 30, H7's LOWER BOUND SITS BELOW THE PIPELINE'S OWN ACCURACY POINT ESTIMATE OF 83.7% (78.7% at 95% agreement, 74.4% at 90%). A ceiling whose interval admits values BENEATH the thing it bounds is not a ceiling. *** Matching 4.2pp needs 195 double-labelled leaves at 90% agreement = 65% OF THE CORPUS, against the 10% specified and the '1-2 h of B's time' #51 estimates. *** NO RECOMMENDATION GIVEN, DELIBERATELY: all three options spend Abdallah's hours and the cheapest (accept 10%, restate H7's purpose downward) costs the programme nothing and costs the agent nothing — exactly the shape of recommendation to distrust from an agent. *** STATED AGAINST ITSELF: agreement and accuracy are different quantities so matching precision is a design CHOICE not a law; the lower-bound point survives that objection, which is why it leads. *** ACTIONABLE HALF IS A DEADLINE NOT A DECISION: the relabel sample RULE and salt are already pre-committed with membership uncomputed until #47 finishes (DA2). Changing the overlap SHARE does not weaken that — the rule ranks all leaves and takes a prefix — BUT IT MUST CHANGE BEFORE #47 COMPLETES or the choice becomes visible to the sample it selects. *** #51 does NOT close: onboarding stays owed (C24/DA37). DA40.
...
in_the_middle_of: RUN 62 — branch chore/accuracy-c24-two-seat carries DA40, the journal entry and BUILD/accuracy-runs/two-seat-47-2026-08-29/. Open its PR to develop and merge on green. CUT THE NEXT BRANCH FROM origin/develop AFTER IT MERGES — every accuracy PR appends to DECISIONS/JOURNAL and edits the state header, so a branch cut before its predecessor squashes ALWAYS conflicts (cost 3 rebuilt PRs this session). Cherry-pick, never rebase; and cherry-pick --continue does NOT re-sign, check git cat-file commit HEAD | grep -c gpgsig. *** MERGED THIS SESSION: #167 (C20/C21, DA31-32), #169 (#136 four parser defects, DA34), #171 (#161 ratchet restatement, DA33), #173 (C22-C25 + #166 diagnosis, DA35-38), #174 (#110+#112, DA39). *** HUMAN ASKS NOW OPEN, none blocking agent work: (1) #52 — cases are pulled and posted, H8 log stays empty until you rule; (2) H7's overlap share — see DA40, and note the DEADLINE: it must change before #47 completes; (3) whether to ARM escalate_on_duplicate_leaf_ids — 21 schemes still collapse 36 leaves, and arming sends them to a fallback that DA35 measured failing ~50% of the time. *** NEXT AGENT WORK: #95 (fully unblocked by #136 and #110 — but READ THE COMMENT ON IT: 130 schemes' totals moved, alternative points are now is_alternative=True so fixtures differ in STRUCTURE, and the 36 remaining collapsed leaves WOULD BE BAKED IN). Then #39 (see the filtered-sum comment: its 'filtered = 0 of 575' does not hold on the wider corpus and bullet 3's tautology claim is population-dependent), #41 under B1, #127 folded into #95 by C5. *** #166 IS DIAGNOSED, NOT FIXED. A fix is NOT authorised (validating one needs spend). It has TWO parts — make Gemini set is_alternative on alternative routes, AND stop it emitting empty {} objects inside parts; addressing only one moves the failure rate without closing the gap. C25 condition 1 says 'fixed or formally re-scoped', which is the HUMAN's judgment. *** PR #159 (develop -> main) IS OPEN — DO NOT MERGE, MISSION 12.3 is human-only; its section-13 scorecard predates C11. *** LESSONS: when a fix removes a ONE-DIRECTIONAL error, the metric that error was inflating GETS WORSE before the corpus gets better — optimising it ships the wrong variant; read the fields the code already fills before spending to rediscover them; an ABORT IS NOT A PILOT; a premise I WROTE is not evidence; opposite-sign defects HIDE EACH OTHER; re-sum all four worktree ledgers every run (6.370451, headroom $1.629549).
---
