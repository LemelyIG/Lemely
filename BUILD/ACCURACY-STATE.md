...
spend_usd: 1.425511
in_the_middle_of: #40 (M1.5 coherence gate) — PR #83 OPEN, https://github.com/LemelyIG/Lemely/pull/83, base develop, CI RUNNING AND NOT YET DECIDED. *** DO NOT OPEN A SECOND PR — run 'gh pr list --head feature/accuracy-40-coherence-gate-awarded-marks-must' first. *** POLL WITH: 'gh pr checks 83' (Actions run 32656594089). As of the last poll: test(3.12) PASS 10m33s, pre-commit PASS 1m27s, web PASS 1m19s, test(3.13) and test(3.14) still PENDING at 0s — they are QUEUED BEHIND 3.12, not hung; the matrix runs serially so allow ~20-25 min from 3.12's finish. accuracy-pr-land wf_3dbdaa0d-468 reported conclusion=TIMEOUT at its 1200s watch window: that is NOT a pass and NOT a failure, it is UNKNOWN — do not record it as green and do not merge on it. mergeable=MERGEABLE mergeStateStatus=UNSTABLE (unstable only because of the two pending jobs). IF GREEN: merge with 'gh pr merge 83 --squash --delete-branch' (base develop, NEVER main) then 'accuracy_board.py done 40'. IF RED: route to accuracy-gate-triage, do NOT re-run hoping for a different answer. *** NEVER pass allow_merge_without_ci — the waiver is permanently void, Actions is fixed and genuinely running. *** BRANCH CONTENT: 5 commits on 941b2a7 — ac35be7+902b25a (docs-only #36 blocker records, ride in by design, name them in the PR body), 396b350 (#40 feature), a7b99e3 (chore), c116140 (repair: range rule + type-scoped exemption + honest bullet 4), c6e4cc1 + 17c741d (DA10 corrections). All signed, tree clean, pre-commit --all-files green, 'scripts/check.sh --fast tests/test_correction_ai.py tests/test_accuracy_harness.py tests/eval/' ALL GATES PASSED (fast mode = NOT a merge signal, coverage unmeasured). Supervisor sweep covers d9adbea only, NOT this branch tip — do not claim section 9.3 green from it. REVIEW: accuracy-review wf_e14b49f7-643 returned block on ONE docs-only MUST-FIX (DA10 false prevalence), now FIXED and independently re-verified clear by an accuracy-reviewer agent; code was clean on all six dimensions and is UNCHANGED since. *** ACCEPTANCE BULLET 4 IS UNMET, HONESTLY: coherence_trigger_rate has no corpus measurement. The REAL obstacle (recorded in DA10, replacing an earlier FALSE 'the data is absent' claim): the 181 cached AIMarkResponse payloads under .lemely-cache/gemini/ carry matched_point_ids but NO question/answer_points linkage and no manifest, and gemini.py offers no zero-spend cache-hit-only replay mode. Closing it needs either a cache-replay mode or matched_point_ids plumbed into EvalRecord — TRACK THIS BEFORE #40 IS CONSIDERED CLOSED. *** DA10 PREVALENCE, corrected twice, use these numbers: over 13 scheme files (11 tests/golden mark_scheme.json + 2 Sources ms json) = 152 leaves, 53 have empty answer_points, breakdown 48 mcq / 3 explanation / 1 list / 1 multi_step. MCQ IS EXEMPT, so 48/53 stay exempt and the type-scoped exemption newly covers only 5/152 (3.3%), NOT 53/152. Over the 11 golden files alone (71 leaves) all 8 empty-points leaves are mcq, so on the evaluation corpus proper the exemption changes NOTHING. The original 'all 53 are non-exempt' claim was MY error — read off a histogram over ALL leaves instead of the empty subset, then passed downstream as computed fact. A follow-up verifier then claimed I over-credited the diff for the dangling-id check; I CHECKED AND REJECTED THAT — it compared the repair commit to its parent, but 'git show develop:lemely/io/correction_ai.py | grep -c _check_coherence' is 0, so the whole gate including the dangling check IS new in develop..branch. *** #36 (M1.1) STILL PARKED on the unanswered A/B/C acceptance-bullet conflict — BLOCKERS.md section B, branch feature/accuracy-36-... complete at 2cae804, do not restart it from scratch. #28 (M0.4) STILL NOT AUTHORISED — do not start it, not even its preflight. *** TRAP LEARNED THIS RUN: 'accuracy_board.py block' WRITES TO BUILD/BLOCKERS.md as a side effect, which dirties the tree and can abort a git checkout mid-sequence. Also 'git cherry-pick' has NO -q flag — passing it makes the command fail while a 'set -e' chain can still run on, which is how conflict markers once got committed to develop; always grep the file for '<<<<<<<' after any pop/merge.
---
