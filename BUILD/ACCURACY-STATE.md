...
last_run_label: control-58-2026-08-25
last_run_headline: #58 CONTROL ARM (inbox item 5): same-input control scored the SAME 31 leaves the reorder arm scored, 0 differing; a duplicate concurrent copy independently scored 0/31 again, so 0/62 combined, Wilson 95% CI [0.0%, 5.8%], 107 calls, cache_hit=False on every one. THE DIRECTIVE'S PREMISE IS FALSIFIED: there is no 0.1565 to subtract. Under p=0.1565 we expect 4.9 differing leaves per 31; observed 0 across 62, probability 2.6e-05. That figure was measured END TO END (extract+mark, transcription included); this design feeds ground-truth answers straight in with NO extraction step, so it measures MARKING-ONLY churn — a different quantity. BULLET 4 STAYS UNTICKED: the control removes the 'just gemini churn' explanation, but reorder 1/31 vs control 0/62 is Fisher p=1.000, so the design cannot call it a real defect either. UNDERPOWERED IN BOTH DIRECTIONS. Cost $0.287153 (one run costs ~$0.06) because it RAN TWICE CONCURRENTLY — my error, written up in BUILD/accuracy-runs/control-58-2026-08-25/NOTES.md.
...
spend_usd: 2.367788
in_the_middle_of: RUN 24 — NOTHING RUNNING IN THE BACKGROUND, no spend in flight. *** PR #90 IS MERGED (2026-08-25T15:49:51Z, squash, base develop, all 5 CI jobs green on run 32864942585). develop = d2f3662 and now carries lemely/accuracy/metamorphic.py, its tests, and the #58 metamorphic + control evidence. #58 is CORRECTLY STILL OPEN — the PR said 'Closes nothing', so nothing auto-closed with bullet 4 unmet. *** #58 BULLET 4 REMAINS UNTICKED, deliberately, and the human has the full reasoning on the issue: the authorised control arm ran and FALSIFIED its own premise (marking-only churn 0/62, not 0.1565 — that figure is end-to-end and does not transfer), but reorder 1/31 vs control 0/62 is Fisher p=1.000, so the violation is unresolved in BOTH directions. DO NOT re-run the sweep (MISSION 12.9). The settling experiment — re-mark q11b ALONE perturbed vs unperturbed ~10x each, ~$0.01 — is a DIFFERENT design from the one authorised, is PROPOSED ON #58, and NEEDS THE HUMAN'S WORD FIRST. *** #93 IS IMPLEMENTED AND PUSHED BUT NOT LANDED: branch feature/accuracy-93-profiles-paper-type-lookup-order at e82199f, off develop 10f76e8, NO PR OPEN. Zero spend. It fixes profiles.py:50 (0625 p2 THEORY_CORE -> MCQ) AND the lookup ORDER so cover text outranks the number map. MEASURED, not inferred: 0625_s23_ms_22.pdf went from ParseError (parsed 12, expected 40) to a clean mcq/mcq parse, 40 questions, 40/40 reconciled — these papers previously did not parse AT ALL. Over #45's census of 229 det failures, 40 rows are 0625 p2 and 39 OF THE 40 NOW PARSE CLEANLY (17% of the whole det-failure set moved from the paid Gemini path to the free det path). The 1 residual is 0625_s24_ms_21, which is #94's withdrawn-question defect and is NOT fixed here. *** BLOCKING QUESTION POSTED ON #93 — two human directives conflict and I did not pick the cheap one: A2 (12:03) says this is mark-changing and needs its OWN before/after sweep before merge; #88 item 1 (17:36) says 'FIX FIRST, ZERO SPEND ... Land the fix, THEN re-run the costed preflight'. Landing without a sweep is the cheaper reading, so it waits. ANSWER NEEDED: does #88 item 1 supersede A2's sweep gate, or does the sweep still gate the merge? *** ALSO STILL OPEN, item 6 of the 17:36 directive: may whitespace golden fixtures be ordinary golden cases (joining the accuracy denominator — MISSION 12.2/12.5, and #49 is reopened with the split NOT frozen), or a metamorphic-only fixture set load_golden_cases does not serve? I recommend the second; NOT implemented pending the answer. *** WHEN #93 LANDS, #88's paid-parse preflight MUST BE RE-COSTED — its denominator had 39 schemes on the wrong side. *** STILL THE HUMAN'S, untouched: 17:36 items 2-4 (#88 spend, the ratified 9,464 leaf count, the lemely.toml token-ceiling raise), A5 (#37 sweep), the #59 handwriting corpus. *** THIS BRANCH (chore/accuracy-run-24-bookkeeping) holds only this state update and must be landed or folded into the next PR — it is off develop d2f3662, so it does NOT carry the E4 loss hazard. *** UNPUSHED: chore/accuracy-post-45-bookkeeping holds run-21's E4 remedy (3 commits, local only).
---
