...
branch: chore/accuracy-88-item4-token-ceiling @ 14e7f6b -> PR #133 OPEN (no CI); chore/accuracy-run-40-close -> PR #132 OPEN (no CI); feature/accuracy-41-... @ 09314c7 and feature/accuracy-37-... @ 8f3ba7f pushed, no PRs
last_run_label: run-42-2026-08-26
last_run_headline: ZERO SPEND, no Gemini calls. #88 ITEM 4 DONE (per_run_token_ceiling 2M -> 5M, DA13) and ITEM 2 RE-COSTED FROM MEASUREMENT, posted to #88 before any spend. The no-linear-scaling warning was RIGHT and is now measured: paid set 229 -> 190 (exact subset, 39 removed by #93) but pages only 2084 -> 1967, so mean pages/scheme ROSE 9.1 -> 10.35 (+13.7%). Largest scenario 3.60M tokens / $7.26 on the per-PAGE model, which governs because the failing papers are +23.4% longer than the proxy population the per-scheme model was measured on. *** I CORRECTED MY OWN PREFLIGHT within minutes of posting it: parse-mark-schemes has NO --cache-mode flag, so the 'bypass' condition I restated is not expressible; it runs read_write (gemini.py:158), refresh is structurally unreachable, and bypass was never right for a corpus build with no repeats. *** BLOCKER L: GITHUB ACTIONS STOPPED PROVISIONING at ~13:03Z. PRs #132 and #133 both unmergeable. The 2026-08-23 CI waiver is VOID and was NOT invoked.
...
spend_usd: 2.704228
in_the_middle_of: RUN 42 — NOTHING RUNNING IN THE BACKGROUND. NO GEMINI CALLS, NO SPEND. Ledger unmoved: this worktree 2.2997574, programme-wide header 2.704228 (DA11 basis). *** DONE THIS RUN: #88 ITEM 4 — per_run_token_ceiling raised 2,000,000 -> 5,000,000 in lemely.toml AND VERIFIED LOADED (load_settings reports 5000000). lemely.toml IS GITIGNORED (.gitignore:47) so the raise is LOCAL TO THIS WORKTREE ONLY — it does not survive worktree deletion and is invisible to CI. DA13 in BUILD/DECISIONS.md is the ONLY durable record and says so. *** #88 ITEM 2 PREFLIGHT POSTED (issuecomment-5427654104) BEFORE ANY SPEND, re-measured not scaled, script+artifact at BUILD/accuracy-runs/preflight-88-2026-08-26/. Numbers, do not re-derive: 190 failing (EXACT SUBSET of old 229, 39 removed by #93), 1967 pages, mean 10.35 pages/scheme vs old basis 9.1. Scenarios: per-scheme median 2.67M/$4.92, per-scheme mean 2.98M/$5.70, per-PAGE median 3.42M/$6.82, per-PAGE MEAN 3.60M/$7.26 <- LARGEST, and the per-page rows GOVERN because the failing papers are +23.4% longer (10.35pp) than the 210-scheme proxy population (8.39pp). Projected total after the job $9.53-$9.96, under the $20.00 stop-and-ask. *** CORRECTION I POSTED AGAINST MYSELF (issuecomment-5427672112): parse-mark-schemes has NO --cache-mode flag (cli.py:200-213) and builds GeminiClient(settings) bare (cli.py:234), so it runs default read_write (gemini.py:158). E2's 'bypass' is NOT expressible on this command; refresh is structurally UNREACHABLE, which is what E2 actually exists to prevent; and bypass was never right for a corpus build with no repeats. Cache hits only push actual spend BELOW the estimate. *** THE SWEEP ITSELF IS NOT RUN. It is authorised (17:36 item 2, sequenced behind item 1 = #93, which landed) and fully preflighted, so it is the NEXT RUN'S FIRST ACTION — no further gate. It will outlive a run: start it in the BACKGROUND and record the command AND log path here. TRAP: 'pgrep -f parse-mark-schemes' matches its own shell command and falsely reports RUNNING — use log mtime. The $0.287 duplicate-run incident happened exactly this way. Write to a NEW --output-root, NOT corpus/ (committing Gemini-parsed schemes is a fresh B8-style human decision). *** BLOCKER L IS THE REASON NOTHING LANDED: GITHUB ACTIONS STOPPED PROVISIONING at ~13:03Z. Run 32971934818 queued 2h47m with an EMPTY JOB LIST; PR #133 opened ~15:40Z produced NO RUN AT ALL; newest run repo-wide is still 13:03Z. ci.yml has NO paths filter so a BUILD/-only diff cannot explain it. Runners worked today (4 green runs 11:35-12:25Z), so this is a NEW stall, and it is NOT the same as #132's one real flake (test 3.14, docker port 54322 already in use, postgres never started) which I correctly answered with 'gh run rerun --failed' — that rerun IS the stuck run. *** THE 2026-08-23 CI WAIVER IS VOID AND I DID NOT INVOKE IT: by its own terms it lapsed the moment Actions could provision a runner, and it did. A new outage does not revive a lapsed waiver. NEITHER PR IS MERGED. *** NEXT RUN: re-verify from the API FIRST (gh api actions/runs, then the jobs endpoint of the newest run) — ONLY a non-zero job/steps count means fixed; a newer run id proves nothing (falsified 2026-08-23). If green: merge #132 THEN #133; they conflict on BUILD/ACCURACY-STATE.md and the fix is MERGE, NOT REBASE (rebase strips signatures here). Then run the #88 item-2 sweep. *** STILL ANSWERED-BUT-UNACTIONED: B1, B4, B5, B6, B12, B13, B15. #49 keeps the split NOT FROZEN. Board Ready EMPTY.
---
