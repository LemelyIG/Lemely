...
spend_usd: 1.425511
in_the_middle_of: #28 (M0.4 oracle-transcription ablation) — SWEEP LAUNCHING NOW, run_label ablation-2026-08-24-a, accuracy-measure mode=ablation cache_bypass=true. *** THIS SPENDS REAL MONEY. DO NOT START A SECOND COPY. *** If a later run finds no result recorded, check BUILD/accuracy-runs/ablation-2026-08-24-a/ and the ledger (spend_usd) BEFORE re-running — a partial sweep has already cost money and re-running doubles it. *** AUTHORISATION IS SATISFIED: human authorised #28 on 2026-08-24T01:14:03 with no per-item cap, MISSION 10 controls governing. The required costed preflight IS POSTED as a comment on issue #28 (that ordering is the control, not a formality — #27 was executed before its authorisation and the human rebuked it). PREFLIGHT NUMBERS, verified by me at source not taken on trust: projected ~$0.27 (range $0.10-$1.30) on the CORRECTED basis — #26/M0.2 is CLOSED and lemely/io/gemini.py now carries GA rates ($0.30/$2.50 per 1M) with a comment documenting the 2-4x understatement it fixed, so NEW spend is priced correctly and only the pre-M0.2 historical ledger needed the retrospective correction (1.425511 raw -> ~$3.83 corrected). Projected total after ~$4.10, which does NOT trip the $20.00 stop-and-ask, nowhere near the $25.00 ceiling. Cache posture verified IN CODE not from the E2 note: gemini.py:357 gates the read on cache_mode=='read_write' and :425 gates the write on ('read_write','refresh'), so bypass skips BOTH and is side-effect-free; NEVER use refresh, it would overwrite the shared cache on every repeat. *** DESIGN CORRECTION FROM THE PREFLIGHT, worth keeping: #28's '2x2' is the per-leaf OUTCOME CROSS-TAB (both-correct / extraction-attributable / marking-attributable / masked), NOT four run arms. There are exactly TWO arms (harness.py:670-674): extract+mark (real vision) and oracle+mark (ground-truth text injected; currently dead code since all 11 fixtures ship scan.pdf). Corpus from disk: 11 fixture dirs / 5 distinct papers / 71 fixture-records / 31 DISTINCT LEAVES; ~137 Gemini calls. *** EXPECT 'NOT REPORTABLE — UNDERPOWERED' AND TREAT IT AS A SUCCESS: 31 distinct leaves against the 219-leaf McNemar floor is ~14% of required n, and the published A/A churn floor means any gemini-path delta under 11.6pp is WITHIN NOISE (det path floor is exactly 0.0). MISSION 12.9 FORBIDS re-running at higher n to chase significance — if it comes back underpowered, publish that and stop. Do NOT let a disappointing number get re-run, re-scoped, or quietly dropped. *** UNKNOWNS the preflight flagged rather than guessed: the det-vs-gemini parse-path split cannot be derived statically from mark_scheme.json (no scheme_format populated) and must be read from EvalRecord.parse_path at run time; per-call token counts are planning estimates, not verified counts. *** #57 (M0.7b) STILL QUEUED AND UNSTARTED — also authorised (option A: mark-scheme parsing pass to reach ~300 leaves). Its ordering: DETERMINISTIC MCQ schemes FIRST at zero cost to see how far they get, THEN a costed preflight posted to the issue before any Gemini theory-scheme spend, same $20.00 stop-and-ask. Do NOT freeze #57 over the 71 golden leaves (drop-only makes it irreversible). I intend to make the parsing pass ITS OWN ISSUE rather than bury it in #57, since #45 (M2.2 census) needs the same parsed schemes and would otherwise duplicate the spend. *** LANDED THIS SESSION: #44 (5f7cde7), #40 (d203916), #46 (ba92be3), #36 (5cc58cb). Board M1 Done=3, M2 Done=2. Inbox 0 unchecked, all five 2026-08-24 directives acted on. *** #36 CARRIES ONE STRICT XFAIL: tests/eval/test_analyses.py::test_m0_baseline_band_distribution_reported_honestly asserts the DESIRED non-degenerate band distribution behind xfail(strict=True), because acceptance bullet 6 is UNMET — all 71 rows of the saved A/A-floor corpus carry extraction_conf: null (that corpus predates extraction-confidence capture), so the propagation path is never exercised there. strict=True means it FAILS LOUDLY the moment a baseline re-run with per-answer extraction confidence closes the gap — at which point remove the xfail and assert the real distribution. #36 also newly allows a correct MCQ to be flagged for review, which can RAISE review_rate; unmeasured for the same reason, and covered by the human's gate-9 scoping (routing, not marks) to fold into the M1 milestone-close sweep.
---
