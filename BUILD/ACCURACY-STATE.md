...
spend_usd: 1.425511
in_the_middle_of: #27 (M0.3 A/A churn floor) MEASURED AND COMMITTED; adversarial review IN FLIGHT. Sweep aa-floor-2026-08-23-a is DONE and REPORTABLE — do NOT re-run it (costs money; MISSION 12.9 forbids chasing significance). THE FLOOR, all four figures independently re-derived by the orchestrator with the production _group_by_leaf/_collapse_leaf_group, not taken from the agents: pairwise leaf churn 11.61% (162/1395 over C(10,2)=45 pairs x 31 leaves), per-pair range 0.0-19.35%, leaves ever churned 9/31 Wilson [16.1%,46.6%], det path EXACTLY 0.0 (0/360), gemini 15.65% (162/1035). RULE: gemini-path A/B deltas under 11.6pp are WITHIN NOISE; det floor is 0.0 so det-only arms are not subject to the pooled figure. Cost 0.958711 vs 1.58 preflight; spend_usd now 1.425511 of 25.00. Commits on feature/accuracy-27-publish-the-a-a-churn-floor: b364bf7 (start) + the floor commit (BUILD/ACCURACY-REPORT.md NEW, DA9+DA9a in DECISIONS.md, evidence under BUILD/accuracy-runs/) + 840f0e0 (ruff exclude for BUILD/accuracy-runs — evidence must never be reformatted) + 1abde1f (correction). scripts/check.sh --fast tests/eval/ ALL GREEN (my first attempt named tests/test_eval_analyses.py which does NOT exist — the real path is tests/eval/; a bad path shows up as a pytest FAIL with an EMPTY log, do not misread that as a code failure). TWO CORRECTIONS THIS RUN, both already written into DA9a: (1) the standing 'review-rate gate has ZERO test coverage' claim is WRONG — lemely/eval/review_gate.evaluate_review_rate_gate has 13 tests in tests/eval/test_review_gate.py; only the WRAPPER scripts/check_review_rate_gate.py is untested, and the real defect is wiring (reads a saved golden run, still prints 0.2903) not missing tests. (2) BIG ONE FOR #36: review_rate over the 10 live repeats is mean 32.58%, range 29.03-41.94% — the committed constant 29.03% is the BOTTOM of the range, so arming min(10%, last_merged) against it gates the build on a figure UNCHANGED CODE EXCEEDS 7 TIMES IN 10. Restate it distribution-aware BEFORE arming. Also DA8's 77.4% is the modal draw of a 67.7-80.6% spread; prefer pooled 75.8% (235/310). NEXT: poll review workflow wf_add230fe-ea1 (task we4xskj9r) at /home/sico/.claude/projects/-home-sico-Lemely-worktrees-accuracy/976edbed-b4e6-4102-ac9a-397a3ea97d07/subagents/workflows/wf_add230fe-ea1/journal.jsonl — DO NOT LAUNCH A SECOND COPY; then post the finish comment and hand to accuracy-pr-land (base develop, NEVER main). CI billing block UNCHANGED (run 32632548137 10:00Z still steps=0), so the human's standing waiver applies. Branch carries the 3 E4 chore commits by design — say so in the PR body.
---
