...
spend_usd: 1.488057
in_the_middle_of: RUN 13 WITH NO HUMAN INPUT (14th consecutive). Inbox: no unchecked items. Board `next`: NOTHING READY. Tree clean; NOTHING running in the background (both census jobs finished; findings are durable in BLOCKERS). Supervisor sweep PASSED over f00dfb2 -- it does NOT cover 13d1232 or this run tip. NO SPEND; ledger unmoved at 1.488057. *** FINDING 1 -- THE #28 BLOCKER WAS STALE FOR A DAY (committed 13d1232, BLOCKERS "#28" RESOLVED note). BLOCKERS said "do NOT re-run the sweep until a mechanism exists to force oracle+mark over cases that already have a scan_path". PR #87 (MERGED 2026-08-24T03:31Z, on develop, in this branch history) IS that mechanism: harness.py:626 takes `arm`; :705-710 sets case_arm = arm UNIFORMLY overriding scan_path so the oracle bypass at :733 IS reachable; :678-684 raises up front if arm=extract+mark and a case lacks scan_path; cli.py:1013-1025 exposes --arm wired at :1081. Tested: tests/test_accuracy_harness.py:313 and :352. The blocker release condition IS MET. BUT #28 is CLOSED WITH EVERY ACCEPTANCE BOX UNTICKED and the only ablation ever run is published NOT REPORTABLE -- M0.4 was closed on the MECHANISM; THE 2x2 HAS NEVER BEEN PRODUCED. Re-running is NOT MISSION 12.9 significance-chasing (that run was not underpowered -- one arm did not execute at all, so a re-run is the FIRST correct execution; no larger n proposed). Board item DELIBERATELY NOT force-moved (sits in Backlog while CLOSED; M0 shows 1 Backlog / 10 Done though all 11 M0 issues are CLOSED). *** FINDING 2 -- #39 BULLETS 5 AND 9 ARE NOT JOINTLY SATISFIABLE, AND BULLET 6 DOES NOT REPRODUCE (BLOCKERS section I; commented on #39). Measured with scripts/accuracy_glyph_census.py over 478/479 schemes = 22,825 answer points + all 10 golden fixtures, $0. Bullet 5 VERBATIM (naive adjacent numeric pair) fires 426 pts; a hardened rule (exactly two numeric segments) fires 96 across 73 schemes; naive over-fires by 330 pts (3.4x) on histogram freq densities, Venn cell counts, matrix blocks and guidance text. BUT hardening breaks a CONFIRMED real case: golden 0625_s20_qp_31 11b/p2 `(V\\ns\\n=) (64 \\n 240) \\n 960` = (64 x 240)/960 = 16 matching p3 "16 (V)" -- naive fires, strict MISSES (non-numeric fragments). Its TWO newlines encode TWO lost operators (implicit multiply in the numerator AND the bar), so bullet 5 phrasing does not even describe the case the issue is built on. Second golden hit 12c/p2 `36\\n8` = 36/8 = 4.5 matching p3 "4.5 (mg)". CONCLUSION: no threshold on a newline-shape rule gives zero FPs (bullet 9) while catching the confirmed case -- the signal is PDF geometry (the horizontal rule), linearised away before the string exists; fixing it means consulting layout during parsing, NOT post-hoc regex. NOT proposed, NOT implemented. BULLET 6: searching U+E000-U+F8FF over all 22,825 det points and all 10 fixtures returns ZERO hits, not the "6 instances" the bullet asserts -- needs its evidence re-established or re-scoped. BULLET 7 STANDS: 7,963/22,825 points contain a newline (34.9%) vs the issue 272/1,000 (27.2%). DO NOT resolve any of this by loosening bullet 9. *** THE ASKS NOW STAND AT ELEVEN, ALL PURE DECISIONS, NONE SPENDING: (1) #58 bullet 4 -- authorise ~USD 0.144 or RETIRE as #40 was; the ONLY thing between PR #90 (OPEN, MERGEABLE) and merge. (2) #88 q2 profiles.py:50. (3) #88 q1/q3 (H4/#49). (4) #45 3-option design question. (5) #37 gate-9 sweep or explicit waiver. (6) #38 (a) land on the deterministic proof waiving a provably-blind gate 9, (b) regenerate golden mark_scheme.json through the det parser (MISSION 12.2/12.5; breaks comparability with every published figure), or (c) block until (b). (7) #38 bullet 2 needs RE-STATING -- measured harmful on 56% of affected papers (78 over-sum vs 75 under-sum + 24 exact/672 pts). (8) #39 bullet 4 NOT IMPLEMENTABLE -- is_excerpt landed only on harness.py:75, not on MarkSchemeMetadata; bullets 2 and 4 in direct tension. (9) #28 -- does a spend authorisation granted while an issue was OPEN survive that issue being CLOSED? (a) stands/reopen+run, (b) lapsed/formally retire the boxes as unmet in #40 bullet-4 style NEVER silently ticked, (c) defer. Recommend (a); NOT acted on -- choosing the reading that unlocks spending is exactly the #27 rebuke. (10) NEW: #39 bullet 5 needs re-stating and bullet 9 must NOT be loosened to accommodate it. (11) NEW: #39 bullet 6 evidence re-established or re-scoped. ALSO STANDING: #41 bullet 2 is a MISSION 12.7 human call on CAIE marking principles. *** #38/#39/#41 all remain MARK-CHANGING; none started; no branch for any. NINE COMMITS UNPUSHED: origin/feature/accuracy-45-... at 8847f59; local ahead by 20908e1/377cb5f/5060a94/76f01a0/c617489/a253231/f00dfb2/13d1232 + this one. Disk-only; not pushed per CLAUDE.md.
---
