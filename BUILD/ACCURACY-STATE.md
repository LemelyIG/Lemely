...
spend_usd: 2.659533
in_the_middle_of: RUN 36 — NOTHING RUNNING, NO SPEND (ledger unmoved: 2.2803523500000105 this worktree; DA11 header sum 2.659533 of $25.00). PR #111 MERGED (b644789). *** #112 OPENED, NOT EXECUTED: det alternative-branch markers followed by a newline are missed (rows.py:316-330), so mutually exclusive solution routes are SUMMED. Mark-changing — must NOT be moved to Ready without a human decision. Filed as B15 in ACCURACY-ASKS.md with three options and a self-flagged cheaper-branch recommendation. Overlaps #110 on 0606_s23_ms_12; if either is authorised, sweep them jointly rather than twice. *** DO NOT RE-TEST, both now falsified by the runs that raised them: run 35's flush()/q_row_had_answer hypothesis (zero gap leaves on both deficit papers) and run 36's total/summary-rows suspicion (both leaves are genuine content; page-break-split question 9 with alternative routes collapsed). Evidence on disk at BUILD/accuracy-runs/det-mismatch-diagnosis-2026-08-26/ and BUILD/accuracy-runs/alt-marker-diagnosis-2026-08-26/. Also still on disk, do not re-run: the #95 probe, #45's census, and /tmp/acc57-full/out's 250 parsed schemes. *** TRAP RECORDED FOR #88: prevalence of a defect that triggers escalation is ZERO BY CONSTRUCTION over outputs/schemes/ or the parsed corpus, because affected schemes are escalated out of it. Measure such defects over the FAILURE set. *** THE PROGRAMME REMAINS FULLY HUMAN-BLOCKED on BUILD/ACCURACY-ASKS.md B1-B15. B2 (#93's waiver) is still the highest-leverage — it unblocks #88 -> #57 -> the parsed corpus. B1 (#41's A-mark silent case) holds the only finished branch. *** #41's branch is held UNMERGED on purpose and #41 MUST NOT BE CLOSED or its sweep pre-authorisation lapses. *** FOUR OPEN H ISSUES: #49, #51, #52, #55 — #52's three seed rulings and #51's labeller-B identity are owed by the human; #49 keeps the split NOT FROZEN.
---
