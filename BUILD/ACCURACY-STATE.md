...
last_run_headline: #58 bullet 4 RAN AND COMPLETED, cache genuinely bypassed (169/169 gemini_call lines cache_hit=False, 0 retries/429s). 213 property checks over 11 golden cases: 87 held, 1 violated, 125 skipped. Per property: rename_mark_point_ids 57 held / 0 violated / 14 skipped; reorder_mark_points 30 held / 1 violated / 40 skipped; normalise_answer_whitespace 0 held / 0 violated / 71 SKIPPED — the whitespace property got ZERO live coverage because no golden answer has collapsible whitespace, exactly as the preflight predicted, and no spend can buy that coverage. THE ONE VIOLATION IS NOT ATTRIBUTABLE: reorder_mark_points on 0625_s20_qp_31_theory q11b, baseline 1 mark -> perturbed 2. q11b is a 3-mark 'calculation' with M/A math marks and 3 answer_points, i.e. NON-MCQ, so it was marked on the GEMINI path (correct_paper routes det to MCQ only). Pre-registered caveat applies: gemini A/A churn is 0.1565 and this design marks each leaf once per arm with NO same-input control, so this is consistent with ordinary gemini nondeterminism and is NOT evidence of a reorder defect. A det violation would have been an existence proof; this is not one. Zero det-path violations were observed. HONEST COST OVERRUN: the run made 169 gemini calls (projected 168) and cost $0.279397, which is 1.29x the preflight's $0.216 stop-and-ask and 1.94x its $0.144 central estimate. The stop-and-ask never fired because the run was unattended and nothing was watching the ledger mid-run; the earlier 'tracking to roughly $0.10' note in this file was wrong (it divided by correct_paper invocations, 28, not gemini calls, 169). Ledger 1.390218 -> 1.671560.
...
spend_usd: 1.671560
in_the_middle_of: RUN 22 — #58 bullet 4 IS DONE and the evidence is committed; nothing is running in the background. NEXT: land PR #90 (base develop, CI green on all 5 jobs at 605df52, run 32758424509) after resolving its ONLY conflict, BUILD/ACCURACY-STATE.md — no production code conflicts. *** BULLETS 1/2/5 were already provable offline and are ticked. BULLET 4 is ticked as RAN-WITH-STATED-LIMITS, not as clean: it ran live under bypass, but it exercised only 2 of the 3 properties. BULLET 3 (whitespace) is ticked for the TRANSFORM (unit-tested in tests/test_metamorphic.py::TestNormaliseAnswerWhitespace) and has ZERO LIVE COVERAGE, which no amount of spend fixes — the golden answers contain no collapsible whitespace, so the transform is a no-op on every one of the 71 leaves. Getting live whitespace coverage needs new fixtures, which is a corpus change and the human's call, NOT a re-run. *** DO NOT re-run the sweep to chase the one violation (MISSION 12.9). It is a gemini-path leaf and unattributable by construction; attributing it needs a same-input control arm that is NOT authorised. *** COST NOTE THE HUMAN SHOULD SEE: the run overshot its own preflight — $0.279397 actual vs $0.216 stop-and-ask, 169 calls vs 168 projected. It was unattended so the threshold never fired. If a future sweep carries a stop-and-ask, it needs an in-process ledger check, not a projection in a manifest. *** ALSO PENDING, unchanged: the 14:41 H-issue directive authorises building the labelling machinery at ZERO SPEND (hash-chained eval/rulings.jsonl writer, agreement + Wilson, pre-committed sample manifest); #47 stays blocked. #88's three questions and #45's escalated design question are still the human's. *** UNPUSHED: chore/accuracy-post-45-bookkeeping holds run-21's E4 remedy (3 commits, local only).
---
