...
spend_usd: 1.488057
in_the_middle_of: NOTHING IN FLIGHT; the run is HUMAN-GATED and stopped cleanly. #88 (M2.1b) stays In progress awaiting the 3 questions posted on it 2026-08-24T04:33Z (spend $4.88-6.83 for Gemini-path strata? / fix profiles.py:50 0625-p2->MCQ first? / ruling on the 2894 unbanded leaves). Board 'next' returns nothing Ready. NO SPEND THIS RUN; ledger unchanged at 1.488057. *** WHY NO OTHER ITEM WAS STARTED: MISSION 3.2 permits only ONE issue in flight and #88 holds that slot, so #45 (which the #88 census already fed its 229-failure input to) was NOT started despite being free. Of the open M1 set, #38/#39/#41 are mark-changing and need a sweep = unauthorised spend; #58 requires a golden-set run at cache_mode=bypass = spend. *** THE ONE THING THIS RUN ADDED, free and verified at source, posted on #37: (1) #37 IS MARK-CHANGING under the gate-9 DOES test - normalize_extracted_answers rewrites question_id at answer_extraction.py:78 and its only production caller is GeminiAnswerExtractor.__call__ at :187, so a reassigned answer is marked against the WRONG question's scheme; removing the fallback changes awarded_marks wherever it fires. The 2026-08-24 inbox directive listed #37 as probably-routing-only 'unless one of them turns out to move marks' - it does, and it is now classed with #38/#39/#41. (2) THE FIRING RATE IS UNMEASURED, NOT ZERO: harness.py:337 hardcodes id_match='exact' for every leaf that has an answer (docstring :364), so the 773 'exact' / 8 'unmatched' rows in BUILD/accuracy-runs/ CANNOT show whether the fallback fired - a fallback rewrite is recorded as 'exact'. No structured logs are persisted in either run dir, so the id_positional_fallback warning at :73 is lost. #37 bullet 3 is therefore NOT a rename: id_match has no code path that emits 'fuzzy' at all and must be made to measure three states before any CI target is re-derived. *** develop has 4 UNPUSHED commits (cd2a2c4, 3b2b8a4, 8d0ff8c, 9c9bef6); supervisor PASS covers 9c9bef6, the branch tip.
---
