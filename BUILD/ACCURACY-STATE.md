...
spend_usd: 2.659533
in_the_middle_of: RUN 29 — NOTHING RUNNING, NO SPEND (ledger sum 2.659533 of $25.00). PR #99 and PR #100 both MERGED to develop (c0c9540, 3bb0cca), all CI green; #98 CLOSED and board Done. *** PR #101 IS OPEN AND NEEDS LANDING: base develop, branch chore/accuracy-run-29-bookkeeping @ 243f74f, BUILD/-ONLY (diff against lemely/ web/ tests/ is EMPTY). CI was still running at run end — poll 'gh pr checks 101' and merge once green. It carries the run-29 inbox tick and this pointer. *** THREE RULINGS ASKED OF THE HUMAN ON #98, none resolved by picking the convenient branch, all in the inbox note on the 14:41:54 item: (a) the #51 stratification axis genuinely conflicts between DA2 (full 3-axis DA1 stratum) and the 14:41:54 item (mark band only) — implemented mark-band-only with the axis as a one-line parameter; (b) DEVIATED from DA2's literal sha256(relabel_salt || question_id) to hash the full (paper_id, question_id) leaf identity, because under the literal formula every paper's '1a' shares a rank and enters the sample AS A BLOCK — nothing sampled under either formula so no membership changes, but it is the human's decision that I altered; (c) the salt is cleartext and the module's false 'unpredictable in advance' claim was corrected. *** STILL BLOCKED ON THE HUMAN, unchanged from run 28: #37's two questions (bullet 3's CI target cannot be re-derived from a corpus scoring id_match_rate 1.0 in BOTH arms — that would be PICKING a number); #93's sweep waiver (branch pushed, no PR, null by construction twice over, MUST land before any #57 preflight and before the split freeze); item 6 whitespace fixtures (I recommend a metamorphic-only set load_golden_cases does not serve); #58's q11b experiment (~$0.01, a DIFFERENT design, proposed not run); the 0625 paper-3 constant; PARSED CORPUS regeneration (artifacts do not exist and it must wait for #93). *** 17:36 items 1-2 and 4 (#88) remain sequenced behind #93; item 3 is DONE (DA12). #59 decisions 1-2 unstarted. *** #58 BULLET 4 STILL CORRECTLY UNTICKED (Fisher p=1.000; DO NOT re-run, MISSION 12.9). *** FOUR OPEN H ISSUES: #49, #51, #52, #55 — #98 built only the tooling around their work and touched none of it.
---
