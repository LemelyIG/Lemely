...
branch: chore/accuracy-run-36-alt-marker-diagnosis (worktree HEAD); feature/accuracy-41-... @ 09314c7, feature/accuracy-37-... @ 8f3ba7f, feature/accuracy-93-... @ 70a6957 all pushed, no PRs
last_run_label: run-36-2026-08-26
last_run_headline: NO SPEND, READ-ONLY DIAGNOSIS. PR #111 MERGED (b644789). RUN 35'S OWN SUSPICION REFUTED: 0606_s23_ms_12's 16- and 18-mark leaves are NOT total/summary rows — they are question 9 split across a page break into two same-id leaves, each collapsing MULTIPLE ALTERNATIVE SOLUTION ROUTES into one leaf (the 18-mark leaf carries one answer point three times, indices 0/4/10 byte-identical). The 16+18=34 vs +36 arithmetic was coincidence. NEW DEFECT FOUND AND OPENED AS #112 (opened, NOT executed, mark-changing): rows.py:316-330 matches OR/EITHER only as the whole cell or followed by a SPACE, but pdfplumber returns marker+working in ONE cell separated by a NEWLINE, so every marker is missed and mutually exclusive routes are SUMMED; 'Alternative', the word CAIE 0606 prints, is not in the vocabulary at all. MEASURED over #45's mark_aggregation_overcount bucket (46 parsed): 18 papers affected, 59 missed markers, 77-246 marks of a 673-mark bucket overcount (11.4-36.6%), 17 of 18 are 0606. HONEST VERDICT — real defect, NOT the explanation: even the upper bound leaves ~63% unaccounted and 7 of 18 papers unreconciled. SELECTION EFFECT RECORDED: the same scan over the 250 SUCCESSFULLY-parsed schemes finds 0 true instances, because an affected scheme overcounts and is escalated out — strong selection AGAINST, not strict impossibility (an exactly-cancelling deficit would survive and this scan cannot rule it out). Either way prevalence measured over outputs/schemes/ is biased to near-zero (D18 shape); measure such defects over the FAILURE set. Filed as B15.
...
spend_usd: 2.696466
in_the_middle_of: RUN 36 — NOTHING RUNNING, NO GEMINI CALLS MADE BY THIS RUN. PRs #111 (b644789) and #113 (97fb1d0) both MERGED. PR #115 MERGED (5eafbde) — all five CI jobs green, supervisor sweep PASS over its tip 567a167. THIS run's own closing PR is #116 (branch chore/accuracy-run-36-close, BUILD/-only); if it still reads OPEN, poll 'gh pr checks 116' and merge once green, route red to accuracy-gate-triage. *** LEDGER CORRECTION — my own run-36 claim was FALSE. The landed state line said 'ledger unmoved: 2.2803523500000105'; I then VERIFIED rather than asserted it and the ledger reads 2.2881143500000296, a +$0.0077620 move at 2026-08-26T03:05:45Z, inside the supervisor's pytest window (.coverage 06:17, sweep 06:18:53 after 21m; a find over both worktree roots for 06:00-06:12 returns ONLY this file). Run 36 made zero Gemini calls. OPENED AS #114, not executed. Ruled out: the live billed test (double-gated, LEMELY_LIVE_ACCURACY set nowhere) and test_gemini_client/test_cost_ledger (both redirect output_dir to tmp). CAUSE NOW PROVEN, and it is the worse branch — the spend is SYNTHETIC, not real. tests/test_correction_ai.py:93 and tests/test_answer_extraction.py:108 override cache_dir ONLY; PathsSettings.output_dir (config.py:55) defaults to the RELATIVE path 'outputs', resolving to the repo root under pytest, and gemini.py:162 builds the ledger from exactly that. test_gemini_client.py:114-125 sets BOTH paths and comments why — two modules protected, two not. Those clients are MagicMock, so NO API call and NO money: mock-derived costs are written into the real ledger every pytest run. REPLICATED across two consecutive sweeps (2.2881143500000296 -> 2.2900548500000344 at 03:36:18Z), deltas $0.0077620 then $0.0019405 — an EXACT 4.0 ratio, i.e. a fixed unit written N times, which is not token-billed spend. THIRD REPLICATION (sweep over 567a167, reported 07:15:33): ledger moved again to 2.291995350000039 at 04:02:26Z, delta EXACTLY $0.0019405 = exactly 1.0 unit. Three consecutive sweeps, three writes, 4/1/1 units of a $0.0019405 quantum — the unit is now pinned, not inferred. CONSEQUENCE: under DA11 cumulative_usd is NOT money spent, and the contaminated portion CANNOT be reconstructed (one running total, no history), so every ceiling check and preflight headroom figure rests on it. Fixing the test isolation is easy; choosing the ledger's basis going forward is the human's call. DA11 makes this file authoritative for money spent, so either the sweep silently bills the $25 ceiling or the authoritative record is polluted with test data; whether the $0.0077620 is backed out is a HUMAN call and I have not touched it. *** #112 OPENED, NOT EXECUTED (mark-changing): det alternative-branch markers followed by a newline are missed (rows.py:316-330), so mutually exclusive solution routes are SUMMED. Filed as B15. Overlaps #110 on 0606_s23_ms_12 — sweep them jointly if either is authorised. *** DO NOT RE-TEST, both falsified by the runs that raised them: run 35's flush()/q_row_had_answer hypothesis and run 36's total/summary-rows suspicion. Evidence under BUILD/accuracy-runs/det-mismatch-diagnosis-2026-08-26/ and alt-marker-diagnosis-2026-08-26/. Also on disk, do not re-run: the #95 probe, #45's census, /tmp/acc57-full/out's 250 parsed schemes. *** TRAP FOR #88: prevalence of an overcount-causing defect measured over outputs/schemes/ is biased to near-zero, because affected schemes are escalated out. Measure over the FAILURE set. *** FULLY HUMAN-BLOCKED on ACCURACY-ASKS.md B1-B15. B2 (#93 waiver) highest-leverage; B1 holds the only finished branch. #41 MUST NOT BE CLOSED or its sweep pre-authorisation lapses. FOUR OPEN H ISSUES: #49, #51, #52, #55; #49 keeps the split NOT FROZEN.
---
