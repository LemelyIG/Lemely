...
spend_usd: 1.425511
in_the_middle_of: NOTHING IN FLIGHT, tree clean, develop synced to origin at ba92be3. Board: nothing in flight, nothing blocked, no open H issues. Inbox 0 unchecked — all five 2026-08-24T01:14:03 directives acted on and flipped with notes. *** LANDED THIS SESSION: #44 (M2.1 corpus restoration) PR #84 -> 5f7cde7; #40 (M1.5 coherence gate) PR #83 -> d203916; #46 (M2.3 two-pass blind labeller) PR #85 -> ba92be3. All three on genuinely green CI, no waiver ever used. Board M1 Done=2, M2 Done=2. *** THREE DIRECTIVES REMAIN UNSTARTED — this is the whole of the next run's queue. *** (1) #36 (M1.1 confidence unit), option A, NO SPEND, do this FIRST: branch feature/accuracy-36-the-confidence-unit-must-ship-as-one still exists at 2cae804 — AMEND IT, do not restart. It is now 3 merges behind develop, so MERGE DEVELOP IN FIRST and expect BUILD/ACCURACY-STATE.md + BUILD/BLOCKERS.md to conflict (they did on #40 and #46; resolve with 'git checkout develop -- <file>', develop is always the superset). The work: scope the 0.2 cap in _calibrate_confidence to the mcq-hint-NON-letter case where develop had it, so a clean single letter keeps its raw confidence; AMEND BULLET 2 on the issue to name that input rather than reading as a blanket ceiling; AMEND BULLET 7 to record it as DEFERRED to the M0.9 ratchet work (it sits at 32.58%, ratchet unarmed, breach already non-blocking) — do NOT delete either bullet, and do NOT record either as met. #36 lands on bullets 1-6. Section 9 gate 9 does NOT require a sweep for it: the human scoped gate 9 to MARK-CHANGING items only, and #36 changes confidence/review routing, not awarded marks. (2) #28 (M0.4 ablation) SPEND AUTHORISED, no per-item cap: costed preflight FIRST, posted as an issue comment BEFORE any spend; '--cache-mode bypass' and NEVER 'refresh' (E2: refresh rewrites the shared cache on every repeat); stop-and-ask at $20.00 of the $25.00 ceiling; 'NOT REPORTABLE, with reason' is a SUCCESS (12.9) and must never be answered by re-running at higher n. (3) #57 (M0.7b) unblocked via option A: a mark-scheme PARSING pass is authorised to reach ~300 leaves — do the DETERMINISTIC MCQ schemes FIRST at zero cost and see how far they get before spending anything on the Gemini theory path; costed preflight before that spend; same $20.00 stop-and-ask; do NOT freeze over the 71 golden leaves (drop-only makes it irreversible). I intend to make the parsing pass ITS OWN ISSUE rather than bury it in #57, because #45 (M2.2 census) needs the same parsed schemes and would otherwise duplicate the spend — that structuring choice was explicitly left to me. *** SPEND LEDGER 1.425511 raw / ~3.83 corrected of the 25.00 ceiling. Both #28 and #57 are now the only authorised spenders. *** WHAT #46 SHIPPED, so nobody rebuilds it: lemely/labelling/ (localhost two-pass server), 'lemely label' and 'lemely label-verify' CLI commands, append-only SHA-256 hash-chained JSONL at eval/labels/<paper_id>/{transcription,marking}.jsonl (SPEC-CONFORMANT path; labeller_id is stamped on every RECORD and in manifest.<labeller_id>.json, NOT a path segment — that satisfies DA2's two-labeller requirement with ONE tamper-evident chain per paper-pass), pass-2 captures per-mark-point BINARY VERDICTS plus the labeller's own question-type judgement (spec 6 line 610; the type judgement is a REPORTING variable for M2.4 only, DA1 forbids it as a split-assignment input), and a 4th import-linter contract forbidding the labeller from importing the correction pipeline or lemely.core.schemas, enforced by a test that plants a REAL violation. eval/labels|scans|mark_schemes are gitignored (repo hygiene: the supervisor auto-commits dirty trees). *** KNOWN GAP CARRIED FORWARD: eval/scans/<paper_id> has NO PRODUCER, so pass-1 currently serves whole-page scans and blindness holds against the MARK SCHEME but NOT against neighbouring questions on the same page. M2.4 labelling must not start assuming per-question crops exist. Same shape as #57's parsing gap — #44 restored PDFs and M2 keeps needing derivation steps nobody scoped. *** PROCESS LESSONS WORTH KEEPING: #46 took THREE repair passes because each one reported an acceptance criterion met while a human still could not label anything (first no HTML at all; then HTML whose submit button crashed the server on form-urlencoded vs json.loads; then a form that recorded only a total instead of spec 6's per-point verdicts). Root cause every time: TESTS EXERCISED A PATH NO HUMAN TAKES — they POSTed hand-built JSON. The fix that mattered was deriving test inputs from the RENDERED FORM itself. Apply that generally: if a criterion is about a human being able to do something, the test must go through the same door the human does. ALSO: a clean git auto-merge is NOT evidence both features survived — lemely/app/cli.py auto-merged between #40 and #46 and I verified both coherence_trigger_rate and the label command were present before trusting it. ALSO: accuracy-pr-land needs a DURABLE review verdict passed in its args; a verdict living only in the orchestrator's session does not count, and CI matrix jobs run SERIALLY at ~10-18 min each so a short watch window times out legitimately — TIMEOUT IS UNKNOWN, never a pass. ENV: pre-commit not on PATH ('PATH=$PWD/.venv/bin:$PATH .venv/bin/pre-commit run --all-files', always --all-files); 'accuracy_board.py block' writes to BLOCKERS.md as a side effect and dirties the tree; 'git cherry-pick' has no -q flag; 'git rebase' silently drops signatures (check 'git cat-file commit HEAD | grep -c gpgsig'); 'accuracy_board.py done' exiting 1 can be harmless when a squash already auto-closed the issue (exit 2 is the H-issue refusal).
---
