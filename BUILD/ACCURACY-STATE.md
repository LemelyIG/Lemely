...
spend_usd: 1.488057
in_the_middle_of: REVIEWED #45's BRANCH THIS RUN (17 commits, no PR, no review) and ran the one free experiment its own manifest named as missing. NO SPEND; ledger unmoved at 1.488057. NO new inbox items, NO human answers on any ask -- 3rd consecutive run with none. *** #45 ESCALATION INDEPENDENTLY CONFIRMED, checked in code not taken on trust: classify_theory_residual returns UNCLASSIFIED ONLY on the final computed_total == maximum_mark branch, and every scheme is in the failure set BECAUSE of mark_total_mismatch_escalating, so that branch is unreachable for exactly the rows needing it -- the census genuinely cannot say 'we don't know'. mismatch_cause is total over the rest (> overcount, <= genuine), so 70+47 = 117/229 = 51% land in buckets that claim to know. Scope claims also hold: ZERO production files in the whole diff, profiles.py zero-diff, classified-failures.txt 229 rows matching manifest counts exactly. Hunted a 6th instance of the defect class and did NOT find one; the limitation I went after (paper_profile_misconfiguration asserts causation from a branch difference) is already disclosed verbatim in its own manifest. *** NEW EVIDENCE I CONTRIBUTED -- the counterfactual reparse the census explicitly did NOT attempt. Real DeterministicMarkSchemeParser, 40 stems x 2 arms, patching only _PHYSICS_PROFILE.paper_type_by_number[2] -> PaperType.MCQ (the exact runtime state the profiles.py:50 fix gives). AS SHIPPED 0/40 parse (control -- reproduces the census's own failure set), COUNTERFACTUAL 39/40 parse. FINDING 1: for 39 rows paper_profile_misconfiguration is now a DEMONSTRATED CAUSE not just a branch difference -- strongest-evidenced bucket in the taxonomy, M3 can consume it. FINDING 2: 0625_s24_ms_21 is MISATTRIBUTED -- still fails after the fix with the SAME signature ('parsed 12, expected 40'), so the profile bug is not a sufficient cause for it; honest count is 39 causal + 1 misattributed, NOT 40. Artifact committed at BUILD/accuracy-runs/counterfactual-0625p2-2026-08-24/ (manifest + script, re-derivable free). Posted to #45 and #88. *** BEARING ON #88 q2, measured not estimated: the fix moves 39 schemes from the paid set to the free set, 229 -> 190 (-17.0%). #88's costed rows assumed 189 after the fix, so they are optimistic by exactly ONE scheme -- immaterial to the decision but recorded, not quietly carried. profiles.py NOT fixed: it moves 39 schemes between DA1 strata and can change awarded marks, so under gate-9 I read it as mark-changing and human-gated. *** THREE HUMAN ASKS STILL OPEN, all pure decisions: (1) #58 bullet 4 -- authorise ~USD 0.144 or formally RETIRE it as was done for #40's; PR #90 is OPEN, MERGEABLE, green on all 5 jobs at 10b5ec5 and now reviewed (2 defects found and fixed last run), so bullet 4 is the ONLY thing left. (2) #88 q1/q2/q3 -- q1 and q3 are DA1/H4 (#49) territory, MISSION 3.5 forbids working around them. (3) #45's escalated 3-option design question (make UNCLASSIFIED reachable / add cause_indeterminate / instrument the parser); its own recommendation is (2) now and (3) as its own issue. *** develop = 1092d8f, local ahead=0. Both feature branches PUSHED and rebased clean on it. ALWAYS REBASE WITH -S (commit.gpgsign unset here). Do not prune local branch chore/accuracy-build-records-2026-08-24 -- it holds the 8 individually-signed originals GitHub auto-deleted. Nothing running in the background.
---
