...
run_pointer: run-2026-08-29-i
worktree: /home/sico/Lemely-worktrees/accuracy
branch: measure/accuracy-57-axis-constant (this)
last_run_label: split-axis-57-2026-08-29
last_run_headline: RUN 69 — #57 CANNOT PROPOSE A SPLIT: DA1's PARSE-PATH AXIS IS A CONSTANT, NOT SPARSE STRATA. Zero spend. *** corpus/manifest.json records gemini_used: False, cost_usd: 0.0, totals {source_pdfs 479, parsed 289, failed 190}. EVERY ONE of the 289 committed schemes is DET-PARSED. There are ZERO Gemini-parsed schemes in the population #57 partitions, so the axis has ONE LEVEL. You cannot stratify on a constant, and bullet 1 cannot be executed as DA1 specifies. *** THIS CORRECTS A LOOSER STATEMENT OF MINE. A previous run posted on #57 that 'the Gemini strata are empty because #88's sweep aborted at 6 of 190'. That describes the HISTORY OF A RUN when the question was about the CONTENTS OF A POPULATION. The sweep's 6 and C20's 12 went to STAGING DIRECTORIES and were never in corpus/ at all — they could not have populated the axis even if the sweep had finished. cost_usd: 0.0 was in the manifest the whole time and is stronger and simpler. Two different objects, and I checked the one I had recently been working on rather than the one the question was about. *** POPULATING IT IS NOT SIMPLY FINISHING #88: #88 is blocked by #166 (fallback fails ~50% of det-failures, 100% of 0606), so a completed sweep buys a LOPSIDED, PARTLY-UNFILLABLE stratum. The remainder did shrink 190 -> 148 via DA34/39/41, but corpus/ still holds the 289 at parser_sha 8758dba, so realising that needs a regeneration = #95, which is itself blocked (DA42). *** THREE ROUTES, NO RECOMMENDATION, and the reason is specific: option 2 (stratify on the two axes that DO vary and record parse path as a constant) is the ONLY one that lets #57 proceed today, and it works by CHANGING THE MEASUREMENT DESIGN TO FIT WHAT THE DATA CURRENTLY SUPPORTS. DA1 was fixed in a human interview precisely so an agent could not re-derive it around an inconvenient measurement. DA47.
