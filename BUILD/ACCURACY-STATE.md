...
spend_usd: 2.700347
in_the_middle_of: RUN 36 (cont.) — NOTHING RUNNING, NO GEMINI CALLS BY THIS RUN. All of run 36's PRs MERGED: #111 (b644789), #113 (97fb1d0), #115 (5eafbde), #116 (0ec8aaf). *** PR #117 IS OPEN AND NEEDS LANDING: base develop, branch chore/accuracy-a13-missing-inbox-note, BUILD/-only, one file. Poll 'gh pr checks 117' and merge once green; route red to accuracy-gate-triage. *** A13's inbox note was MISSING for eleven runs — the only item of 2026-08-25T12:03:43 without one, and the item that produced B1. Written in #117 from evidence (branch feature/accuracy-41-... @ 09314c7, one commit, VERSION 4->5, strict M-then-A as fallback only); left UNTICKED because bullet 2's sweep is unrun and blocked on B1. #41 MUST NOT BE CLOSED or its sweep pre-authorisation lapses. *** #114 REPLICATED FIVE TIMES, one write per supervisor sweep, units 4/1/1/1/1 of an exact $0.0019405 quantum (latest 2.2958763500000487 at 04:51:21Z). PROVEN CAUSE: test_correction_ai.py:93 and test_answer_extraction.py:108 override cache_dir ONLY; PathsSettings.output_dir (config.py:55) defaults to the RELATIVE path 'outputs' -> repo root under pytest; gemini.py:162 builds the ledger from it. Clients are MagicMock, so NO API call and NO money — the ledger accrues SYNTHETIC cost every sweep. Under DA11 cumulative_usd is NOT money spent and the contaminated portion CANNOT be reconstructed (one running total, no history). Verified this run: #41's branch adds tests to that same module but builds no GeminiClient, so landing it does NOT worsen the rate. Ledger UNTOUCHED; basis decision is B16. *** #112 and #110 OPENED, NOT EXECUTED, both mark-changing, overlapping on 0606_s23_ms_12 — sweep jointly if either is authorised. *** DO NOT RE-TEST: run 35's flush()/q_row_had_answer hypothesis, run 36's total/summary-rows suspicion, and my wrong guess that the four test_web_* modules wrote the ledger — all three falsified and recorded. Evidence under BUILD/accuracy-runs/. Do not re-run the #95 probe, #45's census, or /tmp/acc57-full/out's 250 parsed schemes. *** TRAP FOR #88: an overcount-causing defect measures near-zero over outputs/schemes/ because affected schemes are escalated out — measure over the FAILURE set. *** PRACTICE: re-sum the four worktree ledgers every run; never carry spend_usd forward. *** FULLY HUMAN-BLOCKED on ACCURACY-ASKS.md B1-B16. B2 (#93 waiver) highest-leverage; B1 holds the only finished branch. FOUR OPEN H ISSUES: #49, #51, #52, #55; #49 keeps the split NOT FROZEN.
---
