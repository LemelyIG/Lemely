...
branch: chore/accuracy-run-24-bookkeeping (PR #97, worktree HEAD); feature/accuracy-93-... @ 70a6957 pushed, no PR
last_run_label: sweep93-2026-08-25
last_run_headline: #93 GATE-9 BEFORE/AFTER, $0.00 AND ZERO GEMINI CALLS. The costed preflight's honest answer was DO NOT SPEND: the harness sweep is NULL BY CONSTRUCTION on two independent grounds — (1) the harness never invokes the det parser (harness.py:103-109 deserializes pre-parsed mark_scheme.json; lemely.io.det is imported nowhere under lemely/accuracy or lemely/eval; the det question-paper extractor is reachable only from the question-bank CLI), and (2) the golden set contains ZERO 0625 paper-2 cases. Same ground the #38 sweep was waived on (A6 ruling (a), BLOCKERS section H). SUBSTITUTE MEASUREMENT, a census not a sample: 489 schemes, 406 unchanged, 81 changed, 2 unreadable. 0625 paper 2 theory_core -> mcq (41 schemes, THE FIX — scheme_format POINT_BASED -> MCQ, parse path genuinely changes, these papers previously did not parse at all). 0625 paper 3 theory_extended -> theory_core (40 schemes, A SECOND REAL BUG fixed by the ORDERING change not the constant; cover of 0625_m19_ms_32.pdf reads 'Paper 3 Core Theory'; CAUSALLY INERT — scheme_format POINT_BASED both sides, metadata label only). NO OTHER SUBJECT OR PAPER CHANGED. Regression probe for the real hazard (a theory paper whose cover mentions 'multiple choice' flipping to MCQ and breaking its parse): did not occur once in 489 schemes; nothing moved INTO mcq except 0625 paper 2.
...
spend_usd: 2.369728
in_the_middle_of: RUN 26 — NOTHING RUNNING, NO SPEND THIS RUN (ledger sum 2.369728, zero gemini calls). *** PR #97 IS OPEN AND NEEDS LANDING: base develop, branch chore/accuracy-run-24-bookkeeping @ 813f156, BUILD/-ONLY (git diff origin/develop...HEAD -- lemely/ scripts/ tests/ is EMPTY; whole diff is ACCURACY-INBOX.md +20/-2 and ACCURACY-STATE.md +5/-5). CI was UNSTABLE/running at run end — land it once green. IT RESTORES FOUR HUMAN DIRECTIVES THAT WERE MISSING FROM DEVELOP ENTIRELY (the E4 hazard, realised): 14:36:24 PRE-AUTHORISATION (#93 AND #41 bullet-2 gate-9 sweeps both pre-authorised — this was reachable ONLY as a GitHub issue comment, so an orchestrator reading develop's inbox could not have known), 14:37:23 TWO RULINGS, 14:41:54 H-ISSUE PRE-COMMITMENTS, 15:00:17 the #58 ~$0.144 authorisation. Also restores completion notes for A4, #88 q3 (true leaf count 9,464 not 12,358) and #59 decision 3. chore/accuracy-post-45-bookkeeping is now MERGED INTO this branch and no longer needs carrying. *** #93 STILL WAITS ONLY ON A WAIVER. Branch feature/accuracy-93-profiles-paper-type-lookup-order @ 70a6957, pushed, NO PR. Conditions 1 and 3 SATISFIED (costed preflight posted, answer = DO NOT SPEND; adversarial review done). The sweep is NULL BY CONSTRUCTION twice over — harness never invokes the det parser (lemely.io.det imported nowhere under lemely/accuracy or lemely/eval) and the golden set has ZERO 0625 paper-2 cases — so the ask is the same instrument-blindness waiver #38 got under A6 ruling (a). Substitute evidence committed at BUILD/accuracy-runs/sweep93-2026-08-25/: census of 489 schemes at $0.00, 41 schemes 0625 p2 theory_core -> mcq (the fix), 40 schemes 0625 p3 theory_extended -> theory_core (a SECOND real bug, causally inert), no other subject/paper changed, and the theory-flips-to-MCQ hazard did not occur once. *** PARSED CORPUS DIRECTIVE CANNOT BE DONE AS WRITTEN — the artifacts DO NOT EXIST. outputs/schemes/ has 33 files, not 250, and there is NO parse.log anywhere. The 250 is census-2026-08-24-a's parsed_by_det, written to a throwaway output-root that was not kept. Regeneration is free (~40 min, det-only) but MUST WAIT FOR #93: a corpus generated today would be MISSING 39 schemes that ParseError and mislabel 40 more, and git history is permanent. Recommendation posted in the inbox note and PR #97; the human's call. *** FOUR THINGS STILL WAITING ON THE HUMAN, none re-derivable by me: (1) #93's sweep waiver; (2) whether to open a separate issue for the wrong 0625 paper-3 CONSTANT (left wrong-but-annotated deliberately); (3) item 6 whitespace fixtures — ordinary golden cases joining the accuracy denominator, or a metamorphic-only set? I recommend the second; (4) #58's q11b settling experiment (~$0.01, a DIFFERENT design from the authorised control, proposed not run). *** #58 BULLET 4 STILL CORRECTLY UNTICKED: control arm falsified its premise (marking-only churn 0/62, not 0.1565 — that figure is end-to-end), but reorder 1/31 vs control 0/62 is Fisher p=1.000, unresolved in BOTH directions. DO NOT re-run (MISSION 12.9). *** ALSO UNSTARTED: 17:36 items 2-4 (#88 spend sequenced behind #93, the ratified 9,464 leaf count, the lemely.toml token-ceiling raise), A5 (#37 sweep, authorised, preflight first), #59 decisions 1-2 (rasterise; restate as handwriting-not-scan).
---
