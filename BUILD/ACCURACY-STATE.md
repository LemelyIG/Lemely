...
spend_usd: 1.425511
in_the_middle_of: #36 (M1.1 confidence unit) IMPLEMENTED AND REVIEWED — BLOCKED ON A HUMAN DECISION, NOT ON ENGINEERING. NO PR OPENED; board item stays In progress. Branch feature/accuracy-36-the-confidence-unit-must-ship-as-one, ONE signed commit 2cae804 rebased onto LOCAL develop (so 048f839+45b72e1+941b2a7 ride into the squash — the workflow had cut it from ORIGIN/develop, the exact E4 stranding hazard; also note git rebase DROPS the signature, caught with 'git cat-file commit HEAD | grep -c gpgsig' returning 0, re-signed with commit -S --amend). Tree clean, pre-commit --all-files green, pytest tests/eval/ + tests/test_accuracy_harness.py + tests/test_answer_extraction.py all pass. accuracy-review wf_86031f3f-b69 returned recommendation=BLOCK (6/6 dimensions ran, 0 unreviewed; 1 skeptic agent died, which is why two findings read 'unverified-budget-exhausted' — the two load-bearing ones I re-verified at source myself). *** THE BLOCKER IS A SPEC CONFLICT, WRITTEN UP IN FULL IN BUILD/BLOCKERS.md SECTION B AND AS A 2026-08-23 COMMENT ON #36 — READ THOSE, DO NOT RE-DERIVE. *** In one line: bullet 1 (propagate extraction confidence) is genuinely UNMET — correction_ai.py:182-194 still hardcodes confidence_score=1.0/HIGH/needs_teacher_review=False on the MCQ success branch, exactly the defect the issue's own Why names, so the threaded extraction_confidence field is read by NO decision path. But bullet 2 as written ('raw-0.90 single-letter + source_region ends <=0.20, not 0.23 or 1.00') CONFLATES TWO DIFFERENT INPUTS: on develop a clean single letter took an UNCAPPED +0.1 (->1.00) while an mcq-HINT-with-a-non-letter answer took min(conf,0.2) and leaked to 0.23. Satisfying bullet 2 literally means capping clean letters at 0.2, a NEW ceiling develop never had — and once bullet 1 lands that makes every correctly-extracted MCQ read LOW/needs-review (the whole det path, 8 of 31 golden leaves ~26%) on top of review_rate_signal already at 32.58%, so bullets 1+2 make bullet 7 (<=8%) strictly less reachable. HUMAN MUST PICK: (A) cap scoped to the non-letter case as develop had it, clean letters keep raw confidence (0.93), bullet 2 amended; (B) cap everything MCQ-shaped at 0.2 as written, accept every correct MCQ becoming needs-review, formally retire bullet 7 for M1.1; (C) other. *** WHY THIS WAS NOT DECIDED HERE, and do not 'helpfully' decide it next run: option A is the option that makes the GATED METRIC EASIER. Lowering MCQ review volume is the narrowed-denominator/moved-goalpost shape accuracy-reviewer exists to catch, and an agent picking it on its own judgement is indistinguishable from that failure mode even when the engineering argument is good. This is structurally DA1 (a spec sentence unsatisfiable as written) and DA1's precedent is: propose the amendment, leave the spec alone until the human says. *** FIXED THIS RUN (do not re-litigate): paper_grade_confidence weighted by truth_marks = marks EARNED not tariff (harness.py sets truth_marks=gt.awarded_marks), dropping 23 of 71 baseline rows INCLUDING the corpus minima marker_conf 0.55 and 0.65, biasing every band upward and making an all-zero paper vanish. Fixed by weighting on the TARIFF via new EvalRecord.maximum_marks: int | None = None — THE DEFAULT IS LOAD-BEARING, StrictModel is extra='forbid' which rejects UNKNOWN keys but accepts a MISSING key with a default, so published aa-floor-2026-08-23-a JSONL still parses (verified, all 71 rows load). Rows weighted 48/71 -> 71/71. Rejected the reviewer's max(truth_marks,1) — still weights by earned marks, same bias attenuated. *** BULLET 6 UNMET AND MY HYPOTHESIS WAS WRONG: I predicted the truth_marks bias caused the all-HIGH degeneracy; it did not — all-HIGH before AND after (0.947/0.9197/1.0/0.9362/0.982 over the 5 golden papers). Structurally guaranteed while marker_conf is pinned at 1.0 for every correct MCQ; re-read it once bullet 1 lands, do NOT relax the bullet. BULLET 7 unmet at 32.58% and this diff PROVABLY cannot move it (review_rate reads only 'triggers', from needs_teacher_review, untouched). Both left honestly unmet; section 9 gate 8 records the M0 breach non-blocking precisely so M1.1 can still merge. *** ONE SHOULD-FIX DELIBERATELY LEFT UNDONE so the branch matches exactly what the review saw: no test exercises the oracle-arm gate at harness.py:746. *** #28 (M0.4) STILL NOT AUTHORISED — do not start it, not even its preflight. CI WAIVER PERMANENTLY VOID, never pass allow_merge_without_ci. M1 IS OPEN (the old 'M1 is gated' claim is retracted): if #36 stays blocked, #37/#38/#40/#41/#58 have NO dependencies and #39's only edge (#32) is Done — but section 3.2 allows only ONE issue in flight, so move #36 out of In progress before starting another. ENV: pre-commit not on PATH, use 'PATH=$PWD/.venv/bin:$PATH .venv/bin/pre-commit run --all-files', always --all-files.
---
