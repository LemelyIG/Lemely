...
last_run_label: run-40-2026-08-26
last_run_headline: ZERO SPEND, no Gemini calls. MERGED: PR #128 (#93 — B2 waiver + B7 folded in) -> 3c40ffd and PR #129 (run-39 close) -> 8758dba, both green on all five CI jobs. *** B8 DELIVERED, and the ordering the human insisted on PAID: regenerating AFTER #93 yields 289 parsed of 479 ($0.00, det-only) against the 250 of census-2026-08-24-a — #93 unblocked exactly 39 0625 p2 schemes, verified end to end (0625_m19_ms_22.json now records mcq/mcq). Committing at 250 would have enshrined a corpus missing 39 schemes that the very next merge fixed, permanently, in a public repo. Corpus lands in corpus/ WITH parser_sha 8758dba, because outputs/ is gitignored and has NEVER been tracked — B8 could not be done as literally written. PR #131. *** B17 option 3 IMPLEMENTED, PR #130: accuracy_board.py done now closes off-board non-H issues, so MISSION 13 is reachable for #114/#120/#121/#122/#124. Review found TWO real gaps, both fixed: only the H-NUMBER half of the ruling had shipped (owner:human label now also refuses, exit 2), and str(payload['title']) turned a degraded fetch into a PERMISSIVE CLOSE — str(None) matches no H-pattern, so a null title sailed past the guard. Now validated, fail-closed, proven by mutation (4 of 6 degraded cases fail on the old coercion). *** TWO MEASUREMENT ERRORS OF MY OWN, both caught and corrected: the manifest's 289 file digests were ALL stale because pre-commit's end-of-file-fixer rewrote the files after generation; and 'pgrep -f parse-mark-schemes' matched its own shell command, so I reported the parse RUNNING several times after it had finished.
...
spend_usd: 2.704228
in_the_middle_of: RUN 40 — NOTHING RUNNING, NO GEMINI CALLS, NO SPEND. The B8 parse FINISHED (479 total, 289 parsed, 190 failed, $0.00). *** TWO PRs OPEN, BOTH NEED LANDING — poll 'gh pr checks <n>', merge when green, route red to accuracy-gate-triage: PR #130 (B17 option 3, branch feature/accuracy-b17-board-done-for-off-board-issues @ 1fe6621) and PR #131 (B8 parsed corpus, branch feature/accuracy-b8-commit-parsed-corpus @ b585edc, ~20MB). Both were adversarially reviewed; #130's review findings are already fixed in 1fe6621. *** AFTER #130 MERGES, RUN THE FIVE CLOSES B17 EXISTS FOR: '.venv/bin/python scripts/accuracy_board.py done N' for N in 114 120 121 122 124 — all merged-but-open. Expect exit 0 and 'off-board: no project item'. NOT done this run: the mechanism landed but the closes are a deliberate separate step. *** THEN #88 items 2 and 4 (item 1 was #93, DONE; item 3 ratified earlier): parse the det failure set for STRATUM COVERAGE (est $4.88-6.83 but RE-COST after #93 — the denominator changed, 289 parse now vs 250), and raise per_run_token_ceiling (lemely.toml:19) recording the raise in DECISIONS.md. Then #57's costed preflight. *** MEASURED THIS RUN, use it instead of re-deriving: the corpus is 289/479 with 190 failures; paper_type counts theory_extended 87, mcq 79, theory_core 73, practical 26, alternative_practical 24; #93 unblocked exactly 39 0625 p2 schemes and 0625_m19_ms_22.json now records mcq/mcq. *** CORRECTIONS TO THE RECORD: (1) 'outputs/' is GITIGNORED and 'git ls-files outputs/' returns ZERO — the 2026-08-25 A2 note calling outputs/schemes/0625_s23_ms_22.json 'committed' was WRONG; it is local state. The corpus therefore lands in corpus/, not outputs/. (2) 'pgrep -f parse-mark-schemes' MATCHES ITS OWN SHELL COMMAND and falsely reports RUNNING — check the log mtime and the summary table instead. *** STILL ANSWERED-BUT-UNACTIONED: B1, B3, B4, B5, B6, B9, B10, B12, B13, B15, B16. B17's comment half is NOT fixed — cmd_comment still gates on board membership, so MISSION 13's finish-comment stays unreachable off-board; recorded in the module docstring. *** SECURITY: do NOT rotate the key (ruled); never echo its value; it must never reach a committed file, CI log, or public artefact. *** FOUR OPEN H ISSUES #49/#51/#52/#55; #49 keeps the split NOT FROZEN. Board Ready EMPTY.
---
