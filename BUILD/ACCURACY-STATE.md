...
spend_usd: 1.488057
in_the_middle_of: NOTHING IN FLIGHT, tree clean. *** #28 (M0.4) SWEEP RAN AND IS **NOT REPORTABLE** — AND THE NEXT STEP IS CODE, NOT SPEND. DO NOT RE-RUN THE SWEEP. *** run ablation-2026-08-24-a, --cache-mode bypass, cost $0.062546, ledger 1.425511 -> 1.488057, evidence committed under BUILD/accuracy-runs/ablation-2026-08-24-a/. THE ORACLE+MARK ARM PRODUCED ZERO RECORDS: measure_accuracy() picks the arm purely from case.scan_path (harness.py:670-671), all 11 golden fixtures ship scan.pdf, and no CLI flag or harness parameter can force the oracle branch — so only one arm could ever have run and there is no 2x2, no delta, same_denominator_both_arms=false, mcnemar()/ablation_2x2() degenerate at b=0 c=0 n_pairs=0 (reported as -1 sentinels, NOT as a null effect). *** MY SEQUENCING ERROR, recorded so it is not repeated: #28's own Why says 'all fixtures ship a scan.pdf, so the second arm is dead code' and its acceptance bullet 1 is 'Both arms run over all fixtures' — that is IMPLEMENTATION work. #28 is an implement-THEN-measure item and I treated it as a pure measurement. My own costed preflight even reported the oracle arm as 'currently dead code' and I launched anyway. The preflight worked; I failed to act on it. $0.0625 of avoidable spend. RULE: before launching any accuracy-measure run, check that every arm the mode needs can actually PRODUCE RECORDS on this corpus. *** WHAT #28 NEEDS NEXT (effort S, no spend): a real mechanism to run oracle+mark over cases that ALREADY have a scan_path — the arm cannot be selected by fixture shape when both arms must run over the SAME fixtures. Then paired EvalRecords over an identical (paper_id, question_id) leaf set, then ablation_2x2(). Even then n=31 is ~14% of MCNEMAR_IMPROVEMENT_N_FLOOR=219, so the honest output is a bounded NO-DETECTION statement, never a direction. MISSION 12.9: a not-reportable result is a SUCCESS and must not be answered by re-running — the fix is code. *** DESCRIPTIVE SINGLE-ARM RESULT, safe to cite with the caveat: extract+mark leaf accuracy 77.4% (24/31 distinct leaves), Wilson [60.2%, 88.6%]; det 8/8 (underpowered), gemini 16/23=69.6%; cache genuinely bypassed (_cache_hit_detected_any_attempt=false), 0 excluded, abstain/unmatched/under all in the denominator. TWO FIGURES THAT MUST NOT BE MISQUOTED: the manifest's embedded summary.mark_accuracy=0.9014 is over 71 PRE-DA6-COLLAPSE fixture-variant rows and is NOT the leaf figure (0.7742 is); and the distinct-leaf count is 31, NOT the 28 quoted in some earlier briefs (records re-counted independently). *** GENUINELY NEW: a live review-budget ratchet breach — review_rate_signal = review_rate_total = 29.03% (9/31; det 0/8, gemini 9/23=39.1%) against the 8%/10% ceilings, per-paper p95 66.7% vs the 15% target on only 5 papers (underpowered as a percentile). blocking_failure=False ONLY because review_rate_ratchet_armed=False at M0. *** #57 (M0.7b) STILL QUEUED AND UNSTARTED, authorised via option A: mark-scheme parsing pass to reach ~300 leaves. ORDERING: deterministic MCQ schemes FIRST at zero cost, THEN a costed preflight posted to the issue before any Gemini theory-scheme spend, $20.00 stop-and-ask. Do NOT freeze #57 over the 71 golden leaves (drop-only makes it irreversible). Make the parsing pass ITS OWN ISSUE rather than burying it in #57 — #45 (M2.2 census) needs the same parsed schemes and would otherwise duplicate the spend. *** LANDED THIS SESSION: #44 (5f7cde7), #40 (d203916), #46 (ba92be3), #36 (5cc58cb). Board M1 Done=3, M2 Done=2. Inbox 0 unchecked. #36 carries ONE strict xfail (bullet 6 unmet: all 71 rows of the saved A/A corpus have extraction_conf null, so propagation is never exercised there; strict=True fails loudly once a baseline re-run with per-answer extraction confidence closes it). SPEND 1.488057 raw of the 25.00 ceiling; $20.00 stop-and-ask is the only cap on #28/#57.
---
