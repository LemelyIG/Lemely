...
spend_usd: 1.488057
in_the_middle_of: #88 (M2.1b, NEW) created and In progress — the mark-scheme parsing pass that #57 was blocked on. Structuring choice the human left to me: DECIDED as its own issue, because #45 (M2.2) needs the identical det-failure set and a second run would duplicate the Gemini spend. ZERO-COST DET PARSE RUNNING: '.venv/bin/lemely parse-mark-schemes /home/sico/PaperScraper/papers --output-root /tmp/acc57-full/out', log /tmp/acc57-full/parse.log, output /tmp/acc57-full/out, ~1-4s/scheme over 479 mark schemes. If this run died, just re-run it — det-only, no network, no spend, idempotent (skips existing JSON). Count leaves with /tmp/acc57-full/count.py <out_dir>. NO SPEND HAS OCCURRED and none is authorised until a costed preflight is posted on #88. KEY FINDING ALREADY ESTABLISHED: parse path is det-if-det-succeeded, Gemini-if-det-failed (ChainedMarkSchemeParser), so DA1's Gemini-path strata are EMPTY under a det-only corpus and the det FAILURE SET is exactly the Gemini preflight denominator — a measured count, not an estimate. Probe: 40/40 MCQ (0625 p1) parsed det at zero cost in 12s; theory probe 8/12, 4 failed mark-total reconciliation. DEVIATION RECORDED: accuracy_board.py has NO add-to-project command, so #88 was added with 'gh project item-add' and linked under epic #43 with addSubIssue; every STATUS mutation still went through the guarded script.
---
