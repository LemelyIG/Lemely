...
spend_usd: 1.488057
in_the_middle_of: #45 (M2.2) round 4 IN FLIGHT on feature/accuracy-45-failure-reason-census-over-the-failing (tip c8fac52). Round 3 DELIVERED the substance: D7 revised DOWNWARD and published as 132/229 = 57.6% (from the false 69.4%), no bucket loosened in prose, artifact byte-for-byte reproduced by the reviewer from a live zero-cost re-run, denominator 229 unnarrowed, profiles.py diff empty, spend unchanged. Blocked on 4 things, all verified by the orchestrator. Work state (not a handle): artifact at BUILD/accuracy-runs/census-2026-08-24-b/; round-4 brief posted on the issue; NO PR. Workflow dead = UN-RUN, re-run accuracy-issue-execute {issue:45} (MISSION 7). *** (1) SUBSTANTIVE, 3rd round of the same class: the sufficiency gate at classify_failures.py:327 is 'empty_count>0 and (computed_total<=maximum_mark or excess<=empty_count)'. The FIRST disjunct is unconditional, so the DEFICIT direction has no magnitude check — round 3 implemented the property on the overcount side only and missed its mirror. 6 rows have computed_total < empty_count, impossible since each empty cell contributes >=1: 0606_w23_ms_11 emits '65 empty marks cell(s) defaulted to 1; computed_total=0', evidence falsifying its own label; also 0625_w21_ms_43 empty=119 computed=90, 0625_w21_ms_41 empty=96 computed=78, 0606_w19_ms_13 empty=69 computed=61, 0606_s23_ms_22 empty=60 computed=54, 0625_w21_ms_53 empty=54 computed=41. FIX = two-sided bound, and every bucket tested against a counterexample in BOTH directions. *** (2) SUBSTANTIVE: module docstring line 55 claims the 'computed < maximum_mark / 2' check is enforced in _classify_mcq AND _classify_theory. Verified false — the sole occurrence is line 469, inside _classify_mcq (437-490); _classify_theory starts at 491. Prose asserting enforcement that does not exist. Moves the headline 132/229 (57.6%) vs 130/229 (56.8%) under the docstring's own rule, and makes table_layout_extraction_failure=0 partly an artefact. *** (3) MECHANICAL, MINE, FIXED THIS RUN: c8fac52 blanked this line, leaving a trailing space; the whitespace hook stripped it in the worktree so pre-commit failed at HEAD and the tree was dirty. Restored to true state here. DO NOT blank or clear this line — accuracy-pr-land clears it after a real merge, nothing else. *** (4) GATE, AND THE WORKFLOW IS DOING IT WRONG: its gate list runs a bare whole-repo 'pytest -q', which was KILLED at ~68% with no exit code. That suite takes 20.2min and is the SUPERVISOR's job, not a run's. Round 4 must use 'scripts/check.sh --fast tests/test_census_45.py' instead; a bare whole-repo pytest must never be launched from inside a run. *** STILL DO NOT TOUCH lemely/io/det/profiles.py (question 2 on #88, the human's). ZERO SPEND, ledger 1.488057. *** #88 UNCHANGED: In progress, blocked, its 3 questions (2026-08-24T04:33Z) unanswered for a 6th run. *** origin/develop is 8 commits BEHIND local develop, so #88's commits would ride inside #45's diff — a HUMAN must reconcile the refs before accuracy-pr-land opens the PR. Confirmed origin/develop..origin/main is 0.
---
