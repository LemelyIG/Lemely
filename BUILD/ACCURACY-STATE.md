...
branch: feature/accuracy-38-defaulted-mark-provenance @ 52765fa — PR #148 OPEN, awaiting CI
...
spend_usd: 3.138147
in_the_middle_of: RUN 54 — PR #148 (#38 bullet 1) OPEN, 4/5 CI GREEN, only 'test (3.14)' still in_progress. Poll background id bdogidr2q, log /tmp/claude-1001/-home-sico-Lemely-worktrees-accuracy/dfc5ba97-22db-4b2f-87d0-c8567da1e6c9/tasks/bdogidr2q.output (re-reads PR head; 'TIMEOUT' = UNKNOWN not red). Merge 'gh pr merge 148 --squash' once 3.14 lands green. **#38 MUST STAY OPEN** — the PR says Refs not Closes; bullets 2+3 are escalated, not done. *** ZERO SPEND, ledger 3.138147. *** LANDED THIS SESSION: #145 (d9c5c45, #136 diagnosis), #146 (037f8e1, DA21+close), #147 (e04d0e0, #37) with #37 CLOSED. *** #38 bullet 1 (marks_defaulted) shipped in #148 with 5 tests, 3 proven failing first. Bullets 2+3 ESCALATED (issuecomment-5432831135): their shared premise that defaulting is RARE is falsified — 44.4% of papers carry >=1 defaulted point, 21.6% of ALL answer points are defaulted (60-PDF sample, 54 parsed). Arming bullet 3 as written routes ~44% of the corpus to PAID Gemini. Bullet 2 says 'the minted placeholder point' singular but 0625_s19_ms_43 is over-sum by +1 while carrying 50 defaulted points — deleting one arbitrary point makes the checksum match and leaves 49, i.e. making a number match rather than repairing a defect. TWO QUESTIONS POSTED, candidate narrower predicate offered (defaulted AND answer text ends in a multi-mark code = the lossy DA21 (B) subset) but NOT chosen. *** #39 SCOPING NOTE POSTED (issuecomment-5433004082), NOT STARTED, no branch. FINDING: bullet 3 (paper-level Gemini aggregate) only LOOKS blocked by A8's retirement of the is_excerpt bullet. Verified in source — det's gate runs at PARSE time (det/parser.py:88, inside DeterministicMarkSchemeParser, NOT a pydantic validator), while golden fixtures are never parsed (load_golden_cases, harness.py:130, round-trips already-parsed mark_scheme.json through MarkScheme.model_validate_json). So a gate placed in lemely/io/mark_schemes.py at parse time is STRUCTURALLY INCAPABLE of seeing the 6 excerpt fixtures and needs no is_excerpt exemption. A gate written as a pydantic validator WOULD reject them — placement decides it. Does NOT reinstate the retired bullet, does NOT touch A9 (fix stays parser-side, sweep stays DEFERRED). UNRESOLVED caveat recorded: if a Gemini-parsed scheme is ever legitimately an excerpt at parse time, a parse-time gate rejects it with no exemption. Also flagged: any under-sum check (bullet 2) interacts with the 21.6% marks_defaulted leak — a paper can be under-sum BECAUSE of the leak, and bullet 9's zero-false-positive requirement must NOT be loosened. *** #127 still unlabelled/untriaged, unexamined. *** STILL HUMAN-GATED: B15/#112+#110 sweep waiver (also gates #136 fix half), #88 item 2 DO NOT RESUME, B1/#41 (branch @1d41a97, NO PR by design), #58's two questions, B14/H #49/#51/#52/#55 (split NOT FROZEN). #59 not Ready; #95 blocked on #136. *** STANDING LESSON: a 'done' note is not evidence — #37 was caught only by reading develop directly.
---
