...
spend_usd: 2.704228
in_the_middle_of: RUN 40 — *** A BACKGROUND JOB IS RUNNING: the B8 parsed-corpus regeneration. COMMAND: .venv/bin/python -m lemely.app.cli parse-mark-schemes /home/sico/PaperScraper/papers --output-root outputs/schemes-regen-8758dba --on-error continue ; LOG: outputs/parse-logs/parse-8758dba.log ; OUTPUT DIR: outputs/schemes-regen-8758dba/ . Poll with 'ls outputs/schemes-regen-8758dba/ | wc -l' and 'tail outputs/parse-logs/parse-8758dba.log'. 479 source PDFs; ~40 min; $0.00 det-only, NO --use-gemini. Expect ~289 parsed (not 250) because #93 fixes 41 that previously ParseError'd. If the process died, restart it with the same command — it is idempotent into a fresh root. *** PARSER SHA IS 8758dba (develop tip when the job started) and MUST be recorded in the corpus manifest per ask B8. *** BLOCKER FOUND, NEEDS A DECISION BEFORE THE CORPUS CAN BE COMMITTED: 'outputs/' is GITIGNORED (.gitignore:46) and 'git ls-files outputs/' returns ZERO — nothing under outputs/ has ever been tracked. So B8's 'commit the parsed schemes to the repo' cannot be done in place. Options: (a) 'git add -f outputs/schemes/', which fights an ignore that exists to keep run artifacts out; (b) a dedicated TRACKED directory, which I prefer. ALSO CORRECTS THE RECORD: the 2026-08-25 A2 inbox note called 'the committed outputs/schemes/0625_s23_ms_22.json' committed — IT IS NOT, and never was; it is local gitignored state. Any reasoning that treated it as a committed corpus is built on a false premise. *** MERGED THIS RUN: PR #128 (#93, B2 waiver + B7 folded in) -> 3c40ffd, and PR #129 (run-39 close) -> 8758dba. Both were green on all five CI jobs. develop tip = 8758dba. *** NEXT AFTER THE CORPUS: #88 items 1-2 and 4 (item 1 IS #93, done) — parse the det failure set for STRATUM COVERAGE (est $4.88-6.83, RE-COST after #93), and raise per_run_token_ceiling (lemely.toml:19) recording the raise in DECISIONS.md; then #57's costed preflight. *** STILL ANSWERED-BUT-UNACTIONED: B1, B3, B4, B5, B6, B9, B10, B12, B13, B15, B16, B17. B17 option 3 then closes #114/#120/#121/#122/#124, all landed-but-open. *** SECURITY: do NOT rotate the key (ruled); never echo its value into a transcript; it must never reach a committed file, CI log, or public artefact. *** FOUR OPEN H ISSUES #49/#51/#52/#55; #49 keeps the split NOT FROZEN. Board Ready EMPTY.
---
