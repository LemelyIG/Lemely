...
spend_usd: 0.4026
in_the_middle_of: #29 (M0.5 honest denominators) IMPLEMENTED in this run: D18 fixed in lemely/accuracy/harness.py (loop now iterates case.ground_truth, not correction.questions), exclusion funnel wired into format_report(), DA7 + docs/ACCURACY-STRATEGIES.md updated with legacy 83.8% vs honest baseline (90.1% raw n=71 / 77.4% DA6-collapsed n=31). Tests added and pre-commit run. NOT YET committed as of this note (see commit step next). FOUR (still-unmerged) BRANCHES QUEUED BEHIND THE BILLING BLOCK, DO NOT REDO ANY OF THEM: (a) #77 -> PR #78 OPEN, reviewed clean; (b) #30 (M0.6 paired stats) at 3f569ee, PUSHED, no PR; (c) #33 (M0.9 ratchet) at 693d76e, PUSHED, no PR; (d) #29, this branch, now has commits. ORDER WHEN BILLING IS FIXED: (1) 'gh pr checks 78', if green merge+delete-branch, accuracy_board.py done 77; (2) accuracy-pr-land for #30; (3) accuracy-pr-land for #33; (4) accuracy-pr-land for #29; (5) THEN #27 (M0.3 A/A floor), MUST use --cache-mode bypass, never refresh. REBASE HAZARD: #30, #33, #29 all cut from origin/develop; #30/#33 both touch lemely/eval/analyses.py (#30 adds McNemar n-floor block; #33 changes review_rate's numerator + _group_by_leaf); #29 also touches lemely/eval/analyses.py imports from harness.py plus the exclusion funnel. Expect textual, not semantic, conflicts on integration — whichever lands later must rebase and re-run gates, not merge blind. CRITICAL CROSS-ISSUE INTERACTION: #29 CHANGES THE DENOMINATOR #33's 29.03% review-rate baseline sits on (unmatched/excluded rows change leaf/row counts). #29 deliberately did NOT touch #33's artifacts (BUILD/review-rate-baseline.json, review_rate_last_merged in lemely/runtime/config.py) per orchestrator note. AFTER BOTH #29 and #33 MERGE, that baseline MUST BE RECOMPUTED by whichever follow-up branch does it -- do not skip this. #29's own honest baseline is NOT 83.8%; every reference to 83.8% is now labelled historical in BUILD/DECISIONS.md DA7 and docs/ACCURACY-STRATEGIES.md. STILL HALTED: GitHub Actions is billing-blocked for LemelyIG -- a human must fix Settings > Billing & plans; nothing can merge. Do NOT re-triage or re-run that workflow. spend_usd 0.4026 is a lower bound (stale pricing); #29's local honest-baseline run hit Gemini cache only, no fresh spend. Precondition re-checked: origin/develop..origin/main = 0. WORKFLOW HAZARD (seen repeatedly): do not blank this key or title a commit 'cleared' until #29 has ACTUALLY landed (merged), only after real commits exist on this branch. RE-RUN EVERY GATE YOURSELF; an ImportError is NOT a falsification. STANDING RED GATE, do NOT re-triage: BUILD/BLOCKERS.md:666. ENV: jq NOT installed (use gh --jq); pre-commit needs .venv/bin on PATH; verify signing with git cat-file commit <sha> | grep -c gpgsig, NEVER %G?.
---
