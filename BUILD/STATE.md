...
CURRENT PHASE:      **6 IN PROGRESS. 6.1 (`adapt`) DONE**, verified green on a
                    full run. 6.2-6.5 remain.
CURRENT SURFACE:    Phase 6 is a sweep, not a surface.
NEXT ACTION:        **Phase 6.2 - `harden`.** Error, edge, long-content and
                    slow-network passes on the critical flows, and §6.2 names
                    one of them outright: **the paper upload + marking wait**,
                    which is the only flow in the product with real latency.
                    Design the waiting experience, the progress feedback and
                    the failure recovery properly rather than checking a
                    spinner exists. Then 6.3 `optimize`, 6.4 accessibility,
...
LAST UPDATED:       2026-08-14T14:45+03:00
