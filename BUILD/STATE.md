...
CURRENT PHASE:      **6 IN PROGRESS. 6.1 (`adapt`) and 6.2 (`harden`) DONE.**
                    6.3-6.5 remain.
CURRENT SURFACE:    Phase 6 is a sweep, not a surface.
NEXT ACTION:        **Phase 6.3 - `optimize`.** transform/opacity-only
                    animation, no blur on scrolling content, the z-index scale
                    respected, lazy images (WebP/AVIF), skeletons that reserve
                    space (CLS < 0.1), and a font-display strategy. Then 6.4
                    accessibility (there is an axe harness already in
                    `scripts/audit.mjs` and `e2e/phase4-journey.spec.ts`), then
                    6.5 strategic omissions.
...
LAST UPDATED:       2026-08-14T16:20+03:00
