...
last_updated: 2026-08-10T13:44:00Z   # **Ninety-first session: attached to the SAME run, still alive at 32m18s, 11/13, inside `puppeteer-audit`.** Wrapper PID **`1541133`** / `check.sh` PID **`1541134`** (32m18s in `ps`), audit child `node scripts/audit.mjs` PID **`1591614`** (8m35s) — one continuous run, **NOT relaunched**; Monitor re-armed persistently on `EXIT=`/`^FAIL`/`^PASS `/Traceback/Error/Killed/OOM. All eleven gates through `playwright-e2e` PASS; only `puppeteer-audit` and `ui-thresholds` remain. INBOX had no unhandled items; no open blockers. Working tree on entry: **195 paths, every one under `reports/phase-5/`, ZERO outside, zero `D` entries** — the expected mid-audit reading, so again **no wip commit**. **No source file touched.** **The procedure when `EXIT=` arrives is unchanged and must not be re-derived — session-85 lines below.**
#                                    Prior (session 90): (Measured with `date -u`, not carried forward.) **Ninetieth session: `playwright-e2e` PASSED. The gate this entire relaunch existed to answer is GREEN — 27a0be2's pinned-`deviceId` `storageState` fix is CONFIRMED against the live stack, not inferred.** The run is now **11/13** and inside `puppeteer-audit`: `node scripts/audit.mjs` PID **`1591614`** (parent `sh -c` `1591613`) is visible in `ps`, so the audit leg is doing real work. Only `puppeteer-audit` and `ui-thresholds` remain. Same wrapper PID **`1541133`** / `check.sh` PID **`1541134`** sessions 84–89 recorded, confirmed from `ps` — one continuous run started **16:09:23 local (27 min elapsed at entry)**, attached, Monitor re-armed persistently with `tail -F` on `EXIT=`/`^FAIL`/`^PASS `/Traceback/Error/Killed/OOM, **NOT relaunched**. INBOX had no unhandled items; B1/B2/B3 all resolved, no open blockers. **No source file touched.**
#                                    **Working tree on entry: 195 paths (142 untracked + 53 modified), every one under `reports/phase-5/`, ZERO outside, zero `D` entries** — the expected reading for an audit leg mid-write, so again **no wip commit**. Committing `reports/` now would capture partial evidence (the trap `96ce495`/`4e7e772` sprang in session 72), and `pre-commit run --all-files` is unusable while the audit is writing into `reports/` — both already recorded below.
#                                    **What is left to decide, and the only thing that can still go wrong:** `puppeteer-audit` re-runs the same audit that already landed `EXIT=0` in session 76, but this time on the post-`a4b73e9` tree, so a regression there would be new information; `ui-thresholds` then reads the corpus that audit writes. **A `FAIL` line is NOT the end of the run — `check.sh` does not abort on a failed gate** (the trap that cost session 82→83). Decide liveness from `ps` and the absence of `EXIT=`, never from a gate line.
#                                    **The procedure when `EXIT=` arrives is unchanged and must not be re-derived — it is spelled out on the session-85 lines below.**
#                                    Prior (session 89): the run advanced to 10/13 at 20m12s and entered `playwright-e2e`; `pytest` plus all five web gates had cleared. Timing calibration kept from that entry: the previous run hit 10/13 at 18m36s, so ~1.5 min of variance is normal.
#                                    Prior (session 88): **Eighty-eighth session: THE RELAUNCHED RUN — THE FIRST EVER TO EXERCISE 27a0be2 — IS STILL ALIVE AT 15m54s ON ENTRY AND IS STILL 4/13 (`ruff-check`/`ruff-format`/`mypy`/`import-linter` PASS, inside `pytest`).** Same wrapper PID **`1541133`** / `check.sh` PID **`1541134`** sessions 84–87 recorded, confirmed from `ps` rather than inferred from the log — one continuous run, attached, Monitor re-armed persistently with `tail -F` on `EXIT=`/`^FAIL`/`^PASS `/Traceback/Error/Killed/OOM, **NOT relaunched**. The `pytest` child is visible as PID **`1541223`**, which is the direct confirmation that the flat gate count is work rather than a hang. INBOX had no unhandled items; B1/B2/B3 all resolved, no open blockers. Working tree on entry: **196 paths, every one under `reports/phase-5/` and ZERO outside, zero `D` entries** — identical to sessions 85–87, the expected reading and not a stall, because `pytest` writes nothing into `reports/phase-5/`; the corpus only grows once `playwright-e2e` and `puppeteer-audit` start. So again **no wip commit**. **No source file touched.**
#                                    **Nothing new was investigated this session, deliberately** — same call as sessions 86/87, for the reason session 72 recorded: the P5.11 brief is done (sixteen measured points, "do not add a seventeenth"), P5.12 was fully briefed in session 60, and every gate on this tree except `playwright-e2e` is already known-green from sessions 83/84's held lanes. Nothing reading more code can answer will move P5.11; only the run can. Attach, arm, record.
#                                    **This session's entry time is the one that matters for reading the next one: 15m54s and still 4/13 is INSIDE the window session 87 measured as normal** (the previous run sat at 4/13 from 7m29s through 14m53s and only reached 10/13 at 18m36s). So the jump to 10/13 is now genuinely imminent, and it, not the gate count, is the first real signal. **Do not read stillness as a stall before ~19 minutes; past ~19 minutes with no 10/13, that IS worth investigating.**
#                                    **The one thing this run is being watched for is `playwright-e2e`, and nothing else. The procedure when `EXIT=` arrives is unchanged and must not be re-derived — it is spelled out on the session-85 lines below.**
#                                    Prior (session 87): **Eighty-seventh session: THE RELAUNCHED RUN — THE FIRST EVER TO EXERCISE 27a0be2 — IS STILL ALIVE AT 12m19s ON ENTRY AND IS STILL 4/13 (`ruff-check`/`ruff-format`/`mypy`/`import-linter` PASS, inside `pytest`).** Same wrapper PID **`1541133`** / `check.sh` PID **`1541134`** sessions 84–86 recorded, confirmed from `ps` rather than inferred from the log — one continuous run, attached, Monitor re-armed persistently with `tail -F` on `EXIT=`/`^FAIL`/`^PASS `/Traceback/Error/Killed/OOM, **NOT relaunched**. INBOX had no unhandled items; B1/B2/B3 all resolved, no open blockers. Working tree on entry: **196 paths, every one under `reports/phase-5/` and ZERO outside** — identical to sessions 85/86, which is the expected reading and not a stall, because `pytest` writes nothing into `reports/phase-5/`; the corpus only grows once `playwright-e2e` and `puppeteer-audit` start. So again **no wip commit**. **No source file touched.**
#                                    **Nothing new was investigated this session, deliberately** — same call as session 86, and for the reason session 72 recorded: the P5.11 brief is done (sixteen measured points, "do not add a seventeenth"), P5.12 was fully briefed in session 60, and every gate on this tree except `playwright-e2e` is already known-green from sessions 83/84's held lanes. Nothing reading more code can answer will move P5.11; only the run can. Attach, arm, record.
#                                    **One measurement worth carrying, because it changes what "unchanged at 4/13" means: `pytest` is the long pole of this run and 12 minutes inside it is normal, not a hang.** Sessions 77–79 saw the previous run sit at 4/13 from 7m29s through 14m53s and only reach 10/13 at 18m36s. So a flat gate count here is the expected shape of the run, and the first real signal is the jump to 10/13. Do not read stillness as a stall before ~19 minutes.
#                                    **The one thing this run is being watched for is `playwright-e2e`, and nothing else. The procedure when `EXIT=` arrives is unchanged and must not be re-derived — it is spelled out on the session-85 lines below.**
#                                    Prior (session 86): **Eighty-sixth session: THE RELAUNCHED RUN — THE FIRST EVER TO EXERCISE 27a0be2 — IS STILL ALIVE AT 8m00s ON ENTRY AND IS UNCHANGED AT 4/13 (`ruff-check`/`ruff-format`/`mypy`/`import-linter` PASS, inside `pytest`).** Same wrapper PID **`1541133`** / `check.sh` PID **`1541134`** sessions 84/85 recorded, confirmed from `ps` rather than inferred from the log — one continuous run, attached, Monitor re-armed persistently with `tail -F` on `EXIT=`/`^FAIL`/`^PASS `/Traceback/Error/Killed/OOM, **NOT relaunched**. INBOX had no unhandled items; B1/B2/B3 all resolved, no open blockers. Working tree on entry: **196 paths, every one under `reports/phase-5/` and ZERO outside** — identical to session 85's count, which is the *expected* reading rather than a stall, because `pytest` writes nothing into `reports/phase-5/`; the corpus only grows once `playwright-e2e` and `puppeteer-audit` start. So again **no wip commit**: the dirt is the live run's output, and freezing a mid-run snapshot is what produced session 73's partial-corpus trap. **No source file touched.**
#                                    **Nothing new was investigated this session, deliberately, and that is the correct use of a held lane at this point.** Session 72 recorded "the brief is done, do not add a seventeenth point" for P5.11, P5.12 was fully briefed in session 60, and every gate on this tree except `playwright-e2e` is already known-green from session 84's held lane. There is no open question left that reading more code can answer — the only thing that can move P5.11 is the run itself. Attach, arm, record.
#                                    **The one thing this run is being watched for is `playwright-e2e`, and nothing else. The procedure when `EXIT=` arrives is unchanged and must not be re-derived — it is spelled out on the session-85 lines directly below.**
#                                    Prior (session 85): **Eighty-fifth session: THE RELAUNCHED RUN — THE FIRST EVER TO EXERCISE 27a0be2 — IS ALIVE AT 3m48s ON ENTRY, 4/13 (`ruff-check`/`ruff-format`/`mypy`/`import-linter` PASS).** Same wrapper PID **`1541133`** / `check.sh` PID **`1541134`** session 84 launched, confirmed from `ps` (not inferred from the log), so this is one continuous run — attached, Monitor re-armed persistently with `tail -F` on `EXIT=`/`^FAIL`/Traceback/Error/Killed/OOM/`^PASS `, **NOT relaunched**. INBOX had no unhandled items; B1/B2/B3 all resolved, no open blockers. Working tree on entry: **196 paths, every one under `reports/phase-5/` and ZERO outside**, so again **no wip commit** — the dirt is the live run's output, and freezing a mid-run snapshot is what produced session 73's partial-corpus trap. `git status reports/phase-5` shows **zero `D` entries**, which mid-run proves nothing; the load-bearing check is the one at completion. **No source file touched.**
#                                    **The one thing this run is being watched for is `playwright-e2e`, and nothing else.** Session 83's held lane already banked `puppeteer-audit` PASS and `ui-thresholds` PASS on this exact tree, and neither gate reads `web/playwright.config.ts` or `web/e2e/`, so those two verdicts carry over — every gate except `playwright-e2e` is known-green here. If `playwright-e2e` clears, the run is green and P5.11 is done; if it fails, the failure is about 27a0be2 specifically and not about anything else in the tree. **Do not re-derive the rest of the gate list as open questions.**
#                                    **Procedure when `EXIT=` arrives — unchanged, do not re-derive: on `EXIT=0` ONLY — `npm run compare-screens -- --baseline reports/phase-4/screens --candidate reports/phase-5/screens` (the default baseline is Phase 2.5 and is wrong), re-check `git status reports/phase-5` for `D` entries, then commit `reports/phase-5/`; P5.11 is then DONE and only P5.12 remains. On anything else, do not commit the corpus.** And session 83's generalisable rule still governs the read: **a FAIL line in the log is NOT the end of the run — `check.sh` does not abort on a failed gate.** Decide liveness from `ps` and the absence of `EXIT=`, never from a gate verdict.
#                                    Prior (session 84): **Eighty-fourth session: THE RED RUN LANDED (`EXIT=1`) AND — THE PART WORTH KEEPING — ITS LAST TWO GATES BOTH PASSED. `puppeteer-audit` PASS AND `ui-thresholds` PASS.** Session 83 held the lane specifically to buy this signal, and it paid: chunk E's G-01 horizontal-scroll fix and the `heading-order` fix both cleared `check_ui_gates.py`, on this exact tree. Final tally 12 PASS / 1 FAIL, the single failure still `playwright-e2e` (the pre-fix `teacher-journey` 409). **So the relaunch's only open question is `playwright-e2e` itself — every other gate is now known-green on this tree.** The corpus was NOT committed (run red; that discipline is what neutralizes session 73's partial-corpus trap); the red log is preserved at `/tmp/p511-gate-red.log`.
#                                    **RELAUNCHED, and this is the first run that has ever exercised 27a0be2.** Lane confirmed free (`pgrep` clean) before launch, then full `./scripts/check.sh` under `setsid` with `LEMELY_REPORT_DIR=reports/phase-5` exported — new wrapper PID **`1541133`**, `check.sh` PID **`1541134`**, log `/tmp/p511-gate.log`, ends in an `EXIT=` line. Already 4/13 (`ruff-check`/`ruff-format`/`mypy`/`import-linter` PASS). **Do NOT relaunch it.** Monitor armed persistently — note it uses `tail -F`, not `-f`: the previous monitor was left tailing a deleted inode when the log was rotated, and `-F` is what survives that.
#                                    **Two preconditions of the fix were verified from primary source this session rather than trusted, because both fail ~10 seconds into a run and cost the whole 30 minutes.** (a) `web/e2e/device-state.json` exists, is git-tracked, and carries `lemely.deviceId = e2e00000-0000-4000-8000-000000000001` under origin `http://127.0.0.1:5173` — which **matches `baseURL` exactly** (`playwright.config.ts:73`). A `localhost`-vs-`127.0.0.1` mismatch there would have silently dropped the pinned id and reproduced the identical 409 with no clue as to why. (b) `storageState` (`:110`) sits inside the **top-level** `use:` block (`:72-111`), and the project-level `use` (`:115`) spreads only `devices["Desktop Chrome"]`, which carries no `storageState` — so it merges rather than overrides.
#                                    **One class of bypass the fix's own reasoning never covered, now checked and clean: a spec that builds its own context gets no pinned id.** All eight `newContext` call sites in `web/e2e/` (`screenshots` ×2, `correct-paper`, `rbac` ×4, `at-risk-flags`) are `playwright.request.newContext()` — **API** request contexts, not browser contexts — so none of them opens a UI session or bypasses `storageState`. **Worth re-checking if a future spec adds a `browser.newContext()`: that one WOULD mint a fresh device id and re-open the exact hole 27a0be2 closed.**
#                                    **Procedure when `EXIT=` arrives: on `EXIT=0` ONLY — `npm run compare-screens -- --baseline reports/phase-4/screens --candidate reports/phase-5/screens` (the default baseline is Phase 2.5 and is wrong), re-check `git status reports/phase-5` for `D` entries, then commit `reports/phase-5/`; P5.11 is then DONE and only P5.12 remains. On anything else, do not commit the corpus.** Working tree on entry: **196 paths, every one under `reports/phase-5/` and ZERO outside**, so again **no wip commit** — the dirt is the run's output, not leftover work.
#                                    Prior (session 83): **Eighty-third session: SESSION 82'S "NEXT SESSION'S FIRST JOB" COULD NOT BE DONE, AND FINDING OUT WHY IS THIS SESSION'S ONE REAL RESULT: THE RED GATE RUN IS STILL ALIVE. `check.sh` DOES NOT ABORT ON A FAILED GATE.** Same wrapper PID `1457845` / `check.sh` PID `1457847` sessions 76–82 recorded, at **29m34s** on entry, now inside `puppeteer-audit` (`node scripts/audit.mjs`, 7m26s elapsed). The log carries `FAIL playwright-e2e` at line 16 and then *keeps going* — 10 PASS, 1 FAIL, and the `EXIT=` line has not been written. **Session 82 wrote its plan as though the run had ended when it read the failure; it had not.** Relaunching on entry — the instruction that plan gave — would have re-run `globalSetup` and re-seeded the database underneath the in-flight audit, which is the exact collision session 82 itself identified and then unintentionally set up the next session to cause. **Do not relaunch until the `EXIT=` line exists.** Monitor armed persistently on `EXIT=`/FAIL/Traceback/Error/Killed/OOM/`PASS `.
#                                    **The generalisable form, worth more than this instance: a FAIL line in `check.sh`'s log is NOT the end of the run.** Every prior session inferred liveness from PIDs and gate counts; session 82 inferred *deadness* from a failure message. Read `ps` and the absence of `EXIT=`, never a gate verdict, to decide whether the lane is free.
#                                    **The 27a0be2 fix was verified from primary source this session rather than re-trusted, and it holds.** Session 82 asserted the pinned `deviceId` cannot weaken G-10; that claim is now confirmed in `lemely/db/device_repo.py:199-209` — `_match_existing` returns `None` when `client_device_id` is `NULL` *and* when a non-NULL id matches no row, so the pinned `e2e00000-…-0001` matches none of G-10's three seeded NULL-id rows → `device is None` → with `allow_eviction=False` and 3 live devices the 409 challenge still fires (`:137-141`, `MAX_DEVICES=3` at `:39`). The same read confirms the fix's *other* half: a stable id that DOES match a live row is reused, so the shared teacher's repeated UI logins now refresh one row instead of consuming three slots. **No source file was touched this session** — the tree is the one the next run must verify.
#                                    **The two gates still to land in this run are real signal and are worth waiting for, even though the run is red overall.** `puppeteer-audit` and `ui-thresholds` do not touch the deviceId path, so their verdicts on this tree carry over to the relaunch — if they pass here, the relaunch's only open question is `playwright-e2e` itself. That is the cheap way to learn whether chunk E's G-01 horizontal-scroll fix and the `heading-order` fix actually cleared `check_ui_gates.py`.
#                                    **Procedure when `EXIT=` arrives (unchanged in substance from session 82, with the ordering corrected): (1) the run is red — do NOT commit the corpus, whatever the remaining gates said; (2) relaunch the full `./scripts/check.sh` under `setsid` with `LEMELY_REPORT_DIR=reports/phase-5` exported, which is what actually exercises 27a0be2 for the first time; (3) only on `EXIT=0` from THAT run, `npm run compare-screens -- --baseline reports/phase-4/screens --candidate reports/phase-5/screens` (the default baseline is Phase 2.5 and is wrong), re-check `git status reports/phase-5` for `D` entries, then commit `reports/phase-5/`.** Working tree on entry: **196 paths, every one under `reports/phase-5/` and ZERO outside**, so again **no wip commit** — the dirt is the live run's output, and freezing a mid-run snapshot is what produced session 73's partial-corpus trap.
#                                    Prior (session 82): **Eighty-second session: THE GATE RUN WENT RED AND THE CAUSE IS FOUND, DIAGNOSED FROM EVIDENCE, AND FIXED. `playwright-e2e` FAILED at 33/34 — every other gate before it PASSED (`ruff-check`/`ruff-format`/`mypy`/`import-linter`/`pytest` + all five web gates), and ALL FOUR of P5.11's new acceptance flows PASSED**, including `engagement.spec.ts`'s leaderboard-ordering assertion (the one criterion MISSION §4 names for this phase), the two-role announcement flow, and both `reduced-motion` specs. The single failure is `teacher-journey.spec.ts:22` — pre-existing, untouched by P5.11 — whose teacher login never left `/login`.
#                                    **ROOT CAUSE (confirmed from the failure's own `error-context.md` page snapshot, NOT inferred): the login screen was rendering G-10's 409 device-limit challenge — `region "You're signed in on 3 devices"`, three `Chrome on Windows` rows active 2/2/3 minutes ago. The teacher login was not broken; the D1.11 3-device limit was working.** Playwright hands every test a fresh context → fresh `localStorage` → a fresh `deviceId`, so each real UI login as the *shared seeded teacher* consumed a distinct slot. Before P5.11 there were exactly three (`reduced-motion` twice via its `gotoSchemes` helper, plus `teacher-journey`) — **exactly at `MAX_DEVICES=3`, zero headroom**. P5.11's new `engagement.spec.ts` teacher login made a fourth, and `teacher-journey` runs LAST alphabetically, so it took the 409. **This is session 62's dedicated-account trap firing on the teacher account: the seed correctly dedicated an account for G-10, but nothing applied the same reasoning to the shared teacher, which was already at the cap.**
#                                    **THE FIX (committed): one global `use.storageState` in `web/playwright.config.ts` pinning a single `lemely.deviceId` (`web/e2e/device-state.json`) across every context.** This is the product's designed semantics, not a workaround — `lemely/db/device_repo.py`'s own contract says a stable client-supplied `client_device_id` makes a re-login *reuse its row rather than consume a slot*, and `src/lib/auth/storage.ts` mints it once into `localStorage` for exactly that reason. One CI browser on one machine IS one device; the per-test fresh id was the test-isolation artifact. **Chosen global rather than per-spec deliberately:** converting one spec to `injectSession` would get under the cap today but leave headroom of one, so the next spec that logs in as a shared account would break a *different, alphabetically later* spec — silent and order-dependent. **Verified preconditions rather than assuming: (a)** `injectSession` only sets `lemely.session` and never clears storage, so it cannot clobber the pinned id; **(b)** every run mints a fresh `run_tag` and `_signup_account`s a brand-new teacher, so the account starts each run with zero client-device rows and stale rows from prior runs belong to different accounts (signup's own NULL-id row plus the one pinned-id row = 2 of 3, headroom of 1); **(c) it does not weaken G-10** — the dedicated device-limit account's three seeded rows carry `client_device_id=None` precisely so no browser can match one, so a pinned id still gets its 409.
#                                    **NOT YET VERIFIED BY A RUN, and that is the next session's first job.** The fix could not be exercised while the audit legs still held the stack: `playwright-e2e` re-runs `globalSetup`, which would re-seed the database underneath the in-flight `puppeteer-audit`. Editing `web/e2e/` + `playwright.config.ts` mid-run was safe because neither remaining gate reads them. **The corpus from this run must NOT be committed — the run is red, and "never commit the corpus on anything but `EXIT=0`" is what neutralizes session 73's partial-corpus trap.** Next session: re-run the full `./scripts/check.sh` with `LEMELY_REPORT_DIR=reports/phase-5` exported, under `setsid`, and only on `EXIT=0` run `npm run compare-screens -- --baseline reports/phase-4/screens --candidate reports/phase-5/screens` (default baseline is Phase 2.5 and is wrong), re-check for `D` entries, then commit `reports/phase-5/`.
#                                    Prior (session 81): **Eighty-first session: the same P5.11 gate run is STILL ALIVE at 20m28s on entry and is UNCHANGED at 10/13 the same P5.11 gate run is STILL ALIVE at 20m28s on entry and is UNCHANGED at 10/13 — the log is still 254 bytes, so `playwright-e2e` has been running for ~2 minutes and has neither passed nor failed.** Same wrapper PID `1457845` and same `check.sh` PID `1457847` sessions 76–80 recorded: one continuous run — attached, Monitor re-armed persistently on `EXIT=`/FAIL/Traceback/Error/Killed/OOM/`PASS `, NOT relaunched. INBOX had no unhandled items; B1/B2/B3 all resolved, no open blockers. Working tree on entry: **195 paths, every one under `reports/phase-5/` and ZERO outside** (session 80 committed its own STATE note in `2785538`), so again **no wip commit** — same call sessions 74–80 made: the dirt is the live run's output, and freezing a mid-run snapshot is what produced session 73's misleading partial corpus. `git status reports/phase-5` shows **zero `D` entries**, still the mid-run reading that proves nothing; the load-bearing check is the one at completion. **No source file touched.** The post-`EXIT=` procedure is unchanged and recorded below; do not re-derive it. **The 195-vs-190 path growth between sessions 80 and 81 is the only new signal and it is a healthy one:** the corpus is still being written, so `playwright-e2e` is doing work rather than hanging. **No new brief work was attempted, deliberately** — session 72 recorded "the brief is done, do not add a seventeenth point" for P5.11, and P5.12 was fully briefed in session 60; inventing more measurement now would be motion, not progress. The correct use of a held lane at this point is to attach, arm, and record.
#                                    Prior (session 80): **Eightieth session: the same P5.11 gate run is STILL ALIVE at 18m36s on entry and has ADVANCED to 10/13 — `pytest` cleared plus all four web gates (`web-typecheck`/`web-lint`/`web-build`/`web-test`/`impeccable-detect`), so only `playwright-e2e`, `puppeteer-audit` and `ui-thresholds` remain.** Same wrapper PID `1457845` and same `check.sh` PID `1457847` sessions 76–79 recorded: one continuous run, ~19 minutes in — attached, Monitor re-armed persistently on `EXIT=`/FAIL/Traceback/Error/Killed/OOM/`PASS `, NOT relaunched. INBOX had no unhandled items; B1/B2/B3 all resolved, no open blockers. Working tree on entry: **190 paths, every one under `reports/phase-5/` and ZERO outside** (session 79 committed its own STATE note in `eebf1a4`), so again **no wip commit** — same call sessions 74–79 made: the dirt is the live run's output, and freezing a mid-run snapshot is what produced session 73's misleading partial corpus. **No source file touched.** The post-`EXIT=` procedure is unchanged and recorded below; do not re-derive it. **Note for the session that catches the exit: `pytest` passing here means the backend leg is banked, but the three remaining gates are the expensive live-stack ones — `playwright-e2e` is where P5.11 chunks A–D's four new acceptance flows and the three-edit seed contract first meet a real stack, and `ui-thresholds` is the gate chunk E's G-01 horizontal-scroll fix was pre-empting. Neither has been exercised on this tree before.**
#                                    Prior (session 79): **Seventy-ninth session: the same P5.11 gate run is STILL ALIVE at 14m53s on entry (4/13 — `ruff-check`/`ruff-format`/`mypy`/`import-linter` PASS, still inside `pytest`) — attached, Monitor re-armed persistently on `EXIT=`/FAIL/Traceback/Error/Killed/OOM/`PASS `, NOT relaunched.** Same wrapper PID `1457845` and same `check.sh` PID `1457847` sessions 76–78 recorded: one continuous run, now ~15 minutes in. INBOX had no unhandled items; B1/B2/B3 all resolved, no open blockers. Working tree on entry: **189 paths, 188 of them under `reports/phase-5/` and exactly one outside — session 78's own uncommitted `BUILD/STATE.md` note**, which is committed here (this line). **The 188 report paths get NO wip commit**, same call sessions 74–78 made: the dirt is the live run's output, and freezing a mid-run snapshot is what produced session 73's misleading partial corpus. `git status reports/phase-5` shows **zero `D` entries** at 14m53s — mid-run, so it proves nothing; the load-bearing check is the one at completion. **No source file touched.** The post-`EXIT=` procedure is unchanged and recorded below; do not re-derive it.
      ...
      **PROGRESS (session 72): chunks A, B, C and D are BUILT, COMMITTED and each verified
      ...
      - [ ] **chunk E — AUDIT LEG DONE (`EXIT=0`, session 76); GATE LEG IN FLIGHT.**
            ...
            **63** — a P5.8 screen, and now the worst performer in the build. D4.25 is restated by
            ...
            performance pass.**
            **Gate leg in flight — 11/13 as of session 90, and `playwright-e2e` PASSED**, which
            is the answer the whole relaunch was launched to get: 27a0be2's pinned-`deviceId`
            `storageState` holds against the live stack. Only `puppeteer-audit` (running,
            `audit.mjs` PID `1591614`) and `ui-thresholds` remain. Full `./scripts/check.sh` under `setsid` with
