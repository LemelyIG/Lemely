...
CURRENT PHASE:      **5 IN PROGRESS.** 5.3 (charts) DONE. 5.1, 5.2, 5.4 next.
CURRENT SURFACE:    Phase 5 is a sweep, not a surface. 5.3 touched the student
                    dashboard, the training log, and the two teacher chart
                    screens.
NEXT ACTION:        **Phase 5.1 - the motion sweep**, then 5.2 (celebration
                    register end to end) and 5.4 (reduced-motion audit).

                    5.3 is closed: Nivo installed and gated, one shared theme,
                    two wrappers, four charts moved, `chartTheme.test.ts`
                    verified by inversion. See D5.1.

                    **Read D5.1's first section before writing any motion
                    code.** Its finding generalises past charts: a value that
                    goes through a JS library rather than a CSS class cannot be
                    a `var()` string, because the library parses it. The same
                    is true of any easing or duration handed to a JS animation
                    API. `--dur-*` and the easings live in `:root`, so
                    `duration-slow` does not exist as a utility (D4.6) AND
                    `var(--dur-slow)` is not a number a JS timer accepts. Both
                    failure modes are silent.

                    Standing for 5.1/5.2/5.4 specifically:
                    - **`Reveal` has exactly one call site** (the landing
                      page), and DESIGN.md §9.2 specifies scroll entries as the
                      product-wide baseline. The sweep is the point of 5.1.
                    - **The celebration register is built and partly wired.**
                      `lib/celebration.ts` + `components/ui/celebration.tsx`
                      exist with call sites in `Profile`, `Standings` and
                      `xp-streak`. 5.2 needs the marked-paper result reveal and
                      the streak save. **A leaderboard climb still cannot be
                      celebrated**: no `previousRank` is on the wire, and D4.4
                      refused to invent it. Do not invent it now.
                    - **Charts already have their reduced-motion path** via
                      `useChartAnimation()`, which reads the query live rather
                      than at mount (a chart is long-lived; `Reveal` reads once
                      at mount, deliberately). 5.4 audits everything else.
...
LAST UPDATED:       2026-08-14T09:20+03:00
